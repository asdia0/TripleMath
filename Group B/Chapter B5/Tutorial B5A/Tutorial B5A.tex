...
\begin{document}
    ...
    \solution
        \part
            \begin{align*}
                \der{y}{x} &= a \bp{x \cdot \dfrac1{2\sqrt{x+2}} + \sqrt{x+2} }\\
                &= a \bp{\dfrac{x}{2\sqrt{x+2}} + \dfrac{2(x+2)}{2\sqrt{x+2}}}\\
            ...
            \begin{table}[h]
                ...
                \begin{tabular}{|c|c|c|c|}
                \hline
                $x$          & $\bp{-\dfrac43}^-$ & $-\dfrac43$ & $\bp{-\dfrac43}^+$ \\\hline
                $\der{y}{x}$ & -ve   & 0 & +ve   \\[1ex]\hline
                \end{tabular}
            \end{table}
    ...
    \problem{}
        The function $f$ defined by $f(x) = \ln x - 2\bp{x-\dfrac12}$, where $x \in \R, x > 0$. Find $f'(x)$ and show that the function is decreasing for $x > \dfrac12$. Hence, show that for $x > \dfrac12$, $2\bp{x - \dfrac12} - \ln x > \ln 2$.

    \solution
        We have that $f'(x) = \dfrac1x - 2$. When $x > \dfrac12$, $\dfrac1x < 2 \implies \dfrac1x - 2<0$. Thus, $f'(x) < 0$, whence $f(x)$ is decreasing.

        Note that $f\bp{\dfrac12} = \ln \dfrac12 - 2\bp{\dfrac12 - \dfrac12} = -\ln2$. Since $f(x)$ is decreasing for all $x > \dfrac12$, $f(x) < f\bp{\dfrac12} = -\ln 2 \implies \ln x - 2\bp{x - \dfrac12} < -\ln2 \implies 2\bp{x - \dfrac12} - \ln x > \ln 2$.
