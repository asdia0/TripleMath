...
\title{Tutorial B14\\Differential Equations II}
\author{Eytan Chong}
\date{2024-08-22}
...
\begin{document}
    ...
    \solution
        ...
        For $t > 0$, we have $\derx{y}{t} = 4y - 1 > 0$. Hence, $y$ is concave upward. Thus, the approximation is an underestimate.
    ...
    \solution
        \part
            ...
            \end{align*}
            \boxt{$y(1) \approx 0.756$} Since $\derx{y}{x} = y-x < 0$ for $x$ near 1, $y$ is concave downward near $x = 1$. Thus, the approximation is an overestimate.
    ...
    \solution
        ...
        \part
            For $x > 0$, $\derx{y}{x} = e^y + x > 0$. Hence, $y$ is concave upwards, whence the estimates are underestimates. Since the improved Euler method is more accurate than the Euler method, it will be greater than the value calculated in (a).
    ...
    \solution
        \part
            ...
            For $x > 0$, $\derx{y}{x} = y - x > 0$. Hence, $y$ is concave upwards, whence the value of $y$ is an underestimate.
    ...
    \solution
        ...
        \part
            ...
            By the fundamental theorem of calculus, the area under the graph of $f(x)$ between $x_0$ and $x_1$ is precisely $y_1 - y_0$.

            The Euler method gives the approximation $y_1 - y_0 = \D x \, f(x_0)$. This is represented by the area of the red-shaded rectangle with base $\D x$ and height $f(x_0)$.

            The improved Euler method gives the approximation $y_1 - y_0 = \D x \, \dfrac12 \big[f(x_0) + f(x_1)\big]$. This is represented by the area of the blue-shaded trapezium with base $\D x$ and heights $f(x_0)$ and $f(x_1)$.

            Thus, the improved Euler method gives a better approximation for the area under the graph of $f(x)$ than the Euler method. Thus, the error of the estimate given by the improved Euler method is smaller than that of the Euler method.

            On the interval $[x_0, x_1]$, we have $f(x) > 0$, whence $y$ is concave upwards. Thus, both estimations are underestimates.
    ...
    \solution
        \part
            Let $f(x, y) = 1 + y^2 \tan x$. Let $x_0 = 1$, $y_0 = 1$ and $\D x = 0.1 \implies x_n = 1 + 0.1n$. By the Euler method,
            \begin{align*}
        ...
        \part
            On the interval $x \in [1, 1.2]$, we have $\derx{y}{x} = 1 + y^2 \tan x > 0$, whence $y$ is concave upwards. Thus, the estimates are likely to be underestimates.

        \part
            $f(x, y)$ has a discontinuity at $x = \pi/2 \in (1.5, 1.6)$, whence the Euler method will fail.
