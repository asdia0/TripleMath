...
\begin{document}
    ...
    \solution
        \part
            \begin{align*}
                ...
                &= 3\left(x^2 + 1\right) - \ln \left(x^2 + 1\right) + 2\arctan x + C\\
                &= 3x^2 - \ln\left(x^2 + 1\right) + 2\arctan x + C
            \end{align*}

            \boxt{
                $\displaystyle\int \dfrac{6x^3 + 2}{x^2 + 1} \d x = 3x^2 - \ln\left(x^2 + 1\right) + 2\arctan x + C$
            }
        ...
        \part
            ...
            \begin{alignat*}{2}
                ...
                \implies&&2I &= x^2 \ln x - \dfrac12 x^2\\
                \implies&&I &= \dfrac12 x^2 \ln x - \dfrac14 x^2
            \end{alignat*}
            }
            ...
            \boxt{
                $\displaystyle\int_2^4 x\ln x \d x = 14 \ln 2 - 3$
            }

    \problem{}
        \begin{enumerate}
            \item Use the derivative of $\cos \theta$ to show that $\der{}{\theta} \sec \theta = \sec \theta \tan \theta$.
            \item Use the substitution $x = \sec \theta - 1$ to find the exact value of $\displaystyle\int_{\sqrt2 - 1}^1 \dfrac1{(x+1)\sqrt{x^2 + 2x}} \d x$.
        \end{enumerate}

    \solution
        \part
            \begin{align*}
                \der{}{\theta} \sec \theta &= \der{}{\theta} \dfrac1{\cos\theta}\\
                &= \dfrac1{\cos^2\theta} \cdot \der{}{\theta} \cos\theta\\
                &= \dfrac1{\cos^2\theta}\cdot\sin\theta\\
                &= \dfrac1{\cos\theta} \cdot \dfrac{\sin\theta}{\cos\theta}\\
                &= \sec\theta\tan\theta
            \end{align*}

        \part
            Consider the substitution $x = \sec \theta - 1 \implies \d x = \sec\theta\tan\theta \d \theta$. When $x = 1$, we have $\theta = \dfrac\pi3$. When $x = \sqrt2 - 1$, we have $\theta = \dfrac\pi4$. Also note that $x + 1 = \sec\theta$. Consider $x^2 + 2x$.
            \begin{align*}
                x^2 + 2x &= (\sec \theta - 1)^2 + 2(\sec \theta - 1)\\
                &= \sec^2\theta - 2\sec\theta + 1 + 2\sec\theta - 2\\
                &= \sec^2\theta - 1\\
                &= \tan^2\theta
            \end{align*}

            Hence, $\sqrt{x^2 + 2x} = \tan\theta$. Hence,
            \begin{align*}
                \int_{\sqrt2 - 1}^1 \dfrac1{(x+1)\sqrt{x^2 + 2x}} \d x &= \int_{\tfrac\pi4}^{\tfrac\pi3} \dfrac{\sec\theta\tan\theta}{\sec\theta\tan\theta} \d\theta\\
                &= \int_{\tfrac\pi4}^{\tfrac\pi3} \d \theta\\
                &= \dfrac\pi3 - \dfrac\pi4\\
                &= \dfrac\pi{12}
            \end{align*}

            \boxt{
                $\displaystyle\int_{\sqrt2 - 1}^1 \dfrac1{(x+1)\sqrt{x^2 + 2x}} \d x = \dfrac\pi{12}$
            }
    \problem{}
    ...
    \solution
        ...
        \part
            ...
            \boxt{
                $\displaystyle\int_0^2 \ln\left(9 - x^2\right) \d x = 5\ln5 - 4$
            }
