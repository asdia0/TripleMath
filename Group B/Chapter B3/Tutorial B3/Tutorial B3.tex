...
\begin{document}
    ...
    \solution
        ...
        \part
            ...
            \boxt{$\inv h \colon x \mapsto \dfrac12\left(x + \sqrt{x^2-4} \right), \, x \geq 2$}
    ...
    \solution
        ...
        \part
            \begin{alignat*}{2}
                ...
                \implies&&\left.[\inv f(x)]'\right|_{x = p} &= \left.\dfrac1{f'(\inv f(x))}\right|_{x=p}\\
                \implies&&\dfrac14 &= \left.\dfrac1{f'(\inv f(x))}\right|_{x=p}\\
                \implies&&\left.f'(\inv f(x))\right|_{x=p} &= 4\\
            \end{alignat*}
    ...
    \solution
        ...
        \part
            Note that $h(x) = (x-1)^2 -2$. Hence, $\inv h(x) = 1 + \sqrt{x+2}$ (we reject $\inv h(x) = 1 - \sqrt{x+2}$ since $\ran{\inv h} = \dom h = \R^+$). Further note that $\inv g = e^x - 2$.
            \begin{alignat*}{2}
                &&hg(a) &= -1.5\\
