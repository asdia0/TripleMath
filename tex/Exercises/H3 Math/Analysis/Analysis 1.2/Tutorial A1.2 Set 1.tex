...
\begin{solution}
    ...
    Note that $f(x)$ is decreasing for $x > \e$. Since $\pi > \e$, it follows that \[\frac{\pi^\e}{\e^\pi} = f(\pi) < f(\e) = 1,\] whence $\pi^\e < \e^\pi$.
\end{solution}
...
\end{problem}
\begin{proof}
    Let $\a$ be a root of $\e^x \sin x = 1$. Then $\e^{\a} \sin \a = 1$. Dividing by $\e^{\a}$ and rearranging, we obtain $\e^{-\a} - \sin \a = 0$, so $\a$ is also a root of $f(x) = 0$.

    Let $\a_1$ and $\a_2$ be two distinct roots of $\e^x \sin x = 1$. Then $\a_1$ and $\a_2$ are also roots of $f(x) = 0$, i.e. $f(\a_1) = f(\a_2) = 0$. By Rolle's Theorem, it follows that there exists some $\b \in (\a_1, \a_2)$ such that $f'(\b) = -\e^{-\b} - \cos \b = 0$. Dividing by $\e^{\b}$ and rearranging, we find that $\e^\b \cos \b = -1$. Hence, $\b$ is a root of $\e^x \cos x = -1$. Thus, there is at least one real root of $\e^x \cos x = -1$ (given by $\b$) between any two real roots of $\e^x \sin x = 1$ (given by $\a_1$ and $\a_2$).
\end{proof}
...
\end{problem}
\begin{proof}
    Let $f(x) = \arcsin x$. By the Theorem of the Mean, there exists some $c \in (0.5, 0.6)$ such that \[\frac1{\sqrt{1 - c^2}} = f'(c) = \frac{f(0.6) - f(0.5)}{0.6 - 0.5} = \frac{\arcsin 0.6 - \pi/6}{0.1}.\] Observe that $1/\sqrt{1 - x^2}$ is increasing on $(0.5, 0.6)$. Hence, \[\frac{2\sqrt3}3 = \frac1{\sqrt{1 - 0.5^2}} < \frac1{\sqrt{1 - c^2}} < \frac1{\sqrt{1 - 0.6^2}} = \frac54.\] Thus, \[\frac{2\sqrt3}3 < \frac{\arcsin 0.6 - \pi/6}{0.1} < \frac54.\] Solving for $\arcsin 0.6$, we have \[\frac\pi6 + \frac{\sqrt3}{15} < \arcsin 0.6 < \frac\pi6 + \frac18.\]
\end{proof}
...
\end{problem}
\begin{solution}
    Note that $y = k^2/x$. Hence, $y' = -k^2/x^2$. The tangent to $y = k^2/x$ at $(x_0, k^2/x_0)$ is thus given by \[y - \frac{k^2}{x_0} = -\frac{k^2}{x_0^2} \bp{x - x_0}.\] Simplifying, we have \[y = -\frac{k^2}{x_0^2} x + \frac{2k^2}{x_0}.\] Meanwhile, the line $l$ with gradient $m$ passing through $(t, 2t)$ has equation $y = mx + (2-m)t$. Thus, for $l$ to be tangent to $y = k^2/x$, we require \[-\frac{k^2}{x_0^2} = m \quad \tand \quad \frac{2k^2}{x_0} = (2-m)t.\] Eliminating $x_0$, we see that \[\frac{(2-m)^2 t^2}{m} = \frac{4k^4 / x_0^2}{-k^2 / x_0^2} = -4k^2.\] Multiplying through by $m$, we obtain the desired equation $t^2 (2-m)^2 + 4mk^2 = 0$.

    Expanding this quadratic in $m$, we have $t^2 m^2 + \bp{4k^2 - 4t^2} m + 4t^2 = 0$. Notice that the number of tangents from the point $(t, 2t)$ to the curve $xy = k^2$ is exactly the number of possible $m$'s, which is determined by the discriminant of the above quadratic. One can easily calculate this discriminant to be $\D = 16k^2 \bp{k^2 - 2t^2}$.

    We now examine the number of tangents case by case. Without loss of generality, suppose $k > 0$.

    \case{1} Suppose there are two tangents. Then $D > 0$, so $k^2 - 2t^2 > 0$. Thus, $-k/\sqrt2 < t < k/\sqrt2$.

    \case{2} Suppose there is one tangent. Then $D = 0$, so $k^2 - 2t^2 = 0$. Thus, $t = \pm k/\sqrt2$.

    \case{3} Suppose there are no tangents. Then $D < 0$, so $k^2 - 2t^2 < 0$. Thus, $t < -k/\sqrt2$ or $t > k/\sqrt2$.
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Taking logarithms on both sides, we get $x - \ln x = y - \ln y$, so \[x - y = \ln \frac{x}{y}.\] Let $u = x/y$. Since $0 < x < y$, it follows that $0 < u < 1$. Substituting this into our equation yields $uy - y = \ln u$, whence \[y = \frac{\ln u}{u-1} \quad \tand \quad x = \frac{u \ln u}{u-1}.\] Thus, \[xy = \frac{u \ln^2 u}{(u-1)^2}.\] Let the RHS be $g(u)$.
        ...
        \begin{tikzpicture}[trim axis left, trim axis right]
            \begin{axis}[
                ...
                \addplot[black, very thick, domain=0:0.99] {x * (ln(x))^2 / (x-1)^2};
                \draw[dotted] (0, 1) -- (1, 1);
            \end{axis}
        \end{tikzpicture}
        \end{figure}
...
\begin{solution}
    ...
    \begin{align*}
        \sum_{r = 1}^n r^2 \binom{n}{r} &= \sum_{r = 1}^n \bs{r(r-1) + r} \binom{n}{r}\\
        &= n\bp{n-1} 2^{n-2} + n 2^{n-1}\\
        &= n\bp{n+1} 2^{n-2}.
    \end{align*}
\end{solution}