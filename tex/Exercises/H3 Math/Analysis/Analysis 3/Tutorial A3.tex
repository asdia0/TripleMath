...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Without loss of generality, assume $a \leq b \leq c$. Then \[\frac{a + b}{c} \leq \frac{c + a}{b} \leq \frac{b + c}{a} \quad \tand \quad a^2 + b^2 \leq c^2 + a^2 \leq b^2 + c^2.\] Applying (a), we have 
        \begin{align*}
...
\begin{solution}
    \begin{ppart}
        We begin with the first inequality. By AM-GM, we have $a^2 + b^2 \geq 2ab$, $b^2 + c^2 \geq 2bc$, and $c^2 + a^2 \geq 2ca$. Adding these three inequalities and dividing by two yields $a^2 + b^2 + c^2 \geq ab + bc + ca$. Now, let $a = x/y$, $b = y/z$, and $c = z/x$. Then \[\bp{\frac{x}{y}}^2 + \bp{\frac{y}{z}}^2 + \bp{\frac{z}{x}}^2 \geq \frac{x}{z} + \frac{y}{x} + \frac{z}{y}\] as desired.
...
\begin{proof}
    ...
    Note that $P(1)$ is trivially true: \[\frac11 \sum{i = 1}^1 x_i = x_1 = \prod_{i = 1}^1 x_i^{1/1}.\] $P(2)$ can also be easily proven: expanding $(x_1 - x_2)^2 \geq 0$, we see that $x_1^2 + x_2^2 - 2x_1x_2 \geq 0$. Rearranging, we obtain \[\bp{\frac{x_1 + x_2}{2}}^2 = \frac{x_1^2 + 2x_1x_2 + x_2^2}{4} \geq x_1 x_2.\] Taking square roots, we have \[\frac{x_1 + x_2}{2} \geq \sqrt{x_1 x_2}\] as desired.
    ...
    Suppose now that $P(k+1)$ is true for some $k+1 \in \NN$. We now show that $P(k+1) \implies P(k)$. Taking $x_{k+1} = (x_1 + \dots + x_k)/k$, by $P(k+1)$, we have \[\frac{x_1 + \dots + x_k + (x_1 + \dots + x_k)/k}{k+1} \geq \prod_{i = 1}^{k} x_i^{1/(k+1)} \bp{\frac{x_1 + \dots + x_k}{k}}^{1/(k+1)}.\] The LHS simplifies to \[\frac{x_1 +  \dots + x_k}{k} \geq \prod_{i = 1}^{k} x_i^{1/(k+1)} \bp{\frac{x_1 + \dots + x_k}{k}}^{1/(k+1)},\] so \[\bp{\frac{x_1 + \dots + x_k}{k}}^{k/k+1} \geq \prod_{i = 1}^{k} x_i^{1/(k+1)},\] Raising both sides to the $(k+1)/k$th power, we finally have \[\frac{x_1 + \dots + x_k}{k} \geq \prod_{i = 1}^{k} x_i^k,\] so $P(k+1) \implies P(k)$.
...
\begin{proof}[Proof of \textup{(a)}]
    Let $x = a + b$, $y = b + c$, and $z = c + a$. By AM-GM, \[\frac{z + x}{y} + \frac{y + z}{x} + \frac{x + y}{z} \geq 6 \sqrt[6]{\frac{zxyzxy}{yyxxzz}} = 6.\] Substituting $a, b, c$ back in, we see that \[\frac{2a + b + c}{b + c} + \frac{a + 2b + c}{c + a} + \frac{a + b + 2c}{a + b} = 3 + \frac{2a}{b + c} + \frac{2b}{c + a} + \frac{2c}{a + b} \geq 6.\] Our desired inequality follows immediately.
\end{proof}
...
\begin{solution}
    \begin{ppart}
        Since $x < y$, we have $y - x \neq 0$, so $1/(y-x)$ is real. By the Archimedean property, there exists some $n \in \NN$ such that \[n > \frac1{y-x}.\] Rearranging, we get $ny > 1 + nx$, which is what we wanted.
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        From (b), we have $m_0 - 1 \leq nx$. By (a), we have $m_0 \leq nx + 1 < ny$, so $m_0/n < y$. Meanwhile, from (b), we also have $nx < m_0$, so $x < m_0/n$. Putting the two inequalities together, we see that $x < m_0/n < y$. Taking $r = m_0/n$, which is rational (since $m_0, n \in \NN$), we are done.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Take $f(x) = x^{p-1}$, which is strictly increasing with $f(0) = 0$. Note that $\inv f(x) = x^{1/(p-1)}$. By (a), for all $a, b > 0$, \[ab \leq \int_0^a x^{p-1} \d x + \int_0^b x^{1/(p-1)} \d x = \frac{a^p}{p} + \frac{p-1}{p} b^{p/(p-1)} = \frac{a^p}{p} + \frac{a^q}{q},\] where $q = p/(p-1)$.
    ...
    \begin{ppart}
        ...
        \begin{psubpart}
            ...
            \begin{align*}
                ...
                &\hspace{2em}\leq \bp{\sum_{i = 1}^m \abs{c_i}^p}^{1/p} \bp{\sum_{i = 1}^m \abs{c_i + d_i}^{(p-1)q}}^{1/q} + \bp{\sum_{i = 1}^m \abs{d_i}^p}^{1/p} \bp{\sum_{i = 1}^m \abs{c_i + d_i}^{(p-1)q}}^{1/q}\\
                &\hspace{2em}= \bs{\bp{\sum_{i = 1}^m \abs{c_i}^p}^{1/p} + \bp{\sum_{i = 1}^m \abs{d_i}^p}^{1/p}}\bp{\sum_{i = 1}^m \abs{c_i + d_i}^{p}}^{1/q}
            \end{align*}
