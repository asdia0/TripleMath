...
\begin{solution}
    ...
    \begin{align*}
        ...
        &= 2\int_0^{\infty} \frac{\frac{2}{1+t^2}}{2 - \frac{1-t^2}{1 + t^2}} \d t\\
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        \case{1}[$x < 0$] The inequality simplifies down to \[-\bp{2014\e^x}^2 > -x -1 \implies 2014^2 \e^{2x} < x + 1,\] which is never satisfied for all $x < 0$.

        \case{2}[$0 \leq x \leq 1$] The inequality simplifies down to \[-(x-1) > 2014\e^{-x^2},\] which is clearly never satisfied since $-(x-1) < 0$ for all $x \in [0, 1]$, but $2014 \e^{-x^2} > 0$.

        \case{3}[$x > 1$] The inequality simplifies down to \[-(x-1)^2 > 2014\e^{-x^2}.\] For identical reasons (the LHS is non-positive but the RHS is positive), this is never satisfied.
