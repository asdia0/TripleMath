...
\begin{problem}
    ...
    \noindent\bd{Extension.} If $p$ is a prime and $p \mid 1 + n + n^2 + \dots + n^{p-1}$, then $n \equiv 1 \pmod{p}$.
\end{problem}
\begin{proof}
...
\end{problem}
\begin{proof}
    The characteristic equation of $a_n$ is $m^2 - 5m + 6 = (m-3)(m-2)$. Hence, $a_n = A \cdot 2^n + B \cdot 3^n$ for some constants $A$ and $B$. From the initial conditions, we get $a_1 = 2 = 2A + 3B$ and $a_2 = 4 = 4A + 9B$. Solving, we see that $A = 1$ and $B = 0$. Thus, $a_n = 2^n$.
\end{proof}
...
\end{problem}
\begin{proof}
    Let $b = a - k$ for some integer $k$. Since $\gcd{a, b} = 1$, we have $\gcd{a, k} = 1$. Note also that $\gcd{a+b, a-b} = \gcd{2a, a-b} = \gcd{2a, k}$.
