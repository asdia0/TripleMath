...
\begin{problem}
    ...
    \begin{enumerate}
        \item $1(1!) + 2(2!) + 3(3!) + \dots + n(n!) = (n+1)! - 1$ for all integers $n \geq 1$;
        \item $(-7)^n - 9^n$ is divisible by 16 for all integers $n \geq 1$;
        \item $\bp{1 + 1/n}^n < n$ for all integers $n \geq 3$;
        \item $n^3 < n!$ for all integers $n \geq 6$.
    \end{enumerate}
\end{problem}
\begin{proof}[Proof of \textup{(a)}]
    Let $n \in \NN$, and let $P(n)$ be the statement \[\sum_{i = 1}^{n} i (i!) = (n+1)! - 1.\] The base case $P(1)$ is trivial: \[\sum_{i = 1}^1 i(i!) = 1 = (1 + 1)! - 1.\] Suppose that $P(k)$ is true for some $k \in \NN$. Then
    \begin{align*}
...
\end{proof}
\begin{proof}[Alternative Proof of \textup{(a)}]
    The sum in question telescopes to \[\sum_{i = 1}^{k} i(i!) = \sum_{i = 1}^{k+1} (i+1 - 1)(i!) = \sum_{i = 1}^{k} \bs{(i+1)! - i!} = (k+1)! - 1!.\]
\end{proof}
\begin{proof}[Proof of \textup{(b)}]
    Let $n \in \NN$ and let $P(n)$ be the statement that $16 \mid (-7)^n - 9^n$. The base case $P(1)$ clearly holds, since $(-7)^1 - 9^1 = -16$, which is divisible by 16. Suppose that $P(k)$ is true for some $k \in \NN$. Then
    \begin{align*}
        (-7)^{k+1} - 9^{k+1} &= (-7) (-7)^k - 9 (9)^k\\
        &= (-7) \bs{(-7)^k - 9^k} - 16(9)^k\\
        &\equiv (-7)(0) - (0)(9^n)\\
        &= 0 \pmod{16}.
    \end{align*}
    Thus, $P(k) \implies P(k+1)$. Hence, by the principle of mathematical induction, $P(n)$ is true for all $n \in \NN$.
\end{proof}
\begin{proof}[Alternative Proof of \textup{(b)}]
    Let $u_n = (-7)^n - 9^n$. Then $u_n$ is the general solution of a second-order homogenous linear recurrence relation with characteristic equation $(x+7)(x-9) = x^2 - 2x - 63$. Thus, $u_n = 2u_{n-1} - 63 u_{n-2}$ for $n \geq 2$ with initial conditions $u_0 = 0$ and $u_1 = -16$. Since both $u_0$ and $u_1$ are divisible by 16, and the recurrence relation is homogenous, it follows that $16 \mid u_n = (-7)^n - 9^n$ for all $n \in \ZZ_{\geq 0}$.
\end{proof}
\begin{proof}[Proof of \textup{(c)}]
    Let $n \in \ZZ_{\geq 3}$, and let $P(n)$ be the statement \[\bp{1 + \frac1n}^n < n.\] The base case $P(3)$ is trivial: \[\bp{1 + \frac13}^3 = \frac{64}{27} < 3.\] Suppose that $P(k)$ is true for some integer $k \geq 3$. Then \[\bp{1 + \frac1{k+1}}^{k+1} < \bp{1 + \frac1k}^{k+1} < k \bp{1 + \frac1k} = k + 1.\] Thus, $P(k) \implies P(k+1)$. Hence, by the principle of mathematical induction, $P(n)$ is true for all $n \in \ZZ_{\geq 3}$.
\end{proof}
\begin{proof}[Proof of \textup{(d)}]
    We begin by proving that $n^3 - (n + 1)^2 > 0$ for all integers $n \geq 6$. Firstly, observe that $n^3 - (n+1)^2 = \bp{n^2 - 2}\bp{n - 1} - 3$, which is increasing for $n \geq 6$. Thus, $n^3 - (n+1)^2 \geq \bp{6^2 - 2} \bp{6 - 1} - 3 > 0$.

    Let $P(n)$ be the statement that $n^3 < n!$. We now prove that $P(n)$ is true for all integers $n \geq 6$. The base case is trivial, since $6^3 = 216 < 720 = 6!$. Suppose that $P(k)$ is true for some integer $k \geq 6$. Then \[(k+1)^3 = (k+1) (k+1)^2 < (k+1) k^3 < (k+1) k! = (k+1)!.\] Thus, $P(k) \implies P(k+1)$. Hence, by the principle of mathematical induction, $P(n)$ is true for all $n \in \ZZ_{\geq 6}$.
\end{proof}

\begin{problem}
    Let $f_1, f_2, \dots, f_n, \dots$ be the Fibonacci sequence. That is, the sequence is defined recursively by $f_n = f_{n-1} + f_{n-2}$ for all integers $n \geq 3$, with initial conditions $f_1 = 1$ and $f_2 = 1$.
...
\begin{solution}
    ...
    \end{lemma}
    \begin{proof}
        Let $n = km - r$, where $k, r \in \ZZ$ with $k \in \NN$ and $0 \leq r < m$. Note that $m \mid n \iff r = 0$. Let $P(k)$ be the statement $r = 0 \iff f_m \mid f_{km-r}$.
        ...
        \begin{align*}
            f_{(k+1)m - r} &= f_{km - r} f_{m-1} + f_m f_{km - r + 1}\\
            &= \bp{a f_m + b} f_{m-1} + f_m f_{km -r + 1}\\
    ...
    \end{ppart}
    \begin{ppart}
        Let $P(n)$ be the statement that $f_1 + f_3 + \dots + f_{2n-1} = f_{2n}$. The base case $P(1)$ clearly holds, since $f_1 = 1 = f_2$. Now suppose $P(k)$ holds for some $k \in \NN$. Then
        \begin{align*}
            f_1 + f_3 + \dots + f_{2(k+1) - 1} &= f_1 + f_3 + \dots + f_{2k-1} + f_{2k+1}\\
            &= f_{2k} + f_{2k+1}\\
            &= f_{2k+2}\\
            &= f_{2(k+1)}.
        \end{align*}
...
\begin{problem}
    ...
    \begin{proof}[``Proof''.]
        Let $r$ be any non-zero real number and let the predicate $P(n)$ be $r^n = 1$.
