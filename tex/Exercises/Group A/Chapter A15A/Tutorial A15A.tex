...
\begin{solution}
    ...
    \begin{ppart}
        Note that \[\int_{-1}^{x} f(t) \d t = \frac2\pi \int_{-1}^x \frac{\d t}{1 + t^2} = \frac2\pi \evalint{\arctan t}{-1}{x} = \frac2\pi \arctan x + \frac12.\] Hence, $F(x)$, the cdf of $X$, is given by \[F(x) = \begin{cases}
            0, & x < -1,\\
            (2/\pi) \arctan x + 1/2, & -1 \leq x \leq 1,\\
            1, & x > 1.
        \end{cases}\]
...
\end{problem}
\begin{solution}
    Let $m$ be the median of $X$. Then $F(m) = \sqrt{m} = 1/2$, so the median of $X$ is $m = 1/4$.
...
\begin{solution}
    ...
    \begin{ppart}
        Let $F$ be the cdf of $X$. For $0 \leq x \leq 1$, \[F(x) = \int_0^x \frac{6}{17} t \d t = \frac3{17} x^2.\] For $1 < x \leq 2$, \[F(x) = F(1) + \int_1^x \frac6{17} t^2 \d t = \frac3{17} + \frac{2}{17} \bp{x^3 - 1} = \frac1{17} \bp{2x^3 + 1}.\] Thus, $F$ is given by \[F(x) = \begin{cases}
            0, & x < 0,\\
            (3/17) x^2, & 0 \leq x \leq 1,\\
            (1/17) \bp{2x^3 + 1}, & 1 < x \leq 2,\\
            1, & x > 2.
        \end{cases}\]
    \end{ppart}
    \begin{ppart}
        By inspection, $1 < m \leq 2$. Hence, \[\frac12 = F(m) = \frac{1}{17} \bp{2m^3 + 1},\] which gives $m = 1.55 \todp{2}$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    Observe that $\E{X^2}$ diverges to $\infty$: \[\E{X^2} = \int_{-\infty}^\infty x^2 f(x) \d x = \int_0^2 \frac{x^3}4 \d x + \int_2^\infty \frac{4}{x} \d x > \int_2^\infty \frac{1}{x} \d x \to \infty.\] Hence, the standard deviation $\s$ of $X$ also diverges to $\infty$, since \[\s = \sqrt{\Var{X}} = \sqrt{\E{X^2} - \E{X}^2} > \sqrt{\E{X^2}}.\]
    ...
    Consider $\P{X > a} = 0.005$, which is equivalent to $\P{X \leq a} = 0.995$. By inspection, $a > 2$. Hence, \[\P{X \leq a} = 1 - \frac{2}{a^2} = 0.995,\] so $a = 20$.
\end{solution}
...
\begin{solution}
    ...
    \begin{ppart}
        Let $F$ be the cdf of $X$. Clearly, $F(x) = 0$ for $x < 0$. For $0 \leq x \leq 2$, \[F(x) = F(0) + \int_0^x \frac3{20} t^2 \d t = \frac3{20} \evalint{\frac{t^3}{3}}0x = \frac{x^3}{20}.\] For $2 < x \leq 4$, \[F(x) = F(2) + \int_2^x \frac3{10} (4-t) \d t = \frac{2^3}{20} + \frac3{10} \evalint{4t - \frac{t^2}2}2x = \frac{-3x^2 + 24x - 28}{20}.\] Finally, $F(x) = 1$ for $x > 4$. Thus, \[F(x) = \begin{cases}
            0, & x < 0,\\
            x^3/20, & 0 \leq x \leq 2,\\
            (-3x^2 + 24x - 28)/20, & 2 < x \leq 4,\\
            1, & x > 4.
        \end{cases}\]

        Let $l$ be the lower quartile of $X$. Then $\P{X < l} = 1/4$. By inspection, $0 \leq l \leq 2$. Hence, \[\P{X < l} = \frac{l^3}{20} = \frac14,\] so $l = \sqrt[3]{5}$.
    \end{ppart}
    \begin{ppart}
        We have \[\P{\sqrt[3]{5} < X < 2.2} = F(2.2) - F\bp{\sqrt[3]{5}} = 0.264 \tosf{3}.\]
    \end{ppart}
    \begin{ppart}
        By definition, the probability that an observation of $X$ is less than the median value is 1/2. Hence, the required probability is \[\comb{3}{1} \P{X > 2.2} \bp{\frac12}^2 = \comb{3}{1} \bs{1 - \P{X < 2.2}} \bp{\frac12}^2 = 0.365 \tosf{3}.\]
    \end{ppart}
\end{solution}
...
\begin{problem}
    The cumulative distribution function of a continuous random variable $X$ is given by \[F(x) = \begin{cases}
        0, & x < -2,\\
        k\bp{4x - x^3/3 + 16/3}, & -2 \leq x \leq 2,\\
        1, & x > 2.
    \end{cases}\]
...
\begin{solution}
    \begin{ppart}
        Since $F$ is continuous, we have $F(2) = F\bp{2^+}$, i.e. \[k \bp{4(2) - \frac{2^3}3 + \frac{16}3} = 1,\] which gives $k = 3/32$.
    \end{ppart}
    \begin{ppart}
        Note that \[\der{}{x} \bs{\frac3{32}\bp{4x - \frac{x^3}3 + \frac{16}{3}}} = \frac{3}{32} \bp{4 - x^2}.\] Hence, \[f(x) = \begin{cases}
            (3/32) (4 - x^2), & -2 \leq x \leq 2,\\
            0, & \ow.
        \end{cases}\]
    \end{ppart}
    \begin{ppart}
...
\begin{problem}
    A continuous random variable $X$ has distribution function given by \[F(x) = \begin{cases}
        0, & x < 0,\\
        x^2/12, & 0 \leq x \leq 3,\\
        -x^2/4 + 2x - 3, & 3 < x \leq 4,\\
        1, & x > 4.
    \end{cases}\]

    \begin{enumerate}
        \item Show that the median is $\sqrt{6}$.
        \item Find the corresponding probability density function, and show that the mean is $7/3$.
        \item Find the value of $k$ such that $\P{X < k} = \P{3 < X < 4}$.
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
        Let $m$ be the median of $X$. By inspection, $0 \leq m \leq 3$. Thus, \[\P{X < m} = \frac1{12} m^2 = \frac12,\] which gives $m = \sqrt{6}$.
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Note that \[\P{3 < X < 4} = F(4) - F(3) = 1 - \frac{3^2}{12} = \frac14.\] Hence, by inspection, $0 \leq k \leq 3$. Thus, \[\P{X < k} = \frac{k^2}{12} = \frac14 = \P{3 < X < 4},\] which gives $k = \sqrt{3}$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Note that \[\int \frac1{(x+1)^4} \d x = -\frac1{3 (x+1)^3}.\] Since the probabilities sum to 1, \[1 = \int_{-\infty}^{\infty} f(x) \d x = k \int_0^\infty \frac{1}{(x+1)^4} \d x = k \evalint{-\frac{1}{3(x+1)^3}}0\infty = \frac{k}3,\] so $k = 3$.
        ...
        Consider $\P{X \leq x} = 7/8$: \[\P{X \leq x} = 1 - \frac1{(x+1)^3} = \frac78.\] Then $x = 1$.
    \end{ppart}
    \begin{ppart}
...
\begin{problem}
    The probability that a randomly chosen flight from Stanston Airport is delayed by more than $x$ hours is $(x-10)^2/100$, where $x\in\RR$, $0\leq x\leq10$. No flights leave early, and none is delayed for more than $10$ hours. The delay, in hours, for a randomly chosen flight is denoted by $X$.
...
\begin{solution}
    \begin{ppart}
        Note that \[\P{X > x} = \begin{cases}
            (x-10)^2/100, & 0 \leq x \leq 10,\\
            0, & \ow.
        \end{cases}\] Thus, \[\frac12 = \P{X > m} = \frac1{100} (m-10)^2,\] so $m = 2.93 \tosf{3}$. Note that we reject $m = 17.1$ since $0 \leq m \leq 10$.
    \end{ppart}
    \begin{ppart}
        We have \[F(x) = \P{X \leq x} = 1 - \P{X > x} = \begin{cases}
            0, & x < 0,\\
            1 - (x-10)^2/100, & 0 \leq x \leq 10,\\
            1, & x > 10.
        \end{cases}\]
...
\begin{solution}
    ...
    \begin{align*}
        \E{Y^2} &= \E{\cos^2 X}\\
        &= \frac2\pi \int_0^{\pi/2} \cos[2]{x} \d x\\
        &= \frac2\pi \int_0^{\pi/2} \frac{1 + \cos{2x}}{2} \d x\\
        &= \frac1\pi \evalint{x + \frac{\sin 2x}2}0{\pi/2}\\
        &= \frac12.
    \end{align*}
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        \end{cases}\] Thus,
        \begin{align*}
            \P{10 < Y < 20} &= \P{10 < 4X^3 < 20}\\
            &= \P{\sqrt[3]{5/2} < X < \sqrt[3]{5}}\\
            &= F\bp{\sqrt[3]{5}} - F\bp{\sqrt[3]{5/2}}\\
            &= 0.304 \tosf{3}.
        \end{align*}
    \end{ppart}
    \begin{ppart}
