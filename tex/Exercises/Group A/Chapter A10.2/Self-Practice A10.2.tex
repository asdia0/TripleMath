...
\begin{problem}
    The complex numbers $z$ and $w$ are such that \[\abs{z} = 2, \quad \arg{z} = -\frac{2\pi}3 \quad \tand \quad \abs{w} = 5, \quad \arg{w} = \frac{3\pi}4.\]
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        For $(w/z^2)^n$ to be real, its argument must be an integer multiple of $\pi$, i.e. \[\arg \bp{\frac{w}{z^2}}^n = n \arg{\frac{w}{z^2}} = \frac{n\pi}{12} = k\pi.\] Thus, $n = 12k$ for some integer $k$. It is clear that the smallest value $n$ can be is 12 (occurring when $k = 1$).
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    Two complex numbers are $z_1 = 2\bp{\cos{\pi/18} - \i \sin{\pi/18}}$ and $z_2 = 2\i$.
...
\begin{solution}
    \begin{ppart}
        Note that \[z_1 = 2\bs{\cos{-\frac\pi{18}} + \i \sin{-\frac\pi{18}}} = 2\e^{-\i \pi /18}.\] Thus, \[\frac{z_1^2}{z_1\conj} + z_2 = \frac{z_1^3}{\abs{z_1}^2} + z_2 = \frac{2^3 \e^{-\i \pi/6}}{2^2} + 2\i = 2\bs{\cos{-\frac\pi6} + \i \sin{-\frac\pi6}} + 2\i = \sqrt{3} + \i.\]
    \end{ppart}
    \begin{ppart}
        Let $w = z_1^2/z_1\conj + z_2$. Note that \[\abs{w} = \sqrt{\sqrt{3}^2 + 1^2} = 1 \quad \tand \quad \arg{w} = \arctan{\frac1{\sqrt3}} = \frac\pi6,\] so $w = 2\e^{\i \pi /6}$. Let $z_3 = r\e^{\i \t}$. Since $wz_3$ is real, its argument must be an integer multiple of $\pi$, i.e. \[\arg{w z_3} = \arg{w} + \arg{z_3} = \frac\pi6 + \t = k\pi,\] where $k$ is an integer. Solving for $\t$, we see that \[\t = \frac{\pi \bp{6k - 1}}{6}.\] The only solutions for $\t$ within the specified range $(-\pi, \pi)$ are $\t = -\pi/6$ and $\t = 5\pi/6$. Further, we have \[\frac23 = \abs{wz_3} = \abs{w}\abs{z_3} = 2r,\] so $r = 1/3$. Thus, \[z_3 = \frac13 \bs{\cos{-\frac\pi6} + \i \sin{-\frac\pi6}} \quad \tor \quad z_3 = \frac13 \bp{\cos \frac{5\pi}6 + \i \sin \frac{5\pi}6}.\]
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    The complex numbers $z$ and $w$ satisfy the following equations: \[w - z = 1 - \sqrt3, \quad \i z + w = \bp{\sqrt3 + 1} \i.\] Find $w$ in the form $r\e^{\i \t}$, where $r > 0$ and $-\pi < \t \leq \pi$. Give $r$ and $\t$ in exact form.

    Hence, find the three smallest positive whole number values of $n$ for which $(iw)^n$ is an imaginary number.
\end{problem}
\begin{solution}
    Multiplying the second equation by $\i$ yields $\i w - z = -(1 + \sqrt3)$. Along with the first equation, this gives $w - \i w = (1 - \sqrt3) + (1 + \sqrt3) = 2$, so \[w = \frac2{1-\i} = \frac{2\bp{1 + \i}}{2} = 1 + \i = \sqrt{2} \e^{\i \pi/4}.\]

    For $(\i w)^n$ to be purely imaginary, its argument must be a half-integer multiple of $\pi$, i.e. \[\arg{(\i w)^n} = n \bs{\arg{\i} + \arg{w}} = n \bp{\frac\pi2 + \frac\pi4} = \bp{k + \frac12} \pi\] for some integer $k$. Rearranging, we get \[n = \frac{4k+2}{3}.\] The first three smallest positive values of $n$ are hence $n = 2, 6, 10$ (occurring when $k = 1, 4, 7$ respectively).
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Note that $\e^{-\i \pi/2} = -\i$. Thus, \[\e^{\i (\t - \pi/2)} = \e^{\i \t} \e^{-\i \pi/2} = -\i \bp{\cos \t + \i \sin \t} = \sin \t - \i \cos \t.\]
    \end{ppart}
    \begin{ppart}
        Since $z = \cos \t + \i \sin \t$, we have that
        \begin{align*}
            1 - z^2 &= 1 - \bp{\cos^2 \t - 2\i \cos \t \sin \t - \sin^2 \t}\\
            &= 1 - \bp{1 - 2\sin^2 \t + 2\i \cos \t \sin \t}\\
            &= 2\sin \t \bp{\sin \t - \i \cos \t}.
        \end{align*}
        Thus, \[\arg{1 - z^2} = \arg{\sin \t - \i \cos \t} = \arg{\e^{\i(\t - \pi/2)}} = \t - \pi/2\] by part (a). Also, note that $\abs{\sin \t - \i \cos \t} = \abs{\e^{\i(\t - \pi/2)}} = 1$ by part (a), so $\abs{1 - z^2} = 2\sin \t$, where we note that $\sin \t > 0$ since $0 < \t < \pi/2$.
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Note that \[\arg{\frac{z\conj}{z^3 \bp{1 - z^2}}} = \arg{z\conj} - 3\arg{z} - \arg{1 - z^2} = \bp{-\t} - 3\t - \bp{\t - \frac\pi2} = -5\t + \frac\pi2.\] This must be an integer multiple of $\pi$, i.e. \[-5\t + \frac\pi2 = k \pi\] for some integer $k$. Rearranging, we find that \[\t = \frac{\pi\bp{1 - 2k}}{10}.\] Since $\t \in (0, \pi/2)$, the only possible values of $\t$ are $\t = \pi/10$ and $\t = 3\pi/10$ (corresponding to $k = 0$ and $k = -1$ respectively).
    \end{ppart}
\end{solution}