...
\begin{problem}
    On an Argand diagram, mark and label clearly the points $P$ and $Q$ representing the complex numbers $p$ and $q$ respectively, where \[p = \cos\frac\pi4 + \i\sin\frac\pi4 \quad \tand \quad q = 2\cos\frac\pi4 + 2\i\sin\frac\pi4.\]
...
\begin{solution}
    ...
    Note that $p = \e^{\i\pi/4}$ and $q = 2\e^{\i\pi/4}$, so
    \begin{align*}
    ...
    We have \[p^{13/3}q^{45/2} = \bp{\e^{\i\pi/4}}^{13/3} \bp{2\e^{\i\pi/4}}^{45/2} = 2^{45/2} \e^{\i (161\pi/24)} = 2^{45/2} \e^{\i (17\pi/24)}.\] Hence, $\abs{p^{13/3}q^{45/2}} = \e^{45/2}$ and $\arg{p^{13/3}q^{45/2}} = 17\pi/24$.
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item show that the imaginary part of $q$ is $\cot{\t}/2$.
    \end{enumerate}
\end{problem}
\begin{solution}
    We have \[q = \frac{\e^{\i2\t}}{1 - \e^{\i2\t}} = \frac{\e^{\i\t}}{\e^{-\i\t} - \e^{\i\t}} = \frac{\cos \t + \i \sin \t}{-2\i \sin\t} = -\frac12 - \frac1{2\i} \cot \t
    = -\frac12 + \frac\i2 \cot \t.\] Hence, $\Re q = -1/2$ and $\Im q = \cot{\t}/2$.
\end{solution}

\begin{problem}
    The complex numbers $z$ and $w$ are such that $z = 4\bp{\cos{3\pi/4} + \i\sin{3\pi/4}}$ and $w = 1 - \i\sqrt3$. $z\conj$ denotes the conjugate of $z$.
...
\begin{solution}
    \begin{ppart}
        Note that $z = 4\e^{\i3\pi/4}$ and $w = 2\bp{1/2 - \i\sqrt{3}/2} = 2\e^{-\i\pi/3}$. Hence, \[\frac{w^2}{z\conj} = \frac{\bp{2\e^{-\i(\pi/3)}}^2}{4\e^{-\i(3\pi/4)}} = \frac{4\e^{-\i(2\pi/3)}}{4\e^{-\i(3\pi/4)}} = \e^{\i\pi/12}.\] Thus, $r = 1$ and $\t = \pi/12$.
    \end{ppart}
    \begin{ppart}
        Note that $\bp{w^2 / z\conj}^n = \bp{\e^{\i\pi/12}}^n = \e^{\i n\pi/12}$. Since $\bp{w^2 / z\conj}^n$ is purely imaginary, we have $\arg \bp{w^2 / z\conj}^n = \pi/2+ \pi k$, where $k$ is an integer. Thus, $n\pi/12 = \pi/2 + \pi k$, whence $n =  6 + 12k$. Hence, the set of values that $n$ can take is $\bc{6 + 12k : k \in \ZZ}$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Note that $\angle POQ = \pi - \pi/6 - \pi/4 = 7\pi/12$. Since $\abs{z} = \abs{w}$, we have $OP = OQ$, whence $\angle ROQ = (1/2)(7\pi/12) = 7\pi/24$. Hence, $\arg{w + z} = \pi/4 + 7\pi/24 = 13\pi/24$. Thus, \[\tan{\frac{13}{24}\pi} = \frac{1 + 1/\sqrt2}{1 - \sqrt3/\sqrt2} = \frac{\sqrt2 + 1}{\sqrt2 - \sqrt3} = \frac{2 + \sqrt2}{2 - \sqrt6}.\] However, \[\tan{\frac{13}{24}\pi} = -\tan{\pi - \frac{13}{24}} = -\tan{\frac{11}{24}\pi}.\] Hence, \[\tan{\frac{11}{24}\pi} = -\frac{2 + \sqrt2}{2 - \sqrt6} = \frac{2 + \sqrt2}{\sqrt6 - 2},\] whence $a = 2$ and $b = -2$.
    \end{ppart}
\end{solution}

\clearpage
\begin{problem}
    The complex number $z$ is given by $z = 2\bp{\cos\b + \i\sin\b}$ where $0 < \b < \pi/2$.

    \begin{enumerate}
        \item Show that \[\frac{z}{4 - z^2} = (k\csc\b)\i,\] where $k$ is positive real constant to be determined.
        \item State the argument of $z/(4 - z^2)$, giving your reasons clearly.
        \item Given the complex number $w = -\sqrt3 + \i$, find the three smallest positive integer values of $n$ such that $\bp{z/(4 - z^2)}(w\conj)^n$ is a real number.
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Since $0 < \b < \pi/2$, we know that $\csc \b > 0$. Hence, $\Im{z/(4 - z^2)} > 0$. Furthermore, $\Re{z/(4 - z^2)} = 0$. Thus, $\arg{z/(4 - z^2)} = \pi/2$.
    \end{ppart}
    \begin{ppart}
        Note that $w = -\sqrt{3} + \i = 2\bp{-\sqrt{3}/2 + \i/2} = 2\e^{-\i5\pi/6}$. Hence, \[\arg{\bp{\frac{z}{4 - z^2}}(w\conj)^n} = \frac\pi2 + n\bp{-\frac{5\pi}6} = \pi\bp{\frac12 - \frac{5n}{6}}.\]

        For $\bp{z/(4 - z^2)}(w\conj)^n$ to be a real number, we require $\arg{(z/(4 - z^2))(w\conj)^n} = \pi k$ for some integer $k$. Hence, \[\pi\bp{\frac12 - \frac56 n} = \pi k.\] Simplifying, we see that $3 - 5n = 6k$. Reducing modulo 6, we see that $n \equiv 3$. Hence, the three smallest possible values of $n$ are $3$, $9$ and $15$.
    \end{ppart}
\end{solution}