...
\end{problem}
\begin{solution}
    The even numbers from 20 to 100 inclusive form an AP with common difference 2, first term 20 and last term 100. Since we are adding a total of $(100-20)/2 + 1 = 41$ terms, we get a sum of $41\bp{(20 + 100)/2} = 2460$.
\end{solution}
...
\end{problem}
\begin{solution}
    Let the $n$th term of the geometric series be $ar^{n-1}$, where $1 \leq n \leq k$. We hence have $3r^{k-1} = 384$, which gives $r^k = 128r$. Thus, \[\frac{3(1-r^k)}{1-r} = \frac{3(1-128r)}{1-r} = 765,\] whence $r = 2$.
\end{solution}

\begin{problem}
    \begin{enumerate}
        \item Find the first four terms of the following sequence \[u_{n+1} = \frac{u_n + 1}{u_n + 2},\] with $u_1 = 0$, $n \geq 1$.
...
\begin{solution}
    \begin{ppart}
        Using G.C., the first four terms of $u_n$ are 0, $1/2$, $3/5$ and $8/13$. 
    \end{ppart}
    \begin{ppart}
        \begin{psubpart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        The first term is $1/4$ and the common ratio is $1/2$.
    \end{ppart}
    \begin{ppart}
        As $n \to \infty$, we clearly have $\bp{1/2}^{n+1} \to 0$. Hence, $S_\infty = 1/2$.
    \end{ppart}
\end{solution}

\begin{problem}
    The first term of an arithmetic series is $\ln x$ and the $r$th term is $\ln{xk^{r-1}}$, where $k$ is a real constant. Show that the sum of the first $n$ terms of the series is $S_n = (n/2) \ln{x^2k^{n-1}}$. If $k = 1$ and $x \neq 1$, find the sum of the series $\e^{S_1} + \e^{S_2} + \e^{S_3} + \ldots + \e^{S_n}$.
\end{problem}
\begin{solution}
...
\end{problem}
\begin{solution}
    Let the $n$th term of the geometric progression be given by $ar^{n-1}$. Then, we have $a/(1-r) = 5$, i.e. \[a = 5(1-r). \tag{1}\] Note that the first, fourth, seventh, tenth, $\ldots$ terms forms a new geometric series with common ratio $r^3$: $a, ar^3, ar^6, ar^9, \ldots$. Thus, $a/(1-r^3) = 4$, i.e. \[a = 4(1 - r^3). \tag{2}\] Equating (1) and (2), we have \[5(1-r) = 4(1 - r^3) \implies 4r^3 + 5r + 1 = 0 \implies (r - 1)(4r^2 + 4r - 1) = 0.\] Since $\abs{r} < 1$, we only have $4r^2 + 4r - 1 = 0$, which has solutions $r = (-1+\sqrt2)/2$ or $r = (-1-\sqrt2)/2$. Once again, since $\abs{r} < 1$, we reject $r = (-1-\sqrt2)/2$. Hence, $r = (-1+\sqrt2)/2$.
\end{solution}
...
\begin{solution}
    \begin{ppart}
        ...
        Since $G_1 = A_1$, we have $G_1 = a$. We can thus write $G_n = ar^{n-1}$. From $G_2 = A_4$, we have $ar = a + 3d$, which gives $a = 3d/(r-1)$. From $G_3 = A_6$, we have $ar^2 = a + 5d$. Thus, \[\bp{\frac{3d}{r-1}} r^2 = \frac{3d}{r-1} + 5d.\] Multiplying by $(r-1)/d$ and simplifying, we get $3r^2 - 5r + 2 = 0$ as desired.
    \end{ppart}
    \begin{ppart}
        Note that the roots to $3r^2 - 5r + 2 = 0$ are $r = 1$ and $r = 2/3$. Clearly, $r \neq 1$ else $a = 3d/(r-1)$ would be undefined. Hence, $r = 2/3$, whence the geometric series is convergent.

        Let $S_\infty$ be the sum to infinity of $G_n$. Then $S_\infty = a/(1-r) = 3a$.
    \end{ppart}
    \begin{ppart}
        Note that $d = a(r-1)/3 = -a/9$. Hence, \[S = n \bp{\frac{a + \bs{a + (n-1)d}}{2}} = n \bp{\frac{2a + (n-1)\bp{-a/9}}{2}} = \frac{an}{18}(19 - n).\] Since $S > 4a$, we have \[\frac{n}{18} (19 - n) > 4.\] Using G.C., we see that  $5.23 < n < 13.8$. Since $n$ is an integer, the set of values that $n$ can take on is $\bc{n \in \ZZ : 6 \leq n \leq 13}$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Let $u_n = u_1r^{n-1}$ be the length of the $n$th bar. Since $u_1 = 20$, we have $u_n = 20r^{n-1}$. Since $u_{25} = 5$, we have $r = 4^{-1/24}$. Hence, $u_n = 20 \cdot 4^{-(n-1)/24}$. Now, consider the sum to infinity of $u_n$: \[S_\infty = \frac{u_1}{1 - r} = \frac{20}{1 - 4^{-1/24}} = 356.3 < 357.\] Hence, no matter how many bars there are, the total length of the bars will never exceed 357 cm.
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Let $v_n = a + (n-1)d$ be the length of the wrongly-manufactured bars. Since the length of the 25th bar is still 5 cm, we know $v_{25} = a + 24d = 5$. Now, consider the total lengths of the bars, which is still $L$ cm. \[L = 25\bp{\frac{a + 5}{2}} = 272.26.\] Solving, we see that $a = 16.781$. Hence, $d = (5-a)/24 = -0.491$, and the longest bar is 16.8 = cm long.
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \end{enumerate}
        \item The woman decides that, to assist her in her everyday expenses, she will withdraw the interest as soon as it has been added. She invests \$$y$ at the beginning of each year. Show that, at the end of $n$ years, she will have received a total of \$$n(n+1)y/40$ in interest.
    \end{enumerate}
\end{problem}
\clearpage
\begin{solution}
    \begin{ppart}
        ...
        \end{psubpart}
        \begin{psubpart}
            Let \$$u_nx$ be the amount of money in the account at the end of $n$ years. Then, $u_n$ satisfies the recurrence relation $u_{n+1} = 1.05(1 + u_n)$, with $u_1 = 1.05$. Observe that \[u_1 = 1.05, \quad u_2 = 1.05 + 1.05^2, \quad u_3 = 1.05 + 1.05^2 + 1.05^3, \quad \cdots.\] We thus have \[u_n = 1.05 + 1.05^2 + \cdots + 1.05^n = 1.05 \bp{\frac{1 - 1.05^n}{1 - 1.05}} = 21\bp{1.05^n - 1}.\]Hence, there will be \$$21(1.05^n - 1)x$ in the account after $n$ years.
        \end{psubpart}
        \begin{psubpart}
            Consider the inequality $u_n = 21(1.05^n - 1) \geq 12$, so $n \geq 9.26$. Since $n$ is an integer, the smallest value of $n$ is 10. Hence, after 10 years, he will have at least \$$12x$ in his account for the first time.
        \end{psubpart}
    \end{ppart}
    \begin{ppart}
        After $n$ years, the woman will have \$$ny$ in her account. Hence, the interest she gains at the end of the $n$th year is $ny/20$. Thus, the total interest she will gain after $n$ years is \[\frac{y}{20} + \frac{2y}{20} + \cdots + \frac{ny}{20} = \frac{y}{20} \bp{1 + 2 + \cdots + n} = \frac{y}{20} \cdot \frac{n(n+1)}{2} = \frac{n(n+1)y}{40}.\]
    \end{ppart}
\end{solution}
...
\begin{problem}
    The positive numbers $x_n$ satisfy the relation \[x_{n+1} = \sqrt{\frac92 + \frac1{x_n}}\] for $n = 1, 2, 3, \ldots$.
...
\begin{solution}
    \begin{ppart}
        Observe that \[\t = \lim_{n \to \infty} \sqrt{\frac92 + \frac1{x_n}} = \sqrt{\frac92 + \frac1{\t}}.\] Clearing surds and denominators, we obtain $2\t^3 - 9\t - 2 = (\t + 2)\bp{2\t^2 - 4\t - 1} = 0$. We reject $\t = -2$ since $\t > 0$. We hence consider $2\t^2 -4\t -1 = 0$, which has roots $\t = 1 + \sqrt{3/2}$ and $\t = 1 - \sqrt{3/2}$. Once again, we reject $\t = 1 - \sqrt{3/2}$ since $\t > 0$. Thus, $\t = 1 + \sqrt{3/2}$.
    \end{ppart}
    \begin{ppart}
        Suppose $x_n > \t$. Then \[0 < x_{n+1}^2 = \frac92 + \frac1{x_n} < \frac92 + \frac1\t = \t^2,\] thus $0 < x_{n+1} < \t$.
    \end{ppart}
\end{solution}