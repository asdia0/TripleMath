...
\begin{problem}
    ...
    \renewcommand{\theenumi}{\bd{\Alph{enumi}}:}
    \begin{enumerate}
    ...
    \begin{enumerate}
        \item Find the probability that the first cracked cup appears before the 7th random check in a day under Scheme \bd{A}.
        \item Find the probability that the batch of cups is rejected under Scheme \bd{B}.
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
        Let $X$ be the number of cups checked before the first cracked cup appears under Scheme \bd{A}. Then $X \sim \Geo{1/10}$. Hence, the required probability is $\P{X < 7} = 0.469 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        Let $Y$ and $Z$ be the number of cracked cups in the first and second rounds of Scheme \bd{B}, respectively. Then $Y \sim \Binom{15}{1/10}$ and $Z \sim \Binom{10}{1/10}$.
...
\begin{solution}
    ...
    \begin{ppart}
        Since $L_3 + P_3 \sim \Po{2(3) + 3(3)/5} = \Po{7.8}$, the required probability is $\P{L_3 + P_3 \geq 10} = 0.259 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
