...
\begin{solution}
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|}
        \hline
        \bd{Part} & \bd{Distribution} \\ \hline\hline
        (a) & Binomial \\ \hline
        (b) & Poisson \\ \hline
        (c) & Poisson \\ \hline
        (d) & Geometric \\ \hline
...
\end{problem}
\begin{solution}
    Let $n$ be the number of trials and $p$ be the probability of success of each trial. We have $\m = np = 24$ and $\s^2 = np(1-p) = 20$. Thus, \[p = 1 - \frac{20}{np} = \frac16\] and $n = 24/p = 144$.
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Let $X$ be the number of defective phones in the sample. Then $X \sim \Binom{n}{0.01}$. Consider $\P{X \geq 1} \geq 0.95$, which is equivalent to $\P{X = 0} = 0.99^n \leq 0.05$. Using G.C., we get $n \geq 298.1$. Since $n \in \NN$, the least $n$ is $299$.
    \end{ppart}
    \begin{ppart}
        Take $n = 24$. Then $\P{X \geq 2} = 0.0239$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        We are given that \[\P{A = 15} = \binom{30}{15} p^{15} (1-p)^{15} = 0.06864.\] From this, we see that \[k = p(1-p) = \sqrt[15]{\frac{0.06864}{\binom{30}{15}}} = 0.23790.\] Expanding $k = p(1-p)$ into a quadratic in $p$, we have $p^2 - p + 0.23790 = 0$, so $p = 0.390$ or $0.610$. Since $p < 0.5$, we take $p = 0.390$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \begin{ppart}
        \begin{psubpart}
            The required probability is $\bs{\P{X > 2}}^3 = 0.0130$.
        \end{psubpart}
        \begin{psubpart}
            The required probability is $3! \bs{\P{X = 1} \P{X = 2} \P{X > 2}} = 0.148$.
        \end{psubpart}
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        \case{1}[$\P{X = 2} < \P{X = 3}$] Note that $\P(X = 2) = \binom{8}{2} q^2 (1-q)^{8-2} = 28p^6q^2$. Hence, we require $28p^6q^2 < 56p^5q^3$, which simplifies to $28p < 56(1-p)$, so $p < 2/3$.

        \case{2}[$\P{X = 3} > \P{X = 4}$] Note that $\P{X = 4} = \binom{8}{4} q^4 (1 - q)^{8-4} = 70 p^4 q^4$. Hence, we require $56p^5q^3 > 70p^4q^4$, which simplifies to $56p > 70(1-p)$, so $p > 5/9$.
...
\end{problem}
\begin{solution}
    We have \[3\e^{-\m} \frac{\m^2}{2!} = 3\P{X = 2} = 16\P{X = 4} = 16\e^{-\m} \frac{\m^4}{4!},\] so $\m^2 = \s = 2.25$, whence $\m = 1.5$. Thus, $X \sim \Po{1.5}$.
    \begin{ppart}
        $\P{X \geq 1} = 0.777$.
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        The required probability is $\bs{\P{1 < X < 4}}^2 = 0.142$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        $\P{W_{10} \geq 1} \P{E_{10} \geq 1} = 0.308$.
    \end{ppart}
    \begin{ppart}
        $\P{W_{35} \geq 2} = 0.413$.
    \end{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        The required probability is $\P{X_{12,3} \geq 3} = 0.577 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
...
\end{problem}
\begin{solution}
    Let the number of serious delays in $k$ weeks be $X_k \sim \Po{k}$. We have $\P{X_4 \geq 4} = 0.56653 = 0.567 \todp{3}$.
    
    Let $Y$ be the number of 4-week periods during which at least 4 serious delays occur. Note that $Y \sim \Binom{13}{0.56653}$. Hence, $\P{Y \geq 10} = 0.115 \tosf{3}$.
...
\begin{solution}
    ...
    \begin{ppart}
        $\P{X_8 \geq 3} = 0.620 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        The required probability is $\comb{3}{1} \P{X_8 \geq 3} \P{X_8 = 0} = 0.00309 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        The probability that a given working day has zero demand for XO pies is $\P{X_8 = 0} = 0.40762$. Let the number of days in a week where there is 0 demand for XO pies be $Y \sim \Binom{5}{0.40762}$. Then $\P{Y \leq 1} = 0.985 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    Note that $\P{X_1 > 5} = 0.067308$. Let $Y$ be the number of trays with more than 5 rotten strawberries. We can approximate $Y$ using a Poisson distribution since $n = 60$ is large and $p = 0.067308$ is small. We hence have $Y \sim \Po{np} = \Po{4.0385}$. Then $\P{Y \geq 4} = 0.574 \tosf{3}$.
\end{solution}