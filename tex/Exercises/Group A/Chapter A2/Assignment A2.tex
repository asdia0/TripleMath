...
\begin{problem}
    By considering the graphs of $y = \cos x$ and $y = -x/4$, or otherwise, show that the equation $x + 4\cos x = 0$ has one negative root and two positive roots.
...
\begin{solution}
    ...
    Note that $x + 4\cos x = 0 \implies \cos x = -x/4$. Plotting the graphs of $y = \cos x$ and $y = -x/4$, we see that there is one negative root and two positive roots. Hence, the equation $x + 4\cos x = 0$ has one negative root and two positive roots.
    ...
    We know from the above graph that $\a \in (\pi, 3\pi/2)$. We hence pick $3\pi/2$ as our initial approximation. Using the Newton-Raphson method $x_{n+1} = x_n - f(x_n)/f'(x_n)$ with $x_1 = 3\pi/2$, we have
    \begin{table}[H]
        \centering
        \begin{tabular}{|c|c|}
        \hline
...
\end{problem}
\begin{solution}
    For stationary points, $y' = 3x^2 + 2x = x(3x+2) = 0$, so $x = 0$ or $x = -2.3$. Thus, the coordinates of the stationary points of $y = x^3 + x^2$ are $(0, 0)$ and $(-2/3, 4/27)$.
...
\begin{solution}
    \begin{ppart}
        Consider $f(x) = 2\cos x$. Then $f'(x) = -2\sin x$. Observe that $\sin x$ is increasing on $[1, 1.2]$. Since $\sin 1 > 1/2$, $\abs{f'(x)} > 1$ for all $x \in [1, 1.2]$. Thus, fixed-point iteration fails and will not converge to $\a$.
    ...
    \end{ppart}
    \begin{ppart}
        Consider $f(x) = \cos x + x/2$. Then $f'(x) = -\bp{\sin x - 1/2}$. Since $0 \leq \sin x \leq 1$ for $x \in \bs{0, \pi/2}$, and $[1, 1.2] \subset \bs{0, \frac{\pi}2}$, we know $-1/2 \leq \sin x - 1/2 \leq 1/2$ for $x \in [1, 1.2]$. Thus, $0 \leq \abs{\sin x - 1/2} \leq 1/2$ for $x \in [1, 1.2]$. Hence, fixed-point iteration will work and converge to $\a$.
    ...
    \end{ppart}
    \begin{ppart}
        Consider $f(x) = 2(\cos x + x)/3$. Then $f'(x) = 2(-\sin x + 1)/3$. For fixed-point iteration to converge to $\a$, we need $\abs{f'(x)} < 1$ for $x$ near $\a$. It thus suffices to show that $\abs{-\sin x + 1} < 3/2$ for all $x \in [1, 1.2]$. Observe that $1 - \sin x$ is strictly decreasing and positive for $x \in \bs{0, \pi/2}$. Since $1 - \sin 1 < 3/2$, and $[1, 1.2] \subset \bs{0, \pi/2}$, we have that $\abs{-\sin x + 1} < 3/2$ for all $x \in [1, 1.2]$. Thus, $\abs{f'(x)} < 1$ for $x$ near $\a$. Hence, fixed-point iteration will work and converge to $\a$.
        ...
        For $x \in [1, 1.2]$, $\abs{2(-\sin x + 1)/3} < \abs{-\sin x + 1/2} < 1$. Thus, $x_{n+1} = 2(\cos x_n + x_n)/3$ is the most suitable iteration as it will converge to $\a$ the quickest. Using $F(x_{n+1}) = 2(\cos x_n + x_n)/3$ with $x_1 = 1$,
        \begin{table}[H]
            \centering
            \begin{tabular}{|c|c|}
            \hline
