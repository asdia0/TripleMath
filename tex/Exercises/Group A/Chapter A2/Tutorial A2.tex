...
\begin{problem}
    ...
    \begin{enumerate}
        \item Show that $\a$ is a root of the equation $x = \sqrt{2/(x+2)}$, and use the iterative formula $x_{n+1} = \sqrt{2/(x_n + 2)}$, with $x_1 = 1$, to find $\a$ correct to 2 significant figures.
...
\begin{solution}
    ...
    \begin{ppart}
        We know $f(\a) = 0$. Hence, \[\a^3 + 2\a^2 - 2 = 0 \implies \a^2 (\a + 2) = 2 \implies \a^2 = \frac2{\a + 2} \implies \a = \sqrt{\frac2{\a + 2}}.\] Note that we reject the negative branch since $\a > 0$. We hence see that $\a$ is a root of the equation $x = \sqrt{2/(x+2)}$. Using the iterative formula $x_{n+1} = \sqrt{2/(x_n + 2)}$ with $x_1 = 1$, we have
        \begin{table}[H]
            \centering
            \begin{tabular}{|c|c|}
            \hline
    ...
    \end{ppart}
    \begin{ppart}
        Using the Newton-Raphson method ($x_{n+1} = x_n - f(x_n)/f'(x_n)$) with $x_1 = 1$, we have
        \begin{table}[H]
            \centering
            \begin{tabular}{|c|c|}
            \hline
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        Note that $f'(x) = 1/3 - 1/x$. Using the Newton-Raphson method ($x_{n+1} = x_n - f(x_n)/f'(x_n)$), we have
        \begin{table}[H]
            \centering
            \begin{tabular}{|c|c|}
            \hline
...
\end{problem}
\begin{solution}
    For turning points, $f'(x) = 0$. Since $f'(x) = 3x^2-2x-1 = 0 = (3x+1)(x-1)$, there are stationary points when $x = -1/3$ or $x = 1$, giving the coordinates $(-1/3, -0.815)$ and $(1, -2)$.
    ...
    \begin{center}\tikzsetnextfilename{115}
        \begin{tikzpicture}[trim axis left, trim axis right]
            \begin{axis}[
                ...
                \draw[black!70,TwoMarks=0.1] (1.5,0) -- (1.8,0);
...
\begin{solution}
    ...
    Observe that $f(9)f(10) = (-40901)(50) < 0$. Thus, there must be a root in the interval $(9, 10)$. We now use the Newton-Raphson method ($x_{n+1} = x_n - f(x_n)/f'(x_n)$) with $x_1 = 9$ as the first approximation.
    \begin{table}[H]
        \centering
        \begin{tabular}{|c|c|l|c|c|}
        \cline{1-2} \cline{4-5}
        $n$ & $x_n$ &  & $n$ & $x_n$ \\ \cline{1-2} \cline{4-5} \cline{1-2} \cline{4-5} 
        1 & 9 &  & 4 & 9.9901221 \\ \cline{1-2} \cline{4-5} 
        2 & 10.2178921 &  & 5 & 9.9899912 \\ \cline{1-2} \cline{4-5} 
        3 & 10.0017491 &  & 6 & 9.9899900 \\ \cline{1-2} \cline{4-5} 
        \end{tabular}
    \end{table}
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        From the graph, there is at least one real root to the equation $\ln{x-1} = -2x$. Since $y = \ln{x-1}$ is strictly increasing and $y = -2x$ is strictly decreasing, this root is unique. Furthermore, since $y=-2x$ is negative for all $x > 0$ and $y = \ln{x-1}$ is negative only when $1 < x < 2$, it follows that the root must lie between 1 and 2.

        Let $f(x) = \ln{x-1} + 2x$. Then $f'(x) = 1/(x-1) + 2$. Note that the Newton-Raphson method is given by $x_{n+1} = x_n - f(x_n)/f'(x_n)$.
...
\begin{solution}
    ...
    \begin{table}[H]
        ...
        \cline{1-2} \cline{4-5}
        $n$ & $x_n$ &  & $n$ & $x_n$ \\ \cline{1-2} \cline{4-5} \cline{1-2} \cline{4-5}
        0 & 4.5 &  & 5 & 4.53175 \\ \cline{1-2} \cline{4-5} 
        1 & 4.51223 &  & 6 & 4.53333 \\ \cline{1-2} \cline{4-5} 
        2 & 4.52038 &  & 7 & 4.53437 \\ \cline{1-2} \cline{4-5} 
        3 & 4.52579 &  & 8 & 4.53506 \\ \cline{1-2} \cline{4-5} 
        4 & 4.52937 &  & 9 & 4.53551 \\ \cline{1-2} \cline{4-5} 
        \end{tabular}
    \end{table}
    Hence, $\b = 4.54 \tosf{3}$.

    Note that $x = 3\ln x$, so $x = \e^{x/3}$. Observe also that \[\der{}{x} \e^{x/3} = \frac13 \e^{x/3} \in (-1, 1)\] for all $1 < x < 2$. Thus, the iterative formula $x_{n+1} = F(x_n)$ will converge, whence $F(x) = \e^{x/3}$ is suitable for computing $\a$.
\end{solution}
...
\begin{solution}
    ...
    \begin{ppart}
        Let $g_1(x) = (15-x^3)/3$. Then $g'_1(x) = -x^2$. For values of $x$ near 2, $\abs{g'_1(x)} > 1$. Hence, the iterative formula $x_{n+1} = g_1(x_n)$ will diverge and $g_1(x)$ is unsuitable.
    \end{ppart}

    \begin{ppart}
        Let $g_2(x) = 15/(x^2 + 3)$. Then $g'_2(x) = -30x/(x^2 + 3)^2$. For values of $x$ near 2, $\abs{g'_2(x)} > 1$. Hence, the iterative formula $x_{n+1} = g_2(x_n)$ will diverge and $g_2(x)$ is unsuitable.
    \end{ppart}
...
\end{problem}
\begin{solution}
    Using the point-slope formula, the equation of the line that passes through both $(a, f(a))$ and $(b, f(b))$ is \[y - f(a) = \frac{f(a) - f(b)}{a - b} (x - a).\] The $x$-intercept of this line lies near $(\a, 0)$. Thus, \[0-f(a) \approx \frac{f(a) - f(b)}{a-b} (\a - a).\] Rearranging, we get \[\a \approx \frac{bf(a) - af(b)}{f(a) - f(b)}.\]
...
\begin{problem}
    ...
    The diagram shows a sketch of the graph $y = x/5 - \ln{x+2}$. Find the $x$-coordinate of the minimum point $M$ on the graph, and verify that $y$ is positive when $x = 20$.

    Show that the gradient of the curve is always less than $1/5$. Hence, by considering the line through $M$ having gradient $1/5$, show that the positive root of the equation $x/5 - \ln{x+2} = 0$ is greater than 8.
...
\end{problem}
\begin{solution}
    For stationary points, $y' = 1/5 - 1/(x+2) = 0$, so $x = 3$. By the second derivative test, we see that $y'' = 1/(x+2)^2 > 0$, so $x = 3$ corresponds to the minimum point $M$.
    ...
    Let $(\a, 0)$ be the coordinates of the root of the line through $M$ having gradient $1/5$. We know that the coordinates of $M$ are $(3, 3/5 - \ln5)$. Taking the gradient of the line segment joining $M$ and $(\a, 0)$, we get \[\frac{(3/5 - \ln5) - 0}{3 - \a} = \frac15,\] so $\a = 5\ln 5 = 8.05 > 8$. Since the gradient of the curve is always less than $1/5$, which is the gradient of the line, it stands to reason that $\a$ represents the lowest possible value of the positive root of the curve. Hence, the positive root of the equation $x/5 - \ln{x+2} = 0$ is greater than 8.
...
\begin{problem}
    \begin{enumerate}
        ...
        \item The equation $\sec^2x - \e^x = 0$ has a root $\a$ in the interval $[1.5, 2.5]$. A student uses linear interpolation once on this interval to find an approximation to $\a$. Find the approximation to $\a$ given by this method and comment on the suitability of the method in this case.
...
\begin{problem}
    ...
    \begin{enumerate}
        \item Express g(x) in terms of $x$, where $g(x) = x - f(x)/f'(x)$.
    \end{enumerate}
...
\begin{solution}
    \begin{ppart}
        We know \[f'(x) = -\frac{x}{\sqrt{1-x^2}} - \sin x.\] Hence, \[g(x) = x - \frac{\sqrt{1 - x^2} + \cos x - 1}{-x/\sqrt{1-x^2} - \sin x}.\]
    \end{ppart}
    \begin{ppart}
        \begin{psubpart}
