...
\begin{problem}
    A continuous random variable $X$ has a uniform distribution over the interval $[0, n]$. Write down $\E{X}$ and show that $\Var{X} = n^2/12$. Denoting the expectation and standard deviation by $\m$ and $\s$ respectively, evaluate $\P{\abs{X - \m} < \s}$.
\end{problem}
\begin{solution}
...
\begin{solution}
    \begin{ppart}
        Since probabilities sum to 1, \[1 = \int_{-\infty}^\infty f(x) \d x = \int_0^\infty k\e^{-\l x} \d x = \evalint{\frac{-k\e^{-\l x}}{\l}}0\infty = \frac{k}{\l},\] so $k = \l$.
    \end{ppart}
    \begin{ppart}
    ...
    \begin{ppart}
        ...
        \begin{align*}
            ...
            &= \int_0^\infty \l x^2 \e^{-\l x} \d x\\
            &= \evalint{-x^2 \e^{-\l x}}0\infty + \frac2{\l} \int_0^\infty \l x \e^{-\l x} \d x\\
            &= \frac2\l \E{X} = \frac2{\l^2}.
        \end{align*}
        Thus, \[\Var{X} = \E{X^2} - \E{X}^2 = \frac2{\l^2} - \bp{\frac1\l}^2 = \frac1{\l^2}.\]
    \end{ppart}
    \begin{ppart}
        Let $F$ be the cdf of $X$. For $x \geq 0$, \[F(x) = \int_0^x f(t) \d t = \int_0^x \l \e^{-\l t} \d t = \evalint{-\e^{-\l t}}0x = 1 - \e^{-\l x}.\] Let $m$ be the median. Then \[\frac12 = F(m) = 1 - \e^{-\l m},\] so $m = \ln{2}/\l$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        We have $\P{Z > c} = 0.025$, so $c = 1.96$.
    \end{ppart}
    \begin{ppart}
        We have $\P{0 \leq Z \leq c} < 0.35$, so $\P{Z \leq c} < 0.85$ and thus $c < 1.04$.
    \end{ppart}
\end{solution}
...
\end{problem}
\begin{solution}
    Since $X \Uni{0}{1}$, we clearly have $\E{X} = 0.5$ and $\Var{X} = 1/12$.
...
\begin{solution}
    Since $U \sim \Uni{2f}{3f}$, we have $u-f>0$ and \[F_U(u) = \begin{cases}
        0, & u < 2f,\\
        u/f - 2, & 2f \leq u \leq 3f,\\
        1, & u > 3f.
    \end{cases}\] Note also that \[v = \frac{uf}{u-f}.\] Thus,
    \begin{align*}
        F_V(v) &= \P{V < v}\\
        &= \P{\frac{uf}{u-f} < v}\\
        &= \P{u > \frac{vf}{v-f}}\\
        ...
        &= \begin{cases}
            1, & vf/(v-f) < 2f,\\
            3 - v/(v-f), & 2f \leq vf/(v-f) \leq 3f,\\
            0, & vf/(v-f) > 3f,
        \end{cases}\\
        &= \begin{cases}
            1, & v > 2f,\\
            3 - v/(v-f), & 3f/2 \leq v \leq 2f,\\
            0, & v < 3f/2.
        \end{cases}
    \end{align*}
    Differentiating, we obtain the pdf of $V$: \[f_V(v) = F'_V(v) = \begin{cases}
        f/(v-f)^2, & 3f/2 \leq v \leq 2f,\\
        0, & \ow.
    \end{cases}\] The range of $V$ is hence $[3f/2, 2f]$.
    ...
    Let $m$ be the median. We have \[\frac12 = F_V(m) = 3 - \frac{m}{m-f},\] so $m = 5f/3$.
\end{solution}
...
\end{problem}
\begin{solution}
    Since probabilities sum to 1, \[1 = \int_{-\infty}^\infty f(t) \d t = \int_0^\infty \frac1a \e^{-t/b} \d t = \evalint{-\frac{b}{a} \e^{-t/b}}0\infty = \frac{b}{a},\] so $a = b$.

    Note that the cdf of $T$ is given by \[F(t) = \begin{cases}
        0, & t < 0,\\
        1 - \e^{-t/a}, & t \geq 0.
    \end{cases}\] We are given that $\P{T > 2000} = 0.426$. Thus, \[0.574 = \P{T \leq 2000} = 1 - \e^{-2000/a},\] so $a = 2340 \tosf{3}$.
...
\begin{solution}
    \begin{ppart}
        We have \[20 = \E{T} = \int_{-\infty}^\infty t f(t) \d t = \int_0^\infty \m t \e^{-\m t} \d t = \evalint{-t\e^{-\m t} - \frac1\m \e^{-\m t}}0\infty = \frac1\m,\] so $\m = 0.05$.
    \end{ppart}
    \begin{ppart}
        Note that the cdf of $T$ is given by \[F(t) = \begin{cases}
            0, & t \leq 0,\\
    ...
    \end{ppart}
    \begin{ppart}
        Let $t$ be the required time (in hours). We have $\P{T \geq t} = 0.8$, so $\P{T < t} = 1 - \e^{-t/20} = 0.2$. Solving, we get $t = 4.46$. Thus, the shift should be $4.46$ hours long.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    \begin{ppart}
        Since probabilities sum to 1, \[1 = \int_{-\infty}^\infty f(t) \d t = \int_0^\infty a t \e^{-bt} \d t = a \frac{1!}{b^{1+1}} = \frac{a}{b^2},\] so $a = b^2$. Since the mean of $T$ is $1.5$, \[1.5 = \int_{-\infty}^\infty t f(t) \d t = \int_0^\infty a t^2 \e^{-bt} \d t = a \frac{2!}{b^{2+1}} = \frac{2a}{b^3},\] so $a = 3b^3/4$. Solving the two equations simultaneously, we get $a = 16/9$ and $b = 4/3$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Note that $Y - X \sim \Normal{12-10}{4+9} = \Normal{10}{13}$, so $\P{Y < X} = \P{Y - X < 0} = 0.290 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        Note that $4X + 5Y \sim \Normal{4(10) + 5(12)}{4^2 (4) + 5^2 (9)} = \Normal{100}{289}$. Thus, $\P{4X + 5Y > 90} = 0.722 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        Note that $X_1 + X_2 \sim \Normal{2(10)}{2(4)} = \Normal{20}{8}$. Hence, from $\P{X_1 + X_2 > a} = 1/4$, we have $a = 21.9 \tosf{3}$.
    \end{ppart}
\end{solution}
...
\end{problem}
\begin{solution}
    Let $W$ g be the weight of a Sunny brand orange. We are given $\P{W > 379} = 0.37$, which is equivalent to $\P{W \leq 379} = 0.63$. Normalizing this, we get \[0.33185 = z = \frac{x - \m}{\s} = \frac{379 - \m}{\s}.\] We are also given $\P{366 < W < 379} = 0.40$, so $\P{W \leq 366} = 1 - \P{W > 366} = 1 - (0.40 + 0.37) = 0.23$. Normalizing this, we get \[-0.73885 = z = \frac{x - \m}{\s} = \frac{366 - \m}{\s}.\] Solving the two equations simultaneously, we get $\m = 375 \tosf{3}$ and $\s = 12.1 \tosf{3}$.

    The required probability is given by $\comb{3}{1} \P{W > 379} \bs{\P{366 < W < 379}}^2 = 0.178 \tosf{3}$.
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Note that \[\ol{M} = \frac{M_1 + M_2 + \dots + M_n}{n} \sim \Normal{2.5}{\frac{0.1^2}{n}}.\] Using G.C., from $\P{\ol{M} > 2.53} = 0.0668$, we have $n = 25$.
    \end{ppart}
    \begin{ppart}
        Let the random variable $T$ cm be the total length of 21 mechanics and 24 statistics textbooks. Then
        \begin{align*}
            T &= M_1 + \dots + M_{21} + S_1 + \dots + S_{24}\\
            &\sim \Normal{21(2.5) + 24(2.0)}{21\bp{0.1^2} + 24\bp{0.08^2}} = \Normal{100.5}{0.3636}.
        \end{align*}
        Thus, the probability that the textbooks will fit on the shelf is $\P{T < 100} = 0.203 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        Let the random variable $D$ cm be the difference between the total thickness of 4 statistics textbooks and 3 times the thickness of 1 mechanics textbook. Then
        \begin{align*}
            D &= S_1 + \dots + S_4 - 3 M\\
            &\sim \Normal{4(2.0) - 3(2.5)}{4\bp{0.08^2} + 3^2\bp{0.1^2}} = \Normal{0.5}{0.1156}.
        \end{align*}
        The required probability is thus $\P{D < 0} = 0.0707 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Let the number of rods longer than $1.4$ m be $X$. Note that $\P{L > 1.4} = 0.45243$. Thus, $X \sim \Binom{8}{0.45243}$, and $\P{X \geq 3} = 0.784 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \begin{ppart}
        Note that \$5 and \$4 correspond to 250 g and 200 g of ice cream respectively. We wish to find the probability that exactly one boy gets more than 250 g of ice cream, while the rest get less than 200 g of ice cream each, which is given by $\comb{4}{1} \P{X > 250} \bp{\P{X < 200}}^3 = 0.0239 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        Note that the difference in cost $D$ has distribution
        \begin{align*}
            D &= X_1 + \dots + X_4 - 2Y\\
            &\sim \Normal{4(200) - 2(350)}{4\bp{30^2} + 2^2 \bp{60^2}} = \Normal{100}{18000}.
        \end{align*}
        Thus, the required probability is $\P{D > 0} = 0.772 \tosf{3}$.
    \end{ppart}
