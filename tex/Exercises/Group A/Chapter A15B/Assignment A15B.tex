...
\begin{solution}
    ...
    \begin{align*}
        ...
        &= \P{r\t < l} + \P{r\bp{2\pi - \t} < l}\\
        &= \P{\t < l/r} + \P{\t > 2\pi - l/r}\\
        &= \frac{l}{2\pi r} + \frac{l}{2\pi r}\\
        &= \frac{l}{\pi r}.
    \end{align*}
    Let $m$ be the median of $L$. Then \[F_L(m) = \frac{m}{\pi r} = \frac12,\] so $m = \pi r/2$. Thus, both the mean and median values of the shorter arc $PQ$ are $\pi r/2$.
    ...
    \begin{align*}
        \E{X^2} &= 4r^2 \E{\sin^2 \frac{\t}{2}}\\
        &= 4r^2 \int_0^{2\pi} \sin[2]{\frac{\t}{2}} \frac1{2\pi} \d \t\\
        &= \frac{2r^2}{\pi} \int_0^{2\pi} \frac{1 - \cos \t}{2} \d \t\\
        &= \frac{r^2}{\pi} \evalint{\t - \sin \t}0{2\pi}\\
        &= 2r^2.
    \end{align*}
    Hence, \[\Var{X} = \E{X^2} - \E{X}^2 = 2r^2 - \bp{\frac{4r}{\pi}}^2 = 2r^2 \bp{1 - \frac{8}{\pi^2}}.\]

    Observe that \[2r \sin \frac\t2 = X > r\sqrt3,\] so $\sin{\t/2} > \sqrt3/2$, hence $\t \in \bp{2\pi/3, 4\pi/3}$. Since $\t$ is uniform on $[0, 2\pi)$, \[\P{X > r\sqrt{3}} = \frac{4\pi/3 - 2\pi/3}{2\pi - 0} = \frac13.\]
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        We require $\P{X < M} \leq 1/5$, which is equivalent to $\P{X \geq M} = \e^{-0.05 M} \geq 4/5$. Using G.C., $M \in \bc{1, 2, 3, 4}$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        \begin{align*}
            T &= B_1 + B_2 - 3G\\
            &\sim \Normal{2(52) - 3(42)}{2\bp{4^2} + 3^2 \bp{5^2}} = \Normal{-22}{257}.
        \end{align*}
        The required probability is $\P{T > 0} = 0.0850 \tosf{3}$.
    \end{ppart}
    \begin{ppart}
        Let \[\ol{G} = \frac{G_1 + \dots + G_{10}}{10} \sim \Normal{42}{\frac{5^2}{10}}.\] The required probability is $\P{\ol{G} < 41} = 0.264 \tosf{3}$.
    \end{ppart}
\end{solution}