...
\begin{problem}
    ...
    \begin{table}[H]
        ...
        \hline
        \bd{Performing Arts Group} & \bd{Choir} & \bd{Chinese Orchestra} & \bd{Concert Band} & \bd{Dance} \\ \hline
...
\begin{solution}
    ...
    From the graph, the $x$-coordinates of the intersection points are $-2.51$, $1.92$ and $2.09$. Hence, $x \leq -2.51$ or $-2 \leq x \leq 1.92$ or $x \geq 2.09$.
\end{solution}
...
\end{problem}
\begin{solution}
    Note that the discriminant of $x^2 + 2x + 3 = 0$ is $\D = 2^2 - 4(1)(3) = -8 < 0$. Since the $y$-intercept is positive, it follows that $x^2 + 2x + 3$ is always positive for real $x$.

    Consider the inequality \[\frac{x^2 + 2x + 3}{3 + 2x - x^2} \leq 0.\] Since $x^2 + 2x + 3$ is always positive, it suffices to solve $3 + 2x - x^2 \leq 0$. Observe that the roots of $3 + 2x - x^2 = 0$ are $x = 3$ and $x = -1$. Since $3 + 2x - x^2$ is concave down, we have $x \leq -1$ or $x \geq 3$.

    Replacing $x$ with $\abs{x}$, we get $\abs{x} \leq -1$ (no solutions) or $\abs{x} \geq 3$, whence $x \leq -3$ or $x \geq 3$. The solution set is thus $\bc{x \in \RR : x \leq -3 \tor x \geq 3}$.
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        \item $\displaystyle\frac{2(\ln x)^2 - 7\ln x + 6}{(\ln x)^2 - \ln x - 2} > 1$
        \item $\displaystyle\frac{2 - 7x + 6x^2}{1 - x - 2x^2} \geq 1$
    \end{enumerate}
\end{problem}
\begin{solution}
    Moving all terms to one side, we get \[\frac{2x^2 - 7x + 6}{x^2 - x - 2} -1 = \frac{x^2 - 6x + 8}{x^2 - x - 2} \geq 0.\] Note that $x^2 - 6x + 8$ factors as $(x-2)(x-4)$ while $x^2 - x - 2$ factors as $(x-2)(x+1)$. Hence, \[\frac{x-4}{x+1} \geq 0,\] which implies that $(x-4)(x+1) \geq 0$, so $x < -1$ or $x \geq 4$. Note that $x \neq -1$ since $x^2 - x - 2 \neq 0$.

    \begin{ppart}
        Replacing $x$ with $\ln x$, we get \[\ln x < -1 \quad \tor \quad \ln x \geq 4,\] whence $0 \leq x < \e^{-1}$ or $x \geq \e^4$.
    \end{ppart}
    \begin{ppart}
        Replacing $x$ with $1/x$, we get \[\frac1x < -1 \quad \tor \quad \frac1x \geq 4.\] Hence, $-1 < x < 0$ or $0 < x \leq 1/4$. However, $x = 0$ also satisfies the inequality, so the solution is $-1 < x \leq 1/4$.
    \end{ppart}
\end{solution}

\begin{problem}
    It is given that \[y = \frac{x^2 + x + 1}{x - 1},\] where $x \in \RR$, $x \neq 1$. Without using a calculator, find the set of values that $y$ can take.
\end{problem}
\begin{solution}
    Clearing denominators and rearranging, we obtain the following quadratic in $x$: \[x^2 + (1-y)x + (1+y) = 0.\] Since we are interested in the set of values that $y$ can take, this quadratic must have real roots, so the discriminant $\D$ should be non-negative: \[\D = (1-y)^2 - 4(1 + y) = y^2 - 6y - 3 \geq 0.\] The roots to $y^2 - 6y - 3 = 0$ are $y = 3 \pm 2\sqrt3$, whence the solution set is \[\bc{y \in \RR: y \leq 3 - 2\sqrt3 \tor y \geq 3 + 2\sqrt3}.\]
\end{solution}
...
\end{problem}
\begin{solution}[1]
    Substituting $n = 1, 2, 3, 4$ into the equation, we get the system \[\systeme{a + b + c + d = 1, 16a + 8b + 4c + 2d = 9, 81a + 27b + 9c + 3d = 36, 256a + 64b + 16c + 4d = 100}.\] Solving, we have $a = 1/4$, $b = 1/2$, $c = 1/4$ and $d = 0$.
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Note that the $x$-coordinate of the point of intersection between $y = ax + 4$ and $y = x + 2$ is $x = -2/(a-1)$, while the $x$-coordinate of the point of intersection between $y = ax + 4$ and $y = -(x+2)$ is $x = -6/(a+1)$.

        Now consider the inequality $\abs{x + 2} < ax + 4$.

        \case{1}[$a > 2$] $y = ax + 4$ only intersects the line $y = x + 2$. Hence, $x > -2/(a-1)$.
        ...
        \case{2}[$1 \leq a \leq 2$] $y = ax + 4$ only intersects the line $y = -(x + 2)$. Hence, $x \geq -6/(a+1)$.
        ...
        \case{3}[$-1 < a < 1$] $y = ax + 4$ intersects both $y = x + 2$ and $y = -(x+2)$. Hence, $-6/(a+1) < x < -2/(a-1)$.
        ...
        \case{4}[$a \leq -1$] $y = ax + 4$ only intersects the line $y = x + 2$. Hence, $x \leq -2/(a-1)$.
