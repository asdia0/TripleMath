...
\begin{problem}
    ...
    \begin{table}[H]
        ...
        \hline
        \bd{Country} & \bd{Housing} & \bd{Food} & \bd{Incidental Expenses}  \\\hline
...
\begin{solution}
    \begin{ppart}
        Note that $x^2 - 9 = (x+3)(x-3)$. Thus, we may rewrite the inequality as
        \begin{align*}
            0 &\geq (x+3)\bp{x^2 - 3x + 1} - \bp{x^2 - 9}\\
            &= (x+3)\bp{x^2 - 3x + 1} - (x+3)(x-3)\\
            &= (x+3)\bp{x^2 -4x + 4}\\
            &= (x+3)(x-2)^2.
        \end{align*}
    ...
    \end{ppart}
    \begin{ppart}
        Note that $3-x^2 \neq 0$, so $x \neq \pm \sqrt{3}$. Putting all terms on one side, the given inequality becomes \[\frac{x^2-2x+4}{3-x^2} = \frac{7-2x}{3-x^2} - 1 \leq 0.\] Completing the square, we see that $x^2 - 2x + 4 = (x-1)^2 + 3 > 0$. Thus, it suffices to solve $3-x^2 \leq 0$, which gives $x < -\sqrt3$ or $x > \sqrt3$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Note that $x^2 + 3x + 2 = (x+1)(x+2) \neq 0$ and $x + 2 \neq 0$, whence $x \neq -1, -2$. Multiplying the given inequality through by $(x+1)^2(x+2)^2$, we get $(3x+4)(x+2)(x+1) \geq (x+2)(x+1)^2$, which simplifies to $(x+2)(x+1)(2x+3) \geq 0$.
        ...
        \begin{center}\tikzsetnextfilename{27}
            ...
            \end{tikzpicture}
        \end{center}
        Thus, $-2 < x \leq -3/2$ or $x > -1$.
    \end{ppart}
    \begin{ppart}
        Observe that $\abs{x}^2 = x^2$. Hence, with the map $x \mapsto \abs{x}$, we obtain $-2 < \abs{x} \leq -3/2$ or $\abs{x} > -1$. The second condition is satisfied for all real $x$. Hence, the solution set is $\RR$.
    \end{ppart}
\end{solution}    
