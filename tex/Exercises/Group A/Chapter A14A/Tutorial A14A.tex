...
\begin{problem}
    A computer can give independent observations of a random variable $X$ with probability distribution given by $\P{X = 0} = 3/4$ and $\P{X = 2} = 1/4$. It is programmed to output a value for the random variable $Y$ defined by $Y = X_1 + X_2$, where $X_1$ and $X_2$ are two observations of $X$.

    Tabulate the probability distribution of $Y$ and show that $\E{Y} = 1$.

    The random variable $T$ is defined by $T = Y^2$. Find $\E{T}$ and show that $\Var{T} = 63/4$.
\end{problem}
\begin{solution}
...
\begin{problem}
    The discrete random variable $X$ takes values $-1$, 0, 1 with probabilities $1/4$, $1/2$, $1/4$ respectively. The variable $\ol{X}$ is the mean of a random sample of 3 values of $X$ (i.e. $X_1$, $X_2$ and $X_3$ are independent random variables).

    Tabulate the probability distribution of $\ol{X}$, and use your values to calculate $\Var{\ol{X}}$. Hence, verify that $\Var{\ol{X}} = \Var{X}/3$ in this case.
\end{problem}
\begin{solution}
    By symmetry, we have $\P{\ol{X} = -n} = \P{\ol{X} = n}$. Now, notice that the only way to get a total score of 3 is to have $X_1 = X_2 = X_3 = 1$. Thus, \[\P{\ol{X} = 1} = \P{\ol{X} = -1} = \bp{\frac14}^3 = \frac1{64}.\] Similarly, the only way to get a total score of 2 is to have two 1's and one 0. Thus, \[\P{\ol{X} = \frac23} = \P{\ol{X} = -\frac23} = \binom31 \bp{\frac14}^2 \bp{\frac12} = \frac3{32}.\] Now note that there are two ways to achieve a total score of 1: have two 1's and one $-1$, or have two 0's and one 1. This gives \[\P{\ol{X} = \frac13} = \P{\ol{X} = -\frac13} = \binom31 \bp{\frac14}^2 \bp{\frac14} + \binom31 \bp{\frac12}^2 \bp{\frac14} = \frac{15}{64}.\] Lastly, by the complement principle, we have \[\P{\ol{X} = 0} = 1 - 2\bp{\frac1{64} + \frac3{32} + \frac{15}{64}} = \frac5{16}.\] Hence, the probability distribution of $X$ is given by
    \begin{table}[H]
        \centering
        \begin{tabular}{|c|c|c|c|c|c|c|c|}
        \hline
        &&&&&&&\\[-1em]
        $\ol{x}$ & $-1$ & $-\frac23$ & $-\frac13$ & 0 & $\frac13$ & $\frac23$ & $1$ \\[0.2em] \hline
        &&&&&&&\\[-1em]
        $\P{\ol{X} = \ol{x}}$ & $\frac1{64}$ & $\frac3{32}$ & $\frac{15}{64}$ & $\frac5{16}$ & $\frac{15}{64}$ & $\frac{3}{32}$ & $\frac1{64}$ \\[0.2em] \hline
        \end{tabular}
    \end{table}
    
    We now calculate $\Var{\ol{X}}$. Observe that the means of $X$ and $\ol{X}$ are 0 by symmetry. Hence, \[\Var{\ol{X}} = \E{\ol{X}^2} = 2\bs{1^2\bp{\frac1{64}} + \bp{\frac23}^2 \bp{\frac3{32}} + \bp{\frac13}^2\bp{\frac{15}{64}}} = \frac16.\] Now, note that \[\Var{X} = \E{X^2} = 2\bs{1^2 \bp{\frac14}} = \frac12.\] Thus, \[\Var{\ol{X}} = \frac13 \Var{X}.\]
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item Prove that $\E{X/3} = p$.
        \item Given that $p = 1/3$, and denoting by $E$ the event that $X > 1$, find the probability that in 100 throws of the three coins, $E$ will not occur more than 30 times.
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
...
\begin{problem}
    ...
    The random variable $X$ is the larger of the two scores if they are different, and their common value if they are the same. Show that $\P{X = 2} = 9/32$.

    Show that $\E{X} = 75/32$ and find $\Var{X}$.
\end{problem}
\begin{solution}
    \begin{ppart}
    ...
    Observe that the event $X = 1$ is equivalent to both scores being 1, whence we have $\P{X = 1} = 3/16$ from part (a). From part (b), we also have $\P{X = 3} = 17/32$. Thus, \[\P{X = 2} = 1 - \P{X = 1} - \P{X = 3} = 1 - \frac3{16} - \frac{17}{32} = \frac{9}{32}.\]
...
\begin{problem}
    ...
    \begin{enumerate}
        \item Show that the expectation of Alfred's cash after the first game is \$$(304 - 2x)/3$.
...
\begin{solution}
    \begin{ppart}
        Note that \[\P{\text{score}<5} = \frac{3 + 2 + 1}{6^2} = \frac16 \quad \tand \quad \P{\text{score} \geq 5} = 1 - \frac16 = \frac56.\] Let \$$a_n$ be the expectation of Alfred's cash after $n$ games. Suppose Alfred and Bertie play one more game (i.e. $n + 1$ total games). Then \[a_{n+1} = \frac56 (a_n - x) + \frac16(a_n + x + 8) = a_n + \frac23 \bp{2-x}.\] $a_n$ is in AP with common difference $(2/3)\bp{2-x}$ and is thus given by \[a_n = a_0 + n\bs{\frac23 \bp{2-x}} = 100 + \frac{2n}{3}(2-x).\] Hence, the expectation of Alfred's cash after the first game is \[a_1 = 100 + \frac{2(1)}3(2 - x) = \frac13(304 - 2x).\]
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Let the random variable $X$ be Alfred's cash after one game. Since the payouts are unaffected by $a_0$, we take $a_0 = 0$. When $x = 3$, $\E(X) = -2/3$. Hence, \[\Var{X} = \frac56 \bp{3 - \frac23}^2 + \frac16 \bp{3 + 8 + \frac23}^2 = \frac{245}{9}.\]
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        We have \[\E{X} = 2p + 3\bp{\frac2{10}} + 4\bp{\frac3{10}} + 5q = 4,\] so $2p + 5q = 22/10$. Additionally, we know that the probabilities must sum to 1: \[p + \frac2{10} + \frac3{10} + q = 1,\] so $p + q = 1/2$. We hence get a system of two linear equations. Solving, we have $p = 1/10$ and $q = 2/5$.
    \end{ppart}
    \begin{ppart}
