...
\end{problem}
\begin{solution}
    Let $z = x + \i y$, where $x$ and $y$ are real. Then \[\arg{z-1} = \arctan \frac{y}{x-1} = \frac\pi3,\] so \[\frac{y}{x-1} = \tan \frac\pi3 = \sqrt{3},\] whence $y = \sqrt{3} x - \sqrt3$. Also, \[\arg{z-\i} = \arctan \frac{y-1}{x} = \frac{\pi}6,\] so \[\frac{y-1}{x} = \tan \frac\pi6 = \frac1{\sqrt3},\] whence $y = 1 + x/\sqrt3$.

    Equating the two, we have \[\sqrt{3} x - \sqrt3 = 1 + \frac1{\sqrt3} x.\] Solving, we get \[x = \frac{1 + \sqrt{3}}{\sqrt3 - 1/\sqrt3} = \frac{\sqrt3 + 3}{2}.\] Thus, \[y = \sqrt{3} \bp{x - 1} = \sqrt{3} \bp{\frac{\sqrt3 + 3}{2} - 1} = \frac{3 + \sqrt3}{2},\] so $x = y$ and \[\arg z = \arctan \frac{y}{x} = \arctan 1 = \frac\pi4.\]
\end{solution}
...
\begin{solution}
    ...
    For $b$ to be at a minimum, $PQ$ must be the diameter of the circle. By the Pythagorean Theorem, \[3^2 = \bp{\frac{PQ}{2}}^2 + 1^2,\] whence $PQ = 32$. Thus, \[\min b = \frac{PQ}{2} = \frac{\sqrt{32}}{2} = 2\sqrt{2}.\] The point representing $a$ is then the midpoint of $P$ and $Q$, i.e. $a = 1$.
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item Given that $\abs{z} \leq 2$ and $\arctan{1/2} \leq \arg z \leq \arctan 2$, calculate the area of the region in which the point $P$ can lie.
    \end{enumerate}
\end{problem}
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Given $y = 2x$, we have $z = x + 2\i x = x\bp{1 + 2\i}$, so $\arg{z^2} = 2\arg{1 + 2\i}$. Meanwhile, \[\arg{z + 2\i z} = \arg{z} + \arg{1 + 2\i} = \arg{1 + 2\i} + \arg{1 + 2\i} = 2\arg{1 + 2\i}.\] Since $z^2$ and $z + 2\i z$ have identical arguments, the points representing them must be collinear with the origin.
    \end{ppart}
    \clearpage
    \begin{ppart}
...
\begin{solution}
    \begin{ppart}
        We have \[\abs{z - a} = \abs{\frac1w - a} = \abs{\frac{1 - aw}{w}} = a.\] Multiplying through by $\abs{w}$, we get $\abs{1 - aw} = \abs{aw}$, so \[\abs{w} = \abs{\frac{1 - aw}{a}} = \abs{\frac1a - w}.\] Hence, the locus of $P$ is the perpendicular bisector of the origin and $(1/a, 0)$. Equivalently, it is the vertical line passing through $(1/2a, 0)$.
    \end{ppart}
    \clearpage
    \begin{ppart}
    ...
    \begin{ppart}
        ...
        From the diagram, \[\sin \t = \frac{1/2}{2 - 1/2},\] so $\t = \arcsin{1/3}$. Thus, \[160.5\deg = \pi - \t \leq \arg{z-2} \leq \pi + \t = 199.5\deg.\] As $a \to 0$, $\arg{z-1/a} \to \pi$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        \begin{psubpart}
            Note that $\arg z^2 = 2\arg z = 2\a$. Let $z^2 = x + \i y$, where $x$ and $y$ are real. Then $\arg{z^2} = \arctan{y/x}$. Equating the two yields \[\arctan \frac{y}{x} = 2\a,\] which gives $y = x \tan{2\a}$. Note that $\arg{z^2} = 2\a \in (\pi/2, \pi)$, so $x = \Re{z^2} < 0$.
