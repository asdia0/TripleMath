...
\end{problem}
\begin{solution}
    From $\arg{7/(z+2)} = -\pi/4$, we have $\arg{z - (-2)} = \pi/4$.
...
\begin{problem}
    The point $A$ on an Argand diagram represents the fixed complex number $a$, where $0 < \arg a < \pi/2$. The complex numbers $z$ and $w$ are such that $\abs{z - 2\i a} = \abs{a}$ and $\abs{w} = \abs{w + \i a}$.
    ...
    \begin{enumerate}
        ...
        \item the range of values of $\arg{1/z}$ in terms of $\arg a$.
    \end{enumerate}
\end{problem}
\begin{solution}
    We are given $\abs{w - 0} = \abs{w - (-ia)}$.
    ...
    \begin{ppart}
        Let $B(-\i a)$ and $C(2\i a)$. Note that $W\bp{-\i a/2}$ lies on the locus of $w$ as well as the line passing through $OC$. Since $CW$ is perpendicular to the locus of $w$, it follows that the minimum value of $\abs{z - w}$ is given by \[CW - \abs{a} = \abs{2\i a + \frac12 \i a} - \abs{a} = \frac52 \abs{a} \abs{\i} - \abs{a} = \frac32 \abs{a}.\]
    \end{ppart}
    \begin{ppart}
        Let $D$ and $E$ be such that $OD$ and $OE$ are tangent to the circle given by the locus of $z$. Let $\angle COD = \t$. Observe that $\sin \t = CD/CO = \abs{a}/\abs{2\i a} = 1/2$, whence $\t = \pi/6$. Since $\angle COA = \arg \i = \pi/2$, it follows that $\angle DOA = \pi/2 - \t = \pi/2 - \arcsin{1/2} = \pi/3$. Thus, $\min \arg z = \arg a + \angle DOA = \arg a + \pi/3$. Meanwhile, $\angle COE = \angle COD = \t$, whence $\max \arg z = \arg a + \pi/2 + \t = \arg a + 2\pi/3$. Since $\arg{1/z} = -\arg z$, we thus have $\arg{1/z} \in \bs{-\bp{\arg a + 2\pi/3}, -\bp{\arg a + \pi/3}}$.
    \end{ppart}
\end{solution}
...
\begin{problem}
    \begin{enumerate}
        ...
        \item The roots represented by $z_1$ and $z_2$ are such that $0 < \arg z_1 < \arg z_2 < \pi/2$. Explain why the locus of all points $z$ such that $\abs{z - z_1} = \abs{z - z_2}$ passes through the origin. Draw this locus on your Argand diagram and find its Cartesian equation.
...
\begin{solution}
    \begin{ppart}
        For integers $k$, we have $z^7 = 1 + \i = 2^{1/2} \e^{\i\pi (1/4 + 2k)}$. Taking roots, we get $z = 2^{1/14} \e^{\i\pi (1 + 8k)/28}$. Taking $k \in \bc{-3, -2, \ldots, 2, 3}$, we obtain the seven roots $z = 2^{1/14} \e^{-\i s \pi/28}$, where $s \in \bc{-23, -15, -7, 1, 9, 17, 25}$.
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Since $\abs{z_1} = \abs{z_2} = 2^{1/14}$, the distance between $z_1$ and the origin and the distance between $z_2$ and the origin are equal. Since the locus of $\abs{z - z_1} = \abs{z - z_2}$ represents all points equidistant from $z_1$ and $z_2$, it passes through the origin.

        Observe that the midpoint of $z_1$ and $z_2$ will have argument \[\frac{\pi/28 + 9\pi/28}{2} = \frac{5\pi}{28}.\] Thus, the Cartesian equation of the locus of $z$ is given by $y = \tan{5\pi/28} x$.
    \end{ppart}
    \begin{ppart}
