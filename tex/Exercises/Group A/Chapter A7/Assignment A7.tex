...
\end{problem}
\begin{solution}
    Observe that \[\vec p = \l \vec a + (1 - \l)\vec b = \l \cveciii110 + (1 - \l)\cveciii4{-2}6 = \cveciii{4 - 3 \l}{-2 + 3\l}{6 - 6\l}.\] Thus, \[\abs{\vec p}^2 = (4 - 3\l)^2 + (-2 + 3\l)^2 + (6 - 6\l)^2 = 54\l^2 - 108\l + 56.\] Since the angle between $\vec p$ and $\vec k$ is $45\deg$, \[\frac1{\sqrt2} = \cos 45\deg = \frac{\vec p \dotp \vec k}{\abs{\vec p} \abs{\vec k}} = \frac{6 - 6\l}{\abs{\vec p}},\] which simplifies to give \[\frac{\abs{\vec p}^2}{2} = (6 - 6\l)^2.\] We thus obtain the quadratic equation \[\frac{54\l^2 - 108\l + 56}2 = 36\l^2 - 72\l + 36,\] which is equivalent to $9\l^2 - 18\l + 8 = 0$. We thus find that $\l = 2/3$ and $\l = 4/3$. However, we must reject $\l = 4/3$ since $6 - 6\l = \abs{\vec p}/\sqrt{2} > 0$ implies that $\l < 1$. Hence, $\l = 2/3$.
\end{solution}
...
\begin{solution}
    \begin{ppart}
        $\vec a$ and $\vec b$ either have the same or opposite direction. Let $\vec b = \l \vec a$ for some $\l \in \RR$. Then \[\vec a = (\vec a \dotp \vec b) \vec b = (\vec a \dotp \l \vec a) \l \vec a = \l^2 \abs{\vec a}^2 \vec a.\] It follows that $\l^2\abs{\vec a}^2 = 1$, so $\abs{\vec b} = \abs{\l\abs{\vec a}} = 1$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \begin{ppart}
        We have
        \begin{align*}
            \oa{OP} &= \oa{OA} + \oa{AP} = \oa{OA} + 2\oa{AB} = \cveciii121,\\
            \oa{OQ} &= \oa{OA} + \frac12 \oa{AC} = \cveciii222.
        \end{align*}
        Thus, $\oa{PQ} = \cveciiix101$ and $\oa{AQ} = \cveciiix10{-1}$. Since $\oa{PQ} \dotp \oa{AQ} = 0$, the two vectors are perpendicular, whence $PQ \perp AQ$.
    \end{ppart}
    \begin{ppart}
