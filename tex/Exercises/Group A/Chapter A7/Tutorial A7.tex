...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item Find the value of $d$ if $B$, $C$ and $D$ are collinear. State the ratio $BC/BD$.
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        If $B$, $C$ and $D$ are collinear, then $\oa{BC} = \l \oa{CD}$ for some $\l \in \RR$. This gives \[\cvecii11 = \l\bp{\oa{OD} - \oa{OC}} = \l\bs{\cvecii3d - \cvecii24} = \cvecii{\l}{\l(d-4)}.\] Hence, $\l = 1$ and $\l(d - 4) = 1$, so $d = 5$. Also, $\oa{BC} = \oa{CD}$. Thus, \[\frac{BC}{BD} = \frac{BC}{BC + CD} = \frac{BC}{BC + BC} = \frac12.\]
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    \begin{center}\tikzsetnextfilename{144}
        \begin{tikzpicture}
            \draw (0, 0) -- (-3, 0);
            \draw (0, 0) -- (-1.5, 2);
            \draw (-1.5, 2) -- (-3, 0);
...
\end{problem}
\begin{solution}
    Note that $\oa{OB} = 10 \vec i$ and $\oa{BC} =  6 \vec j$. Thus, $\oa{OC} = \oa{OB} + \oa{BC} = 10 \vec i + 6 \vec j$. Also, note that $\triangle ODE \cong \triangle BCF$. Hence, $\oa{OD} = \oa{BC} = 6 \vec j$. Note that $\triangle ODE$ is isosceles. Let $G$ be the mid-point of $OD$. Since $\tan \t = 4/3$, we have $EG/DG = 4/3$, so \[EG = \frac43 DG = \frac23 OD = \frac23 \cdot 6 = 4,\] thus $\oa{GE} = 4\vec k$. Hence, \[\oa{OE} = \oa{OG} + \oa{GE} = \frac12 \oa{OD} + \oa{GE} = 3\vec j + 4 \vec k.\] It follows that \[\oa{OF} = \oa{OB} + \oa{BF} = \oa{OB} + \oa{OE} = 10 \vec i + 3 \vec j + 4 \vec k.\] To sum, \[\oa{OB} = 10 \vec i, \quad \oa{OC} = 10 \vec i + 6 \vec j, \quad \oa{OD} = 6 \vec j, \quad \oa{OE} = 3 \vec j + 4 \vec k, \quad \oa{OF} = 10 \vec i + 3 \vec j + 4 \vec k.\]
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        \item $\vec u = \vec i - \vec j + \vec k$ and $\vec v = 3\vec i + 2 \vec j + 7 \vec k$,
        \item $\vec u = 2\vec i - 3\vec k$ and $\vec v = -\vec i + 7 \vec j + 2\vec k$.
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
        We have $\vec u = \cveciiix1{-1}1$ and $\vec v = \cveciiix327$. Hence,
        \begin{align*}
            \vec u \dotp \vec v &= (1)(3) + (-1)(2) + (1)(7) = 8,\\
            \vec u \crossp \vec v &= \cveciii{(-1)(7) - (2)(1)}{(1)(3) - (7)(1)}{(1)(2) - (3)(-1)}= \cveciii{-9}{-4}{5}.    
        \end{align*}
        Let the angle between $\vec u$ and $\vec v$ be $\t$. Then \[\cos \t = \frac{\vec u \dotp \vec v}{\abs{\vec u} \abs{\vec v}} = \frac{8}{\sqrt{1^2 + (-1)^2 + 1^2} \sqrt{3^2 + 2^2 + 7^2}},\] so $\t = 54.1\deg \todp{1}$.
    \end{ppart}
    \begin{ppart}
        We have $\vec u = \cveciiix20{-3}$ and $\vec v = \cveciiix{-1}72$. Hence,
        \begin{align*}
            \vec u \dotp \vec v &= (2)(-1) + (0)(7) + (-3)(2) = -8,\\
            \vec u \dotp \vec v &= \cveciii{(0)(2) - (7)(-3)}{(-3)(-1) - (2)(2)}{(2)(7) - (-1)(0)} = \cveciii{21}{-1}{14}.
        \end{align*}
        Let the angle between $\vec u$ and $\vec v$ be $\t$. Then \[\cos \t = \frac{\vec u \dotp \vec v}{\abs{\vec u}\abs{\vec v}} = \frac{-8}{\sqrt{2^2 + 0^2 + (-3)^2}\sqrt{(-1)^2 + 7^2 + 2^2}},\] so $\t = 107.6\deg \todp{1}$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Note that $\vec a \crossp \vec b = \cveciiix{11}{-4}{-5}$. Hence, \[\wh{\vec a \crossp \vec b} = \frac1{\sqrt{162}} \cveciii{11}{-4}{-5}.\]
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    \begin{ppart}
        \[\oa{AB} = \vec b - \vec a = \cveciii5{-1}2 - \cveciii23{-4} = \cveciii3{-4}6.\] Note that $\abs{\oa{AB}} = \sqrt{61}$. Hence, the required vector is $(1/\sqrt{61})\cveciiix3{-4}6$.
    \end{ppart}
    \begin{ppart}
        Since $ABCD$ is a parallelogram, we have that $\oa{AD} = \oa{BC}$. Thus, $\oa{OD} - \vec a = \vec c - \vec b$, so \[\oa{OD} = \vec a - \vec b + \vec c = \cveciii23{-4} - \cveciii5{-1}2 + \cveciii{11}{\l}{14} = \cveciii8{\l + 4}8.\]
    \end{ppart}
    \begin{ppart}
        Given that $A$, $B$ and $C$ are collinear, we have $\oa{AB} = k\oa{BC}$ for some $k \in \RR$. Hence, \[\cveciii3{-4}6 = k \bp{\vec c - \vec b} = k \bs{\cveciii{11}{\l}{14} - \cveciii5{-1}2} = k \cveciii6{\l + 1}{12}.\] We see that $k = 1/2$, so $\l = -9$.
    \end{ppart}
    \begin{ppart}
...
\end{problem}
\begin{solution}
    Let $ABCD$ be a square with side length $2k$ with $A$ at the origin. Then \[\vec p = \oa{AM} = \cvecii{2k}{-k} \quad \tand \quad \vec q = \oa{AN} = \cvecii{k}{-2k}.\] Hence, \[\oa{AC} = \cvecii{2k}{-2k} = \frac23 \cvecii{3k}{-3k} = \frac23 \bp{\vec p + \vec q}.\]
\end{solution}

\begin{problem}
    The points $A$, $B$ have position vectors $\vec a$, $\vec b$ respectively, referred to an origin $O$, where $\vec a$ and $\vec b$ are not parallel to each other. The point $C$ lies on $AB$ between $A$ and $B$ and is such that $AC/CB = 2$, and $D$ is the mid-point of $OC$. The line $AD$ produced meets $OB$ at $E$.

    Find, in terms of $\vec a$ and $\vec b$,

    \begin{enumerate}
        \item the position vector of $C$ (referred to $O$),
        \item the vector $\oa{AD}$. Find the values of $OE/EB$ and $AE/ED$.
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Since $D$ is the midpoint of $OC$, we have $\oa{OD} = (1/6) \vec a + (1/3) \vec b$. Hence, \[\oa{AD} = \oa{OD} - \oa{OA} = \bp{\frac16 \vec a + \frac13 \vec b} - \vec a = -\frac56 \vec a + \frac13 \vec b.\]
        
        Using Menelaus' theorem on $\triangle BCO$, \[\frac{BA}{AC} \frac{CD}{DO} \frac{OE}{EB} = 1,\] so $OE/EB = 2/3$. Using Menelaus' theorem on $\triangle BEA$, \[\frac{BO}{OE} \frac{ED}{DA} \frac{AC}{CB} = 1,\] so $ED/AD = 1/5$, hence \[\frac{AE}{ED} = \frac{AD + DE}{ED} = 6.\]
    \end{ppart}
\end{solution}

\begin{problem}
    \begin{enumerate}
        \item The angle between the vectors $(3\vec i - 2\vec j)$ and $(6\vec i + d\vec j - \sqrt7 \vec k)$ is $\arccos{6/13}$. Show that $2d^2 - 117d + 333 = 0$.
        ...
        \begin{center}\tikzsetnextfilename{145}
...
\begin{solution}
    \begin{ppart}
        Let $\vec a = \cveciiix3{-2}0$ and $\vec b = \cveciiix6d{-\sqrt7}$. Note that $\vec a \dotp \vec b = 18 - 2d$. Let $\t$ be the angle between $\vec a$ and $\vec b$. Then \[\frac6{13} = \cos \t = \frac{\vec a \dotp \vec b}{\abs{\vec a}\abs{\vec b}} = \frac{18-2d}{\sqrt{43 + d^2}\sqrt{13}}.\] Squaring and simplifying, we get \[\frac{9}{13} = \frac{(9-d)^2}{43 + d^2},\] which gives $2d^2 - 117d + 333 = 0$ upon clearing denominators and rearranging.
    \end{ppart}
    \begin{ppart}
        \begin{psubpart}
            By Menelaus' theorem, \[\frac{OC}{CA} \frac{AE}{ED} \frac{DB}{BO} = 1,\] so $AE/ED = 5/18$. Thus, $\oa{AE} = (5/23) \oa{AD}$, whence \[\oa{OE} = \oa{OA} + \frac5{23} \oa{AD}.\] Since $\oa{AD} = \oa{OD} - \oa{OA} = 4\vec b - \vec a$, we have \[\oa{OE} = \vec a + \frac5{23} \bp{4\vec b - \vec a} = \frac{18}{23} \vec a + \frac{20}{23} \vec b.\]
        \end{psubpart}
        \begin{psubpart}
...
\begin{problem}
    ...
    \begin{enumerate}
        \item Find the area of the triangle $ABC$. Hence, find the sine of $\angle BAC$.
...
\begin{solution}
    \begin{ppart}
        ...
        We have \[\sin \angle BAC = \frac{\abs{\oa{AB} \crossp \oa{AC}}}{\abs{\oa{AB}}\abs{\oa{AC}}} = \frac{6}{3\sqrt5} = \frac{2\sqrt5}{5}.\]
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        The perpendicular distance between $C$ and $AB$ is \[\abs{\oa{AC} \crossp \frac{\oa{AB}}{\abs{\oa{AB}}}} = \frac13 \abs{\oa{AB} \crossp \oa{AC}} = 2 \units.\]
    \end{ppart}
\end{solution}

\begin{problem}
    \begin{center}\tikzsetnextfilename{146}
        \begin{tikzpicture}
            \draw (0, 0) -- (4, -2);
            \draw (0, 0) -- (0, 2);
            \draw[dotted] (0, 0) -- (4, 1);
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Note that $\oa{OF} = \cveciiix200$ and $\oa{BF} = \cveciiix20{-1}$. Let $\t$ be the angle the rectangular face $BCEF$ makes with the horizontal base. Then \[\cos \t = \frac{\oa{OF} \dotp \oa{BF}}{\abs{\oa{OF}}\abs{\oa{BF}}} = \frac{4}{2\sqrt5},\] so $\t = 26.6 \deg \todp{1}$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \begin{ppart}
        Note that $\oa{AB} = \cveciiix1{-5}8$. For $OP$ to be perpendicular to $AB$, we must have $\oa{OP} \dotp \oa{AB} = 0$. Hence, \[\cveciii{1+\l}{2-5\l}{-2+8\l} \dotp \cveciii1{-5}8 = -25 + 90\l = 0,\] so $\l = 5/18$.
    \end{ppart}
    \begin{ppart}
        Suppose $\angle AOP = \angle POB$. Then $\cos \angle AOP = \cos \angle POB$. Thus, \[\frac{\oa{OP} \dotp \oa{OA}}{\abs{\oa{OP}} \abs{\oa{OA}}} = \frac{\oa{OP} \cdot \oa{OB}}{\abs{\oa{OP}} \abs{\oa{OB}}}.\] It follows that \[\oa{OP} \dotp \bp{\frac13 \oa{OA} - \frac17 \oa{OB}} = 0.\] Clearing denominators, we get \[\oa{OP} \dotp \bp{7\oa{OA} - 3\oa{OB}} = 0.\] This gives \[\cveciii{1+\l}{2-5\l}{-2+8\l} \dotp \bs{7 \cveciii12{-2} -  3 \cveciii2{-3}6} = \cveciii{1+\l}{2-5\l}{-2+8\l} \dotp \cveciii1{23}{-32} = 0.\] Taking the dot product and simplifying, we see that $111 - 370\l = 0$, whence $\l = 3/10$.
    \end{ppart}
\end{solution}

\begin{problem}
    \begin{center}\tikzsetnextfilename{147}
        \begin{tikzpicture}
            \node[anchor=east] at (0, 0) {$O$};
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        The area of $\triangle ONC$ is given by \[[\triangle ONC] = \frac12 \abs{\oa{ON} \crossp \oa{OC}} = \frac12 \abs{\bs{\frac47\vec a + \frac37(\l \vec a + \m \vec b)} \crossp \bp{\l \vec a + \m \vec b}} = \frac{2\m}{7} \abs{\vec a \crossp \vec b}.\] Meanwhile, the area of $\triangle OMC$ is given by \[[\triangle OMC] = \frac12 \abs{\oa{OM} \crossp \oa{OC}} = \frac12 \abs{\frac12 \vec b \crossp \bp{\l \vec a + \m \vec b}} = \frac{\l}{4} \abs{\vec a \crossp \vec b}.\] Since the two areas are equal, we see that \[\frac{2\m}{7} \abs{\vec a \crossp \vec b} = \frac{\l}{4} \abs{\vec a \crossp \vec b},\] so $\l = 8\m/7$.
    \end{ppart}
\end{solution}