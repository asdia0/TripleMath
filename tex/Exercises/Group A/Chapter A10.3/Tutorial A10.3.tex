...
\begin{problem}
    \begin{enumerate}
        \item Write down the exact values of the modulus and the argument of the complex number $1/2 + \i \sqrt3/2$.
...
\begin{solution}
    \begin{ppart}
        We have $r = \sqrt{\bp{1/2}^2 + \bp{\sqrt{3}/2}^2} = 1$ and $\tan \t = (\sqrt3/2)/(1/2)$, so $\t = \pi/3$. Hence, $\abs{1/2 + \i \sqrt3/2} = 1$ and $\arg{1/2 + \i \sqrt3/2} = \pi/3$.
    \end{ppart}
    \begin{ppart}
        From the quadratic formula, we have \[z = \frac{w \pm \sqrt{w^2 - 4w^2}}{2} = w \bp{\frac12 \pm \frac{\sqrt3}2 \i}.\]
            ...
            Since $\abs{1/2 \pm \i \sqrt{3}/2} = 1$, we have that $OB = OA_1 = OA_2$. Further, since $\arg{1/2 \pm \i \sqrt{3}/2} = \pm \pi/3$, we know $\angle A_1OB = \angle A_2OB = \pi/3$, whence $\triangle A_1OB$ and $\triangle A_2OB$ are both equilateral.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        We have $z^3 = 1 = \e^{\i 2\pi n}$, where $n$ is an integer. Taking cube roots, we see that \[z = \e^{\i 2 \pi n / 3} = \cos \frac{2\pi n}3 + \i \sin \frac{2\pi n}3.\] Take $n = 0, 1, 2$. Then the roots are \[z = 1, \quad -\frac12 + \frac{\sqrt3}2 \i, \quad -\frac12 - \frac{\sqrt3}2 \i.\]
    \end{ppart}
    \begin{ppart}
        We have $(z-1)^4 = -16 = 16\e^{\i \pi (2n + 1)}$, where $n$ is an integer. Taking fourth roots, we see that \[z = 1 + 2\e^{\i\pi (2n + 1)/4} = 1 + 2\bs{\cos{\frac{2n+1}4 \pi} + \i\sin{\frac{2n+1}4 \pi}}.\] Take $n = 0, 1, 2, 3$. Then the roots are $z = (1 + \sqrt2) + \i\sqrt2$, $(1 - \sqrt2) + \i\sqrt2$, $(1 - \sqrt2) - \i\sqrt2$, $(1 + \sqrt2) - \i\sqrt2$.
    \end{ppart}
\end{solution}
...
\begin{problem}
    \begin{enumerate}
        ...
        \item Show that the roots of the equation $(5+z)^5 - (5-z)^5 = 0$ can be written in the form $5\i \tan{k\pi/5}$, where $k = 0$, $\pm 1$, $\pm 2$.
    \end{enumerate}
\end{problem}
\clearpage
\begin{solution}
    \begin{ppart}
        We have $z^5 = 1 = \e^{\i 2\pi n}$, where $n$ is an integer. Taking fifth roots, we have $z = \e^{\i 2\pi n /5}$. Since $-\pi < \t \leq \pi$, we have $z = \e^{-\i 4\pi/5}$, $\e^{-\i 2\pi/5}$, $1$, $\e^{\i 2\pi/5}$, $\e^{\i 4\pi/5}$.
    \end{ppart}
    \begin{ppart}
        From $(5+z)^5 - (5-z)^5 = 0$, we have \[\bp{\frac{5+z}{5-z}}^5 = 1.\] From part (a), we have that \[\frac{5+z}{5-z} = \e^{i 2k \pi/5},\] where $k = 0$, $\pm 1$, $\pm 2$. Solving for $z$, we get \[z = 5\bp{\frac{\e^{\i2k\pi/5} - 1}{\e^{\i2k\pi/5} + 1}} = 5\bp{\frac{\e^{\i k\pi/5} - \e^{-\i k\pi/5}}{\e^{\i k\pi/5} + \e^{-\i k\pi/5}}} = 5\bp{\frac{2\i \sin{k\pi/5}}{2\cos{k\pi/5}}} = 5\i\tan \frac{k\pi}5.\]
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \begin{align*}
        \cos 7\t &= c^7 - 21c^5s^2 + 35c^3s^4 - 7cs^6\\
        &= c^7 - 21c^5\bp{1-c^2} + 35c^3\bp{1 - c^2}^2 - 7c\bp{1 - c^2}^3\\
        &= 64c^7 - 112c^5 + 56c^3 - 7c\\
        &= 64\cos^7\t - 112\cos^5 + 56 \cos^3\t - 7\cos\t.
    \end{align*}

    Observe that we can manipulate the given equation into \[64x^7 - 112x^5 + 56x^3 - 7x = -\frac12.\] Under the substitution $x = \cos \t$, we see that $\cos 7\t = -1/2$, so \[7\t = \frac23 \pi + 2\pi n,\] where $n$ is an integer. Solving for $\t$, we have \[\t = \frac{2\pi}{21} (3n + 1).\] Taking $0 \leq n < 7$, the roots are
    \begin{align*}
        x &= \cos \frac{2\pi}{21}, \quad \cos \frac{8\pi}{21}, \quad \cos \frac{14\pi}{21}, \quad \cos \frac{20\pi}{21}, \quad \cos \frac{26\pi}{21}, \quad \cos \frac{32\pi}{21}, \quad \cos \frac{38\pi}{21}\\
        &= \cos \frac{2\pi}{21}, \quad \cos \frac{4\pi}{21}, \quad \cos \frac{8\pi}{21}, \quad \cos \frac{10\pi}{21}, \quad \cos \frac{14\pi}{21}, \quad \cos \frac{16\pi}{21}, \quad \cos \frac{20\pi}{21}.
    \end{align*}
\end{solution}
...
\begin{solution}
    ...
    \begin{align*}
        \sum_{n=1}^N z^{2n-1} &= \frac1z \sum_{n=1}^N \bp{z^2}^n\\
        &= \frac1z \bp{\frac{z^2 \bs{\bp{z^2}^N - 1}}{z^2 - 1}}\\
        &= \frac{z^{2N} - 1}{z - z^{-1}}\\
        &= z^N \bp{\frac{z^N - z^{-N}}{z - z^{-1}}}\\
        &= z^N \bp{\frac{2\i \sin N\t}{2\i \sin \t}}\\
        &= z^N \bp{\frac{\sin N\t}{\sin \t}},
    \end{align*}
...
\begin{solution}
    ...
    Recall that $\sin^2 n\t = (1 - 2\cos 2n\t)/2$. Thus, \[\sum_{n = 0}^N \sin^2 n\t = \frac12 \sum_{n = 0}^N (1 - \cos 2n\t) = \frac{N + 1}2 - \frac{\sin(N+1)\t \cos N\t}{2\sin\t}.\]
\end{solution}
...
\begin{problem}
    ...
    Find, correct to three decimal places, the values of $\t$ such that $0 < \t < \pi/2$ and $\cos 8\t + 8\cos6\t + 28\cos4\t + 56\cos2\t + 1 = 0$.
\end{problem}
\begin{solution}
    Note that
    \begin{align*}
        z^k + \frac1{z^k} &= z^k + z^{-k}\\
        &= \bp{\e^{\i\t}}^k + \bp{\e^{\i \t}}^{-k}\\
        &= \e^{\i k \t} + \e^{-\i k \t}\\
        &= \bs{\cos{k\t} + \i\sin{k\t}} + \bs{\cos{-k\t} + \i\sin{-k\t}}\\
        &= 2\cos{k\t}.
    \end{align*}

    Thus,
    \begin{align*}
        \cos^8 \t &= \frac1{256} (2\cos \t)^8\\
        &= \frac1{256} (z + z^{-1})^8 = \frac1{256} z^{-8} \bp{z^2 + 1}^8\\
    ...
    We may rewrite the given equation as \[\cos 8\t + 8\cos6\t + 28\cos4\t + 56\cos2\t + 35 = 128\cos^8\t = 34.\] Thus, \[\cos\t = \sqrt[8]{\frac{34}{128}},\] so $\t = 0.560 \tosf{3}$.
\end{solution}