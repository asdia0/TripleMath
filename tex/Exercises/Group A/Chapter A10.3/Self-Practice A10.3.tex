...
\begin{solution}
    ...
    Note that $\cot \t = \sqrt{3}$, so  $\t = \pi/6$, hence \[z^4 = \frac{\sqrt3 + \i}{\sqrt3 - \i} = \e^{2\i \pi/6} = \e^{\i \pi (1/3 + 2k)},\] where $k$ is an integer. Taking fourth roots, $z = \e^{\i \pi (1/12 + k/2)}$. Taking $k = 0, 1, 2, 3$, we see that the four roots are $z = \e^{\i \pi /12}$, $\e^{7\i \pi/12}$, $\e^{13\i \pi/12}$, and $\e^{19\i \pi/12}$.
\end{solution}
...
\end{problem}
\begin{solution}
    Consider $z^3 = 1 + \i \sqrt{3} = 2 \e^{\i \pi /3} = 2\e^{\i \pi (1/3 + 2k)}$, where $k$ is an integer. Taking roots, $z = 2^{1/3} \e^{\i \pi (1/9 + 2k/3)}$. Taking $k = -1, 0, 1$, the cube roots of $1 + \i \sqrt{3}$ are $z = 2^{1/3} \e^{-l\i \pi /9}$, where $l \in \bc{-8, 1, 7}$.
    
    Consider $z^6 - 2z^3 + 4 = 0$. Then $z^3 = 1 \pm \sqrt{3}$. From the positive branch, we get the aforementioned roots. Since the coefficients of the sextic are all real, by the conjugate root theorem, the six roots are $z = 2^{1/3} \e^{-l \i \pi / 9}$, where $l \in \bc{-8, -7, -1, 1, 7, 8}$.
\end{solution}
...
\begin{solution}
    ...
    We are given $z^4 = 8\bp{\sqrt3 - \i} = 16 \e^{-\i \pi/6} = 16\e^{\i \pi (-1/6 + 2k)}$, where $k$ is an integer. Taking roots, $z = 2 \e^{\i \pi (-1/24 + k/2)}$. Taking $k = -1, 0, 1, 2$, the roots are $z = 2\e^{-13\i\pi/24}$, $2\e^{-\i\pi/24}$, $2\e^{11\i\pi/24}$, and $2\e^{23\i\pi/24}$.
\end{solution}

\begin{problem}
    Write down, in any form, the five complex numbers which satisfy the equation $z^5 - 1 = 0$. Hence, show that the five complex numbers which satisfy the equation \[\bp{\frac{2w + 1}{w}}^5 = 1\] are \[\frac{-2 + \cos{2\pi k/5} - \i \sin{2\pi k/5}}{5 - 4 \cos{2\pi k/5}},\] where $k = 0, 1, 2, 3, 4$.
\end{problem}
\begin{solution}
    The fifth roots of unity are given by $z = \e^{2k \i \pi /5}$, where $k = 0, 1, 2, 3, 4$.

    We have that \[\frac{2w + 1}{w} = \e^{2k \i \pi /5}.\] Solving for $w$, we get \[w = \frac1{\e^{2k \i \pi /5} - 2} = \frac{\e^{-2k \i \pi /5} - 2}{\abs{\e^{2k \i \pi /5} - 2}^2}.\] Note that \[\abs{\e^{2k \i \pi /5} - 2}^2 = \bp{\e^{2k \i \pi /5} - 2}\bp{\e^{-2k \i \pi /5} - 2} = 1 - 2\bp{2\cos \frac{2k\pi}{5}} + 4 = 5 - 4\cos \frac{2k\pi}5.\] Thus, \[w = \frac{-2 + \cos{2\pi k/5} - \i \sin{2\pi k/5}}{5 - 4 \cos{2\pi k/5}}\] for $k = 0, 1, 2, 3, 4$.
\end{solution}
...
\begin{solution}
    ...
    We have
    \begin{align*}
        \sum_{k=1}^7 (-1)^{k-1} \sin k\t &= \Im \sum_{k=1}^7 (-1)^{k-1} z^k\\
        &= \Im \frac{z + z^8}{1 + z}\\
        &= \Im \frac{z^{9/2} \bp{z^{7/2} + z^{-7/2}}}{z^{1/2} \bp{z^{1/2} + z^{-1/2}}}\\
        &= \frac{2\cos{7\t/2}}{2\cos{\t/2}} \Im z^4\\
        &= \frac{\cos{7\t/2} \sin 4\t}{\cos{\t/2}}.
    \end{align*}
    Note that $z = \e^{\i \t} \neq -1 = \e^{\i \pi (2k + 1))}$ for integers $k$, so $\t \neq (2k+1)\pi$, i.e. $\t$ cannot be an odd multiple of $\pi$.
\end{solution}