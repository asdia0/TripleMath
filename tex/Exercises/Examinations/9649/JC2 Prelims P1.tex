...
\begin{solution}
    \begin{ppart}
        Note that $z = \e^{\i \t}$, so
        \begin{align*}
            \sin 5\t &= \Im \e^{\i 5 \t} = \Im z^5 = \Im \bp{\cos \t + \i \sin \t}^5\\
    ...
    \begin{ppart}
        \begin{psubpart}
            Let $x = \e^{\i \t}$. By part (a), the given equation becomes $\sin 5\t = -1$. Within the range $0 \leq \t < 2\pi$, the only solutions are \[\t = \frac3{10}\pi, \quad \frac7{10}\pi, \quad \frac{11}{10}\pi, \quad \frac{15}{10}\pi, \quad \frac{19}{10}\pi.\] Since $\sin\frac3{10} \pi = \sin\frac7{10} \pi$ and $\sin\frac{11}{10} \pi = \sin\frac{19}{10} \pi$, the distinct real roots to the original equation are \[x = \sin\frac3{10}\pi, \quad \sin \frac{11}{10} \pi, \quad -1.\]
        \end{psubpart}
        \begin{psubpart}
            Let $s = \sin{\pi/5}$ and $c = \cos{\pi/5}$. From part (a), we see that \[16s^5 - 20s^3 + 5s = \sin \pi = 0.\] Since $s \neq 0$, we may divide both sides by $s$ to get a quadratic in $s^2$: \[16\bp{s^2}^2 - 20s^2 + 5 = 0.\] By the quadratic formula, this gives \[s^2 = \frac{5 - \sqrt{5}}{8}\] where we reject the positive branch since \[s^2 < \sin^2 \frac\pi4 = \frac12 < \frac{5 + \sqrt{5}}{8}.\] Then \[c^2 = 1 - s^2 = \frac{3 + \sqrt5}{8}.\] Hence, \[\tan^2 \frac\pi5 = \frac{s^2}{c^2} = \frac{\frac{5-\sqrt5}{8}}{\frac{3 + \sqrt5}{8}} = 5 - 2\sqrt5.\]
        \end{psubpart}
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Consider first the intersection of $C_1$ and $C_2$. Note that $2 + 2\sin \t = -2\sin \t$ is equivalent to $\sin \t = -1/2$, so $C_1$ and $C_2$ intersect when $\t = \frac76 \pi, \frac{11}6\pi$. From the graph in (a), we see that $C_1$ contributes to the perimeter of $R$ when $\frac32 \pi \leq \t \leq \frac{11}{6} \pi$, while $C_2$ contributes to the perimeter of $R$ when $\frac{11}{6} \pi \leq \t \leq 2\pi$.
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        \case{2}[$\l = 1/2$] The RREF of $\mat M - \frac12 \mat I$ is \[\begin{pmatrix}1 & 0 & 1 \\ 0 & 1 & 0 \\ 0 & 0 & 0\end{pmatrix}.\] Thus, the associated eigenvector is $\cveciiix10{-1}$.

        \case{3}[$\l = 1$] The RREF of $\mat M - \mat I$ is \[\begin{pmatrix}1 & 0 & - 1 \\ 0 & 1 & -2 \\ 0 & 0 & 0\end{pmatrix}.\] Thus, the associated eigenvector is $\cveciii121$.
    \end{ppart}
    \begin{ppart}
        ...
        \end{align*}
        Let the initial population be $P = p_0 + q_0 + r_0$. It follows that
        \begin{align*}
            \cveciii{p_\infty}{q_\infty}{r_\infty} &= \lim_{n \to \infty} \mat M^n \cveciii{p_0}{q_0}{r_0}\\
            &= \frac14 \begin{pmatrix}1 & 1 & 1 \\ 2 & 2 & 2 \\ 1 & 1 & 1\end{pmatrix} \cveciii{p_0}{q_0}{r_0}\\
            &= \cveciii{P/4}{P/2}{P/4}.
        \end{align*}
        Thus, in the long run, a quarter of all plants will have $AA$ genotype, half will have $Aa$ genotype, and the remaining quarter will have $aa$ genotype.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        The characteristic equation $m^2 + \frac7{100} m + \frac3{5000} = 0$ has roots $m = -\frac1{100}, -\frac3{50}$. Thus, the complementary solution is \[y_{(c)} = A\e^{-t/100} + B\e^{-3t/50}.\] For the particular solution, we guess $y_{(p)} = C$. Then \[\frac3{5000} C = \frac1{25}\] so $C = 200/3$. The general solution is thus \[y = y_{(c)} + y_{(p)} = A\e^{-t/100} + B\e^{-3t/50} + \frac{200}{3}.\]
    \end{ppart}
    \begin{ppart}
        Since $y$ converges as $t \to \infty$, we must have $\derx{y}{t} \to 0$. Thus, the desired ratio in the long run is \[\frac{x}{y} = \frac{50\bp{\der{y}{t} + \frac1{20} y}}{y} \to \frac52.\]
    \end{ppart}
\end{solution}