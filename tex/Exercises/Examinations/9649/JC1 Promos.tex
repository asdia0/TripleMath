\section{9649 JC1 Promotional Examination}
...
\end{problem}
\begin{solution}
    Let $x$ and $y$ be parametrized $t$. Implicitly differentiating $f(x, y) = k$ with respect to $t$, we obtain \[\pder{f}{x} \der{x}{t} + \pder{f}{y} \der{y}{t} = 0,\] which we can rewrite as \[\cvecii{\pderx{f}{x}}{\pderx{f}{y}} \dotp \cvecii{\derx{x}{t}}{\derx{y}{t}} = 0.\] Observe that $\cveciix{\pderx{f}{x}}{\pderx{f}{y}}$ is exactly $\nabla f$. Taking $t = x$, we also have $\cveciix{\derx{x}{t}}{\derx{y}{t}} = \cveciix{1}{\derx{y}{x}}$, which is the tangent vector of the level curve at $P$. Since the dot product of the two vectors is 0, they must be perpendicular.
\end{solution}

\begin{problem}
    A harvesting model is given by $\derx{P}{t} = (20-P)\bp{P^2 - 30 P + h}$ where $P$ is the population of the resource at time $t$ and $h$ is the constant harvest rate. It is given that at $t = 0$, $P = 50$. Find the range of values of $h$ such that $P=20$ in the long run.
\end{problem}
\begin{solution}
    Let $f(P) = (20-P)\bp{P^2 - 30 P + h}$. For $P = 20$ in the long run, we need $f(P) < 0$ for all $P \in (20, 50]$. Observe that $20 -P < 0$ for all $P \in (20, 50]$. We hence need $P^2 - 30 P + h > 0$ for all $P \in (20, 50]$. Since $P^2 - 30P + h$ is strictly increasing after $P > 15$, we only require the quadratic to be non-negative at $P = 20$, i.e. $20^2 - 30(20) + h \geq 0$, giving $h \geq 200$.
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Note that $\abs{2\i + 1 + \i z} = \abs{2 - \i + z} = \abs{z - (-2 + \i)}$. The given equation hence becomes $\abs{z - (-2 + \i)} = \abs{z - (-5 + 4\i)}$, whose locus is the perpendicular bisector of the line segment joining $(-2, 1)$ and $(-5, 4)$.
    \end{ppart}
    \clearpage
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
    Clearly, $\max \arg(z- 8\i) = -\pi/2$ (where $z = 6\i$). Let $A(-2, 3)$ and $B(0, 8)$. Let $C$ be the point on the circle such that $BC$ is tangent to the circle. We have $\angle ABO = \arctan{2/5}$. Since $AB = \sqrt{5^2 + 2^2} = \sqrt{29}$, we also have $\angle CBA = \arcsin{\sqrt{13}/\sqrt{29}}$. Thus, \[\min \arg{z - 8\i} = -\frac\pi2 - \arctan \frac{2}{5} - \arcsin \frac{\sqrt{13}}{\sqrt{29}} = -2.68,\] whence $-2.68 \leq \arg{z-8\i} \leq -\pi/2$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        The characteristic polynomial of the recurrence relation is $x^2 - 2x - 2 = 0$, which has roots $x = 1 \pm \sqrt3$. Hence, the general solution is $a_n = A(1 + \sqrt3)^n + B(1 - \sqrt3)^n$ for some constants $A$ and $B$.
        ...
        Solving, we get $A = 1/2 + 1/\sqrt3$ and $B = 1/2 - 1/\sqrt3$. Thus, \[\a_n = \bp{\frac12 + \frac1{\sqrt3}}\bp{1 + \sqrt3}^n + \bp{\frac12 - \frac1{\sqrt3}}\bp{1 - \sqrt3}^n.\]
    \end{ppart}
    \begin{ppart}
...
\begin{problem}
    The curve $C_1$ has equation $(x^2 + y^2)^{3/2} = 2x^2$.
...
\begin{solution}
    \begin{ppart}
        Recall that $x^2 + y^2 = r^2$ and $x = r\cos \t$. Substituting this into the given equation, we get $(r^2)^{3/2} = 2(r\cos \t)^2$, so $r = 2 \cos^2 \t = 1 + 2\cos2\t$.
    \end{ppart}
    \begin{ppart}
        Observe that $t = -\pi/2$ is tangent to the pole to both $C_1$ and $C_2$. Now consider the intersections of $C_1$ and $C_2$. Equating $r = 2\cos^2 \t = 2 - 2\sin^2 \t$ and $r = 1 + \sin \t$, we obtain $2\sin^2 \t + \sin \t -1 = (2\sin\t -1)(\sin \t + 1) = 0$, so $\sin\t = 1/2$ or $\sin \t = -1$, whence $\t = \pi/6$, $5\pi/6$, $-\pi/2$.
        ...
        \begin{align*}
            ...
            &= \evalint{\frac54 \sin 2\t + \frac18 \sin 4\t + 2\cos\t}{-\pi/2}{\pi/6}\\
            &= \frac{27}{16} \sqrt3 \units[2].
        \end{align*}
    \end{ppart}
    \begin{ppart}
        Observe that for $C_1$, $\derx{r}{\t} = -2\sin 2\t$, while for $C_2$, $\derx{r}{\t} = \cos \t$. Hence, the perimeter of $R$ is \[2 \int_{-\pi/2}^{\pi/6} \bp{\sqrt{(1 + \cos2\t)^2 + (-2\sin2\t)^2} + \sqrt{(1 + \sin\t)^2 + \cos^2 \t}} \d \t = 11.7 \units.\]
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item Water is poured into the bowl of the water feature at a rate of 2 units$^3$ per second. Given that the bowl is initially empty, find the rate of change of the depth of the water when the depth is at $\sqrt{3}/2$ units.
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Let the depth of the water be $h$. We have $V = \pi \int_0^h \arcsin{y} \d y$, whence $\derx{V}{h} = \pi \arcsin{h}$. Hence, \[\der{h}{t} = \frac{\derx{V}{t}}{\derx{V}{h}} = \frac2{\pi \arcsin{h}}.\] When $h = \sqrt3/2$, we have $\derx{h}{t} = 2/(\pi \cdot \pi/3)$. Thus, the rate of change of the depth of water is $6/\pi^2$ units/s.
    \end{ppart}
    \begin{ppart}
        Note that $\derx{y}{x} = 2x\cos{x^2}$. Hence, the arc length is given by \[\int_0^{1.25} \sqrt{1 + (2x\cos{x^2})^2} \d x.\] Let $f(x) = \sqrt{1 + (2x\cos{x^2})^2}$. By Simpson's rule, the arc length is approximately \[\frac{1.25 - 0}{3 \cdot 4} \bs{f(0) + f\bp{\frac5{16}} + 2f\bp{\frac{10}{16}} + 4f\bp{\frac{15}{16}} + f(1.25)} = 1.6671 \units \todp{4}.\]
    \end{ppart}
\end{solution}

\begin{problem}
    It is given that $y = f(x)$ satisfies the following differential equation: \[\bp{x^3 + 1} y \der{y}{x} + 3 x^2 y^2 = 2,\] where $y = 2$ when $x = 0$.
...
\begin{solution}
    \begin{ppart}
        Note that $z' = 2y \cdot y'$. Substituting this into the differential equation, we get \[\bp{x^3 + 1} \frac12 z' + 3x^2 z = 2,\] so \[z' + \frac{6x^2}{x^3 + 1} z = \frac{4}{x^3 + 1}.\] The integrating factor is hence \[\IF = \exp \int \frac{6x^2}{x^3 + 1} \d x = \exp{2\ln{x^3 + 1}} = \bp{x^3 + 1}^2.\] Multiplying the integrating factor through, we get \[\der{}{x} \bs{\bp{x^3 + 1}^2 z} = \bp{x^3 + 1}^2 z' +6x^2\bp{x^3 + 1} z = 4\bp{x^3 + 1}.\] Integrating both sides with respect to $x$, we obtain \[\bp{x^3 + 1}^2 z = \int 4\bp{x^3 + 1} \d x = x^4 + 4x + C,\] thus \[y^2 = \frac{x^4 + 4x + C}{\bp{x^3 + 1}^2}.\] Since $y = 2$ when $x = 0$, we get $C = 4$. Hence, \[y = \sqrt{\frac{x^4 + 4x + 4}{\bp{x^3 + 1}^2}}.\] Thus, when $x = 1$, we have $y = 3/2$.
    \end{ppart}
    \begin{ppart}
        When $x = 1$ and $y = 3/2$, we have \[\bp{1^3 + 1} \bp{\frac32} \der{y}{x} + 3 \bp{1^2} \bp{\frac32}^2 = 2,\] so $\derx{y}{x} = -19/12$. By the chain rule, it follows that \[\der{y}{t} = -\frac{19}{12} \cdot \der{x}{t} = -\frac{19}{12} \e^{\sqrt t}.\]

        Let \[f(y, t) = -\frac{19}{12} \e^{\sqrt t},\] $t_0 = 0$, $y_0 = 1.5$. Using Euler's method with $h = 0.2$, i.e. $y_{n+1} = y_n + 0.2 f(t_n, y_n)$, we get \[y_1 = 1.1833, \quad y_2 = 0.68808, \quad y_3 = 0.09204, \quad y_4 = -0.59503, \quad y_5 = -1.36958.\] Hence, when $t = 1$, $y \approx -1.37$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Within the range $0 \leq 5\t \leq 5\pi$, the roots to $\cos 5\t = 0$ are
        \[\t = \frac{\pi}{10}, \quad \frac{3\pi}{10}, \quad \frac{5\pi}{10}, \quad \frac{7\pi}{10}, \quad \frac{9\pi}{10}.\]
    \end{ppart}
    \begin{ppart}
        Multiplying through by $z$, we have $16z^5 - 20z^3 + 5z = 0$. Let $z = \cos \t$. Then $\cos 5\t = 0$, whence $\t = \pi/10$, $3\pi/10$, $5\pi/10$, $7\pi/10$, $9\pi/10$. Hence, the roots to the quartic are \[z = \cos \frac{\pi}{10}, \quad \cos \frac{3\pi}{10}, \quad \cos \frac{7\pi}{10}, \quad \cos \frac{9\pi}{10}.\] Note that we reject $z = \cos{5\pi/10}$ since $z \neq 0$.
    \end{ppart}
    \begin{ppart}
        By the quadratic formula, $z^2 = (5 \pm \sqrt5)/8$, so \[z = \pm \sqrt{\frac{5 \pm \sqrt5}{8}}.\] The corresponding trigonometric forms are
        \begin{gather*}
        ...
        \begin{align*}
            ...
            &= 4 - \bp{\frac{5 + \sqrt5}{8} + \frac{5 - \sqrt5}{8} + \frac{5 - \sqrt5}{8} + \frac{5 + \sqrt5}{8}}\\
            &= \frac32.
        \end{align*}
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        \item \begin{enumerate}
            ...
            \item Show that $w^2 + w^7 = 2\cos{4\pi/9}$.
            \item Using the results above, deduce that \[16\cos\frac{2\pi}9 \cos\frac{4\pi}9 \cos\frac{6\pi}9 \cos\frac{8\pi}9 = 1.\]
        \end{enumerate}
    \end{enumerate}

    Let point $O$ be the origin, and points $A$, $B$ and $C$ represent the complex numbers $w^2$, $2\i w^2$ and $w/u\conj$ respectively, where \[u = \frac13 \bp{\cos \frac{5\pi}{18} - \i \sin \frac{5\pi}{18}}.\]

    \begin{enumerate}
        \setcounter{enumi}{1}
        \item \begin{enumerate}
            \item Find the modulus and arguments of the complex numbers $w/u\conj$ and $2iw^2$, and illustrate the points $A$, $B$ and $C$ on a clearly labelled Argand diagram.
...
\begin{solution}
    \begin{ppart}
        \begin{psubpart}
            For integers $n$, we have $z^9 = \e^{2\pi n \i}$, so $z = \e^{2\pi n \i/9}$. Hence, the roots are $w^n$, where $w = \e^{2\pi \i/9}$.
        \end{psubpart}
        \begin{psubpart}
            Recognizing the sum as a geometric series, we see that
        ...
        \begin{psubpart}
            ...
            \begin{align*}
                ...
                &\hspace{2em}= \bp{w+w^8}\bp{w^2+w^7}\bp{w^3+w^6}\bp{w^4+w^5}\\
                &\hspace{2em}= \bp{w^5 + w^6 + w^{12} + w^{13}}\bp{w^5 + w^8 + w^{10} + w^{13}}\\
                &\hspace{2em}= \bp{w^{10} + w^{13} + w^{15} + w^{18}} + \bp{w^{11} + w^{14} + w^{16} + w^{19}}\\
                &\hspace{4em}+ \bp{w^{17} + w^{20} + w^{22} + w^{25}} + \bp{w^{18} + w^{21} + w^{23} + w^{26}}\\
                &\hspace{2em}= \bp{w + w^4 + w^6 + 1} + \bp{w^2 + w^5 + w^7 + w} \\
                &\hspace{4em}+ \bp{w^8 + w^2 + w^4 + w^7} + \bp{1 + w^3 + w^5 + w^8}\\
                &\hspace{2em}= 2\bp{1 + w + w^2 + w^3 + w^4 + w^5 + w^6 + w^7 + w^8} - \bp{w^3 + w^6}\\
                &\hspace{2em}= -2\cos \frac{6\pi}{9}\\
                &\hspace{2em}= 1.
            \end{align*}
        \end{psubpart}
    \end{ppart}
    \begin{ppart}
        \begin{psubpart}
            Note that $w^2 = e^{4\pi \i/9}$. Hence, $\abs{w^2} = 1$ and $\arg w^2 = 4\pi/9$. Likewise, $\abs{2\i w^2} = 2$ and $\arg{2\i w^2} = 4\pi/9 + \pi/2 = 17\pi/18$. Lastly, note that \[\frac{w}{u\conj} = \frac{uw}{uu\conj} = \frac{\bp{1/3} \e^{-5\pi \i/18} \cdot \e^{2\pi \i/9}}{1/9} = 3\e^{-\i\pi/18},\] whence $\abs{w/u\conj} = 3$ and $\arg{w/u\conj} = -\pi/18$.
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Let $\t$ be the acute angle between the deck and the horizontal. Note that the horizontal plane has normal vector $\cveciiix001$. Thus, \[\cos \t = \frac{\abs{\cveciiix001 - \cveciiix{-1}0{20}}}{\abs{\cveciiix001}\abs{\cveciiix{-1}0{20}}} = \frac{20}{\sqrt{401}},\] so $\t = 2.9\deg \todp{1}$.
    \end{ppart}
\end{solution}
