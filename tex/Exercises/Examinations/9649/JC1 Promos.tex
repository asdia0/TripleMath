...
\begin{solution}
    \begin{ppart}
        Note that $z' = 2y \cdot y'$. Hence, \[\bp{x^3 + 1} y \der{y}{x} + 3 x^2 y^2 = \bp{x^3 + 1} \frac12 z' + 3x^2 z = 2 \implies z' + \frac{6x^2}{x^3 + 1} z = \frac{4}{x^3 + 1}.\] The integrating factor is hence \[\IF = \exp \int \frac{6x^2}{x^3 + 1} \d x = \exp{2\ln{x^3 + 1}} = \bp{x^3 + 1}^2.\] Multiplying through, we get \[\der{}{x} \bs{\bp{x^3 + 1}^2 z} = \bp{x^3 + 1}^2 z' +6x^2\bp{x^3 + 1} z = 4\bp{x^3 + 1}.\] Integrating both sides with respect to $x$, we obtain \[\bp{x^3 + 1}^2 z = \int 4\bp{x^3 + 1} \d x = x^4 + 4x + C,\] thus \[y^2 = \frac{x^4 + 4x + C}{\bp{x^3 + 1}^2}.\] When $x = 0$, $y = 2$, giving $C = 4$. Hence, \[y = \sqrt{\frac{x^4 + 4x + 4}{\bp{x^3 + 1}^2}}.\] Thus, when $x = 1$, we have $y = 3/2$.
    \end{ppart}
    \begin{ppart}
        Substituting $x = 1$ and $y = 3/2$ into the differential equation, we see that \[\bp{1^3 + 1} \bp{\frac32} \der{y}{x} + 3 \bp{1^2} \bp{\frac32}^2 = 2 \implies \der{y}{x} = -\frac{19}{12}.\] By the chain rule, it follows that \[\der{y}{t} = -\frac{19}{12} \cdot \der{x}{t} = -\frac{19}{12} \e^{\sqrt t}.\]
