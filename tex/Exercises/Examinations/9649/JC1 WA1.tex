...
\begin{problem}
    ...
    The sequence of positive numbers, $u_n$, satisfies the recurrence relation $u_{n+1} = \sqrt{2u_n + 3}$ for integers $n \geq 1$.
...
\begin{solution}
    \begin{ppart}
        We have \[m = \lim_{n \to \infty} u_n = \lim_{n \to \infty} \sqrt{2u_{n-1} + 3} = \sqrt{2\lim_{n \to \infty} u_{n-1} + 3} = \sqrt{2m + 3}.\] Squaring both sides, we get $m^2 = 2m+3$, so $m = 3$ or $m = -1$. Since $u_n$ is always positive, we take $m = 3$.
    \end{ppart}
    \clearpage
    \begin{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Let $f(x) = x^2 - 3x + 2 - \e^{-x} = 0$. Then \[x = \frac13 \bp{x^2 + 2 - \e^{-x}}.\] Hence, we should use an iterative method based on the form \[x_{n+1} = \frac13 \bp{ x^2_n + 2 - \e^{-x_n}}.\] Starting with $x_0 = 0.5$,
