...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Take $a = 1/p$, $b = 1/q$ and $c = 1/r$. Note that $abc = 1/pqr = 36$. Using (b), we see that \[\frac{4/p^2}{1/q + 3/r} + \frac{1/q^2}{3/r + 2/p} + \frac{9/r^2}{2/p + 1/q} \geq 9.\] This simplifies to \[\frac{4pqr}{p^3 (r+3q)} + \frac{pqr}{q^3(3p+2r)} + \frac{9pqr}{r^3(2q+p)} \geq 9.\] Recalling that $pqr = 1/36$, we have \[\frac1{9p^3 (r + 3q)} + \frac1{36q^3(3p+2r)} + \frac1{4r^3(2q+p)} \geq 9\] as desired.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        ...
        \begin{psubpart}
            ...
            Finally, by the Principle of Inclusion-Exclusion, the number of 4-by-4 unit square grids that contain at least one entirely shaded 3-by-3 square is given by \[\abs{A_1 \cup A_2 \cup A_3 \cup A_4} = \binom{4}{1} \cdot 2^7 - \bp{4 \cdot 2^4 + 2 \cdot 2^2} + \binom43 \cdot 2^1 - 1 = 447.\]
        \end{psubpart}
    \end{ppart}
    \begin{ppart}
    ...
    \begin{ppart}
        ...
        \end{psubpart}
        \begin{psubpart}
            It suffices to count the number of grids that have at most 4 white unit-squares. Let $d_i$ be the number of white squares in the $i$ column from the left. Then we have $d_1 + d_2 + d_3 + d_4 \leq 4$. Introducing the dummy variable $d_5 \geq 0$ to convert the inequality into an equality, we have $d_1 + d_2 + d_3 + d_4 + d_5 = 4$. By stars-and-bars, there are $\binom{4+4}{4} = 70$ solutions.
        \end{psubpart}
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        \end{psubpart}
        \begin{psubpart}
            Observe that there are $\binom{n}{k}$ ways to construct a subset of size $k$, so $\P{\abs{A_1} = k} = \binom{n}{k} / 2^n$. The given sum can hence be written as \[\sum_{k = 0}^n \binom{n}{k} \frac1{2^{n+k}} = \sum_{k = 0}^n 2^{-k} \cdot \frac{\binom{n}{k}}{2^n} = \sum_{k = 0}^n \P{A_1 \cap A_2 = \varnothing}{\abs{A_1} = k} \P{\abs{A_1} = k}.\] By the law of total probability, this is simply $\P{A_1 \cap A_2 = \varnothing} = (3/4)^n$.
        \end{psubpart}
    \end{ppart}
    \begin{ppart}
        \begin{psubpart}
...
\begin{solution}
    \begin{ppart}
        ...
        \end{psubpart}
        \begin{psubpart}
            Suppose for a contradiction that there exists such a polynomial $f$. Then $f(3) - f(1) = 5 - 2 \equiv 1 \pmod{3-1}$. But $1 \not\equiv 0 \pmod{2}$, contradicting the result in (a)(i). Thus, such an $f$ cannot exist.
        \end{psubpart}
        \begin{psubpart}
    ...
    \begin{ppart}
        \begin{psubpart}
            Note that $2^2 + 2 + 1 \equiv 0 \pmod{7}$ and $4^2 + 4 + 1 \equiv 0 \pmod{7}$. Since $2 \not\equiv 4$ modulo 7, by definition, 2 and 4 are incongruent roots modulo 7 for $x^2 + x + 1$.
        \end{psubpart}
        \begin{psubpart}
            We have $h(\g) \equiv 0 \pmod{p}$, so $h(\g) = kp$ for some integer $k$. By the remainder theorem, there exists some polynomial $Q(x)$ with integer coefficients such that $h(x) = (x-\g) Q(x) + kp$. Reducing modulo $p$, we obtain $h(x) \equiv (x-\g) Q(x) \pmod{p}$.
...
\begin{problem}
    \begin{center}
        \bd{\underline{On the Irrationality of $\pi$}}
    \end{center}
