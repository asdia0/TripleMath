...
\begin{solution}
    \begin{ppart}
        \begin{table}[H]
            ...
            \begin{tabular}{|c|c|c|}
            \hline
            $k$ & \bd{Boxes of 5} & \bd{Boxes of 11} \\ \hline\hline
            40 & 8 & 0 \\ \hline
            41 & 6 & 1 \\ \hline
            42 & 4 & 2 \\ \hline
            43 & 2 & 3 \\ \hline
...
\begin{solution}
    \begin{ppart}
        \begin{psubpart}
            Note that for all $k \in \RR$ and $r \in \NN$, we have $\lim_{x \to \infty} k/x^r = 0$. Thus, \[\lim_{x \to \infty} g(x) = \lim_{x \to \infty} x^n \bp{\frac{a_0}{x^n} + \frac{a_1}{x^{n-1}} + \frac{a_2}{x^{n-1}} + \dots + \frac{a_{n-1}}{x} + a_n} = \lim_{x \to \infty} a_n x^n,\] which approaches $\infty$ if $a_n > 0$, or $-\infty$ if $a_n < 0$.
        \end{psubpart}
        \begin{psubpart}
            By way of contradiction, suppose $P$ is finite, say $P = \bc{p_1, \dots, p_k}$. Then for all $m \in \ZZ$, either $g(m) = 0$ or, by the Fundamental Theorem of Arithmetic, \[g(m) = \pm \prod_{i = 1}^k p_i^{a_i},\] where each $a_i$ is a non-negative integer. By part (a)(i), there exists some $M \in \NN$ such that $g(m) \geq 0$ for all $m \geq M$. Define \[S = \sum_{m = M}^\infty \frac1{g(m)^{1/n}}.\] Since $g$ is a polynomial of degree $n$, it can take on a given value at most $n$ times. Thus, for a given set of integers $a_1, a_2, \dots, a_k$, \[g(m)^{1/n} = \prod_{i = 1}^k p_i^{a_i/n}\] for at most $n$ values of $m$. We thus obtain the following estimate: \[S \leq \sum_{a_1, \dots, a_k = 0}^\infty n \prod_{i = 1}^k p_i^{-a_i/n} = n \prod_{i = 1}^k \sum_{a_i = 0}^\infty p_i^{-a_i/n} = n \prod_{i = 1}^k \frac{1}{1 - p_i^{-1/n}},\] which is finite. But this is a clear contradiction, since \[\frac1{g(m)^{1/n}} \sim \frac1{a_n^{1/n} x},\] so $S$ diverges like the harmonic series. Thus, $P$ must be infinite.
        \end{psubpart}
    \end{ppart}
    \begin{ppart}
