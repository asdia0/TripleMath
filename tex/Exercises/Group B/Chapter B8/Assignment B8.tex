...
\begin{problem}
    \begin{enumerate}
        \item Given that $f$ is a continuous function, explain, with the aid of a sketch, why \[\lim_{n \to \infty} \frac1n \bs{f\of{\frac1n} + f\of{\frac2n} + \ldots + f\of{\frac{n}{n}}} = \int_0^1 f(x) \d x.\]
...
\end{problem}
\begin{solution}
    Since $y = f(x)$, we have $\d y = f'(x) \d x$. Hence, \[\int_\a^\b g(y) \d y = \int_a^b \inv f\of{f(x)} f'(x) \d x = \int_a^b xf'(x) \d x.\] Integrating by parts,
    \[\begin{array}{r c @{\hspace*{1.0cm}} c}\toprule
        & D & I \\\cmidrule{1-3}
        + & x & f'(x) \\
        - & 1 & f(x) \\\bottomrule
