...
\end{problem}
\begin{solution}
    Substituting the given values of $L$, $R$, $C$ and $V(t)$, the differential equation becomes \[0.5 \der[2]{Q}{t} + 10 \der{Q}{t} + \frac{Q}{0.02} = 50 \e^{-10t},\] which simplifies as \[\der[2]{Q}{t} + 20 \der{Q}{t} + 100 Q = 100 \e^{-10t}.\] The characteristic equation $r^2 + 20r + 100 = 0$ has the repeated root $r = -10$. Thus, $Q_c = \bp{A + Bt} \e^{-10t}$. For the particular solution, we try $Q_p = Ct^2 \e^{-10t}$. Differentiating, we have \[\der{Q_p}{t} = C\e^{-10t} \bp{2t - 10t^2} \quad \tand \quad \der[2]{Q_p}{t} = C\e^{-10t} \bp{2 - 40t + 100t^2}.\] Substituting this into the differential equation, we get \[C\e^{-10t} \bp{2 - 40t + 100t^2} + 20C\e^{-10t} \bp{2t - 10t^2} + 100Ct^2 \e^{-10t} = 100\e^{-10t},\] whence $C = 50$. Thus, $Q = Q_c + Q_p = \bp{A + Bt} \e^{-10t} + 50 t^2 \e^{-10t}$. Note that \[\der{Q}{t} = \e^{-10t} \bp{B - 10 A - 10B t + 100 t - 500t^2}.\] Since $Q = 0$ and $\derx{Q}{t} = 0$ at $t = 0$, we get $A = 0$ and $B = 10A = 0$. Hence, $Q = 50 t^2 \e^{-10t}$.
...
\begin{solution}
	...
	Note that the characteristic equation $r^2 + 2r - 3 = 0$ has roots $r = 1$ and $r = -3$. Thus, $y_c = A\e^{\t} + B\e^{-3\t}$. For the particular solution, we try $y_p = C\sin 2\t + D \cos 2\t$. Note that \[y_p' = 2C \cos 2\t - 2D \sin 2\t \quad \tand \quad y_p'' = -4C \sin 2\t - 4D \cos 2\t.\] Substituting this into the DE, we get
    \begin{align*}
        &\bp{-4C \sin 2\t - 4D \cos 2\t} + 2\bp{2C \cos 2\t - 2D \sin 2\t} - 3\bp{C\sin 2\t + D \cos 2\t}\\
        &\hspace{2em}= 3\sin 2\t - \cos 2\t.
    \end{align*}
...
\begin{solution}
    \begin{ppart}
        Note that the characteristic equation of the DE is given by \[r^2 + \frac{c}{m} r + \frac{k}{m} = 0,\] which simplifies to $m r^2 + cr + k = 0$. For $y$ to oscillate (i.e. composed of sine and cosine terms), the roots of the characteristic equation must be non-real. We hence obtain the constraint $\D = c^2 - 4mk < 0$.
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Note that the characteristic equation is $r^2 + w^2 = 0$, whence $r = \pm \i w$. Thus, $y_c = A\cos wt + B\sin wt$. For the particular solution, we try $y_p = Ct\sin wt + Dt \cos wt$. Differentiating, we have
        \begin{align*}
            y_p' &= wt \bp{C\cos wt - D \sin wt} + C\sin wt + D \cos wt,\\
            y_p'' &= w^2 t \bp{-C\sin wt - D \cos wt} + 2w \bp{C \cos wt - D \sin wt}.
        \end{align*}
        Substituting this into the DE, we get
        \begin{align*}
            &w^2 t \bp{-C\sin wt - D \cos wt} + 2w \bp{C \cos wt - D \sin wt} + w^2 \bp{Ct\sin wt + Dt \cos wt}\\
            &\hspace{2em}= \sin wt.
        \end{align*}
        Comparing coefficients, we obtain $C = 0$ and $D = -1/2w$. Thus, \[y = y_c + y_p = A\cos wt + B \sin wt - \frac{t}{2w} \cos wt.\] Note that \[y' = -Aw \sin wt + Bw \cos wt - \frac1{2w} \bp{\cos wt - tw\sin wt}.\] Since $y(0) = y'(0) = 0$, we have $A = 0$ and $B = 1/2w^2$. Thus, \[y = \frac1{2w^2} \sin wt - \frac{t}{2w} \cos wt.\]
    \end{ppart}
\end{solution}