...
\begin{solution}
    \begin{ppart}
        We have \[\frac{2}{\sqrt{1-y^2}} \der{y}{x} = \cos x.\] Integrating with respect to $x$, we get \[\int \frac{2}{\sqrt{1-y^2}} \d y = \int \cos x \d x,\] so \[2\arcsin y = \sin x + C_1.\] Thus, \[y = \sin{\frac12 \sin x + C}.\]
    \end{ppart}
    \begin{ppart}
        We have \[y \der{y}{t} = \frac{t}{1-t^2}.\] Integrating with respect to $t$, we get \[\int y \d y = \int \frac{t}{1-t^2} \d t,\] so \[\frac12 y^2 = -\frac12 \ln \abs{1-t^2} + C_1.\] Thus, \[y^2 = C_2 - \ln \abs{1 - t^2}.\] Since $y = 4 \geq 0$ when $t = 0$, we have \[4^2 = C_2 - \ln \abs{1 - 0}.\] This gives  $C_2 = 16$, hence \[y = \sqrt{16 - \ln \abs{1 - t^2}}.\]
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item $\displaystyle (1 + t^2) \der{y}{t} = 2ty+2$
        \item $\displaystyle (x+1)\der{y}{x} + \frac{y}{\ln{x+1}} = x^2 + x$, where $x > 0$
    \end{enumerate}
\end{problem}
\begin{solution}
    \begin{ppart}
        Note that \[xy' + (2x-3)y = 4x^4 \implies y' + \bp{2 - \frac3x} y = 4x^3.\] Hence, the integrating factor is \[\IF = \exp \int \bp{2 - \frac3x} \d x = \exp{2x - \ln 3} = \frac{\e^{2x}}{x^3}.\] Multiplying through by the integrating factor, we get \[\der{}{x} \bp{\frac{\e^{2x}}{x^3} y} = \frac{\e^{2x}}{x^3}y' + \frac{\e^{2x}}{x^3} \bp{2 - \frac3x} y = 4\e^{2x}.\] Integrating, we have \[\frac{\e^{2x}}{x^3} y = \int 4\e^{2x} \d x = 2\e^{2x} + C,\] so \[y = \frac{x^3}{\e^{2x}} \bp{2\e^{2x} + C} = 2x^3 + Cx^3\e^{-2x}.\]
    \end{ppart}
    \begin{ppart}
        We have \[\der{}{x} \bs{(1+x)y} = (1+x)y' + y = \cos x.\] Integrating, we get \[(1+x)y = \int \cos x \d x = \sin x + C,\] so \[y = \frac{\sin x + C}{x+1}.\] Since $y(0) = 1$, \[1 = \frac{\sin 0 + C}{0+1},\] which gives $C = 1$, whence \[y = \frac{\sin x + 1}{x + 1}.\]
    \end{ppart}
    \begin{ppart}
    ...
    \end{ppart}
    \begin{ppart}
        Note that \[(x+1)\der{y}{x} + \frac{y}{\ln{x+1}} = x^2 + x \implies \der{y}{x} + \frac{y}{(x+1)\ln{x+1}} = x.\] Hence, the integrating factor is \[\IF = \exp \int \frac{1/(x+1)}{\ln{x+1}} \d x = \exp \ln \ln{x+1} = \ln{x+1}.\] Multiplying through by the integrating factor, we get \[\der{}{x} \bp{y\ln{x+1}} = \ln{x+1} \der{y}{x} + \frac{y}{(x+1)} = x\ln{x+1}.\] Integrating, we obtain \[y\ln{x+1} = \int x\ln{x+1} \d x.\] Integrating by parts, we get
        \begin{align*}
...
\end{problem}
\begin{solution}
    Note that \[(x+y)y' = x-y \implies y' = \frac{x-y}{x+y}.\]Let $f(x, y) = \frac{x-y}{x+y}$. Then \[f(kx, ky) = \frac{kx-ky}{kx+ky} = \frac{x-y}{x+y} = f(x, y).\] Hence, the differential equation can be solved with the substitution $y = ux$, whence $y' = u' x + u$. Substituting this into the differential equation, we get \[u' x + u = \frac{x - ux}{x + ux} = \frac{1-u}{1 + u},\] which simplifies to \[\bp{\frac{1 + u}{1 - 2u - u^2}} u' = \frac1x.\] Integrating with respect to $x$, we get \[\int \frac{1 + u}{1 - 2u - u^2} \d u = \int \frac{1+u}{2-(1+u)^2} \d u = \int \frac1x \d x,\] so \[-\frac12 \ln \abs{2 - (1+u)^2} = -\frac12 \ln \abs{2 - \bp{1 + \frac{y}{x}}^2} = \ln x + C_1.\] Hence, $(x + y)^2 = 2x^2 + C$, where $C = -\e^{-2C_1} \in \RR^-$.
\end{solution}
...
\begin{solution}
    ...
    \textbf{Integrating Factor.} Note that the integrating factor is $\e^{\int p(x) \d x}$. Multiplying through, we get \[\der{}{x} \bp{\e^{\int p(x) \d x} y} = \e^{\int p(x) \d x}y' + \e^{\int p(x) \d x} p(x) y = \e^{\int p(x) \d x} f(x) = \frac1{\l} e^{\int p(x) \d x} p(x).\] Integrating, we get \[\e^{\int p(x) \d x} y = \int \frac1{\l} \e^{\int p(x) \d x} p(x) \d x = \frac1{\l} \e^{\int p(x) \d x} + C,\] whence \[y = \frac1{\lambda} + Ce^{-\int p(x) \d x}.\]
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Multiplying through by $x^{-2}$, we get \[\der{}{x} \bp{\frac{z}{x^2}} = \frac1{x^2} \der{z}{x} - \frac{2z}{x^3} = \frac{1}{x^2}.\] Integrating, we get \[\frac{z}{x^2} = \int \frac{1}{x^2} \d x = -\frac{1}{x} + C_1,\] so \[z = -\frac1{2y^2} = -x + C_1x^2.\] Thus, \[y^2 = \frac1{2x + C_2 x^2}.\]
...
\begin{solution}
    ...
    We now solve the given differential equation. Note that \[x \der{y}{x} - 4x^2 y + 2y \ln y = 0 \implies \der{y}{x} - 4x y = -\frac2x (y\ln y).\] Hence, \[P(x) = -4x, \quad Q(x) = -\frac2x.\] Now, from the previous part, we can rewrite the differential equation as \[\der{v}{x} - 4x = -\frac{2v}{x},\] where $v(x) = \ln y$. Thus, \[\der{}{x} \bp{x^2 v} = x^2 \der{v}{x} + 2x v =  4x^3.\] Integrating, we get \[x^2 v = x^2 \ln y = \int 4x^3 \d x = x^4 + C,\] so \[y = \exp{x^2 + Cx^{-2}}.\]
\end{solution}
...
\begin{solution}
    ...
    \begin{align*}
        2xt \ln t \der{x}{t} + (3\ln t + 1)x^2 &= \bp{t\der{y}{t} - x^2} + (3\ln t + 1)x^2\\
        &= t\der{y}{t} + 3x^2\ln t\\
        &= t\der{y}{t} + 3y.
    \end{align*}
    Our differential equation thus becomes \[t\der{y}{t} + 3y = \frac{\e^{-2t}}{t} \implies \der{y}{t} + \frac{3y}{t} = \frac{\e^{-2t}}{t^2},\] whence $P(t) = 3/t$. We now solve the differential equation. Observe that \[\der{}{t} \bp{t^3 y} = t^3 \der{y}{t} + 3t^2y = t \e^{-2t}.\] Integrating, \[t^3 x^2 \ln t = t^3 y = \int t\e^{-2t} \d t = -\frac{1}{2} t\e^{-2t} - \frac14 \e^{-2t} + C_1,\] thus \[x^2 = \frac{C-\bp{2t+1}\e^{-2t}}{4t^3 \ln t}.\]
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        We have \[\frac{1}{x(N-x)} \der{x}{t} = k.\] Integrating with respect to $t$, \[\int \frac{1}{x(N-x)} \d x = \int k \d t.\] Performing partial fractions on the integrand on the LHS, we obtain \[\int \frac1N \bp{\frac1x - \frac1{N-x}} = \int k \d t,\] so \[\frac{\ln x - \ln{N-x}}{N} = kt + C_1.\] This yields \[\ln \frac{x}{N-x} = Nkt + C_2,\] whence \[x = \frac{C_3 N\e^{Nkt}}{1 + C_3 \e^{Nkt}}.\]
...
\end{problem}
\begin{solution}
    Note that \[u = y^{1-n} \implies \der{u}{x} = (1-n)y^n \der{y}{x} \implies \der{y}{x} = \der{u}{x} \cdot \frac{y^n}{1-n}.\] Substituting this into the given differential equation, we get \[\der{u}{x} \cdot \frac{y^n}{1-n} + p(x)y = q(x) y^n.\] Multiplying by $(1-n) y^{-n}$, we have \[\der{u}{x} + (1-n)p(x)u = (1-n)q(x).\]

    Let $n = 2$. Then $u = I^{-1}$. We also have $p(x) = \tan t$ and $q(x) = \sin^2 t$. Then \[\der{u}{t} - (\tan t)u = -\sin^2 t.\] Multiplying by $\cos t$, we see that \[\der{}{t} \bp{u \cos t} = \cos t \der{u}{t} - (\sin t) u = -\cos t \sin^2 t.\] Integrating, we obtain \[u \cos t = \int -\cos t \sin^2 \t \d t = -\frac13 \sin^3 t + C,\] whence \[u = \frac{-1/3 \cdot \sin^3 t + C}{\cos t}\] and \[I = \frac{\cos t}{-1/3 \cdot \sin^3 t + C} = \frac{3\cos t}{3C - \sin^3 t}.\]
