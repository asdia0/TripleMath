...
\begin{solution}
    \begin{ppart}
        Observe that the argument of the square root must be non-negative. Hence, $1 - x^2 - y^2 \geq 0$, so $\dom f = \{ (x, y) \in \RR^2 : x^2 + y^2 \leq 1\}$.
    \end{ppart}
    \begin{ppart}
        Observe that the argument of the natural log must be positive. Hence, $x^2 - y > 0$, so $\dom f = \{(x, y) \in \RR^2 : y < x^2\}$.
    \end{ppart}
    \begin{ppart}
        Observe that the argument of $\arcsin$ must be within the range of the sine function, i.e. between $-1$ and 1 inclusive. Hence, $-1 \leq x + y \leq 1$. Thus, $\dom f = \{(x, y) \in \RR^2 : -1 \leq x + y \leq 1\}$.
    \end{ppart}
    \begin{ppart}
        Observe that the denominator must be non-zero. Hence, $y^2 \neq x^2$, so $y \neq x$ and $y \neq -x$. Thus, $\dom f = \{(x, y) \in \RR^2 : y \neq x \tand y \neq -x\}$.
    \end{ppart}
\end{solution}
...
\end{problem}
\begin{solution}
    Differentiating $f$ with respect to $x$ and $y$ respectively, we have
    \begin{align*}
        f_x(x, y) &= 4x^3 \sin{xy^3} + x^4 y^3 \cos{xy^3},\\
        f_y(x, y) &= 3x^5y^2\cos{xy^3}.
    \end{align*}
...
\begin{problem}
    Given that $z = x^2y$, $x = t^2$, $y = t^3$, use the chain rule to find $\derx{z}{t}$ in terms of $t$.
\end{problem}
\begin{solution}
    \[\der{z}{t} = \pder{z}{x} \der{x}{t} + \pder{z}{y} \der{y}{t} = 2xy \bp{2t} + x^2 \bp{3t^2} = 2t^2t^3 \cdot 2t + \bp{t^2}^2 \cdot 3t^2 = 7t^6.\]
\end{solution}
...
\begin{problem}
    Suppose that a point moves along the intersection of the sphere $x^2 + y^2 + z^2 = 1$ with the plane $x = 2/3$. Find the rate of $z$ with respect to $y$ when the point is at $\bp{2/3, 1/3, 2/3}$.
\end{problem}
\begin{solution}
    Note that $x^2 + y^2 + z^2 = 1$, so $z = \pm \sqrt{1 - x^2 - y^2}$. Given that the object we want (the rate of change of $z$ with respect to $y$) will later be evaluated when $z = 2/3 > 0$, we consider the positive branch. Let $f(x, y) = \sqrt{1 - x^2 - y^2}$. Then \[f_y(x, y) = \frac{-y}{\sqrt{1 - x^2 - y^2}}.\] Evaluating at the desired point, we get, \[f_y\bp{\frac23, \frac13} = \frac{-1/3}{\sqrt{1 - (2/3)^2 - (1/3)^2}} = -\frac12.\]
\end{solution}

\begin{problem}
    \begin{enumerate}
        \item The Cauchy-Riemann equations are such that \[\pder{u}{x} = \pder{v}{y} \quad \tand \quad \pder{u}{y} = -\pder{v}{x}\] for $u(x, y)$ and $v(x, y)$. Show that $u = \e^x \cos y$, $v = \e^x \sin y$ satisfy the Cauchy-Riemann equations.
        \item Show that the function $f(x, y) = \e^x \sin y + \e^y \cos x$ satisfies that Laplace equation \[\pder[2]{f}{x} + \pder[2]{f}{y} = 0.\]
    ...
    \begin{ppart}
        ...
        \end{cases} \quad \tand \quad \begin{cases}
            \begin{aligned}
                ...
                u_{yy} &= -v_{xy}.
            \end{aligned}
        \end{cases}\] For both $u$ and $v$ to satisfy the Laplace equation, we see that
        \[\begin{cases}
            \begin{aligned}
        ...
        \end{cases}
        \]
        The conditions are thus $u_{xy} = u_{yx}$ and $v_{xy} = v_{yx}$.
    \end{ppart}
\end{solution}
...
\end{problem}
\begin{solution}
    Let $V(r, h) = \pi r^2 h/3$ be the volume of the cone. We have $V_r(r, h) = 2\pi r h/3$ and $V_h(r, h) = \pi r^2/3$. The equation of the tangent plane at $r = 12$ and $h = 10$ is given by
    \begin{align*}
        v &= V(12, 10) + V_r(12, 10)(r-12) + V_h(12, 10)(h-10)\\
        &= \frac13\pi(12^2)(10) + \frac23 \pi(12)(10)(r - 12) + \frac13 \pi (12^2) (h-10)\\
        &= 16\pi(5r + 3h - 60).
    \end{align*}
    Evaluating at $r = 11.95$ and $h = 10.01$, we have $v = 16\pi\bs{5(11.95) + 3(10.01) - 60} = 476.48\pi$. The volume of the cone after the changes is hence approximately $476.48\pi$ cm$^3$.
\end{solution}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        For a level path, the instantaneous rate of change of the climber's altitude should be 0. Let the direction of the climber be $u = \cveciix{a}{b}$. \[\evalder{D_{\vec u} f(x, y)}{(-20, 5)} = -4\cvecii{-20}{10} \cdot \cvecii{a}{b} = 0.\] We thus have $b = 2a$, so \[\vec u = \cvecii{a}{b} = \cvecii{a}{2a} = a\cvecii12.\] Thus, the climber should walk in the direction of $\cveciix{1}{2}$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    ...
    \case{3}[$x \in [0, 3]$, $y = 5 - 5x/3$] Observe that \[f\of{x, 5 - \frac53 x} = 3x\bp{5 - \frac53 x} - 6x - 3\bp{5 - \frac53 x} + 7 = -(x-2)(5x-4)\] is concave down and has a turning point at $x = 1.4$. Hence, the function clearly attains a maximum of $1.8$ when $x = 1.4$ and a minimum of $-11$ when $x = 3$ (note that at $x = 0$, the function returns $-8$). 
...
\end{problem}
\begin{solution}
    Let the dimensions of the base of the box be $x$ and $y$ cm. Let the height of the box be $z$ cm. Given that the volume of the box is fixed at 32 cm$^3$, we have $xyz = 32$, so $z = 32/xy$. Let the surface area of the box be measured by $f(x, y)$. Then \[f(x, y) = xy + 2yz + 2xz = xy + 2y\bp{\frac{32}{xy}} + 2x\bp{\frac{32}{xy}} = xy + 64x^{-1} + 64y^{-1}.\] Note that \[\nabla f = \cvecii{f_x(x, y)}{f_y(x, y)} = \cvecii{y-64x^{-2}}{x - 64y^{-2}}.\] For stationary points, $\nabla f = \vec 0$. We hence obtain $y = 64x^{-2}$ and $x = 64y^{-2}$. Multiplying the two equations together and rearranging, we get $x^3 y^3 = 64^2$, so $xy = 16$. Thus, $x = y = 4$ and $z = 2$, so $f(x, y)$ has a stationary point at $(4, 4, 2)$. 
    
    We now consider the nature of this stationary point. Note that $f_{xx}(x, y) = 128x^{-3}$, $f_{yy} = 128y^{-3}$, and $f_{xy} = 1$. Hence, \[D = f_{xx}(4, 4)f_{yy}(4, 4) - \bs{f_{xy}(4, 4)}^2 = 3\] Since $D > 0$ and $f_{xx}(4, 4) = 2 > 0$, by the second derivative test, $f(x, y)$ attains a minimum at $(4, 4, 2)$. Thus, the amount of material required is lowest for a box of dimension 4 cm $\times$ 4 cm $\times$ 2 cm.
\end{solution}
...
\begin{solution}
    ...
    \begin{align*}
        ...
        & \hspace{2em} + \frac12 f_{xx}(1, 1)(x-1)^2 + f_{xy}(1, 1)(x-1)(y-1) + \frac12 f_{yy}(1, 1)(y-1)^2\\
