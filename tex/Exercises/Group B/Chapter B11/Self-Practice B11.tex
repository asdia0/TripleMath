...
\begin{solution}
    ...
    \case{1} At $(-1, -1)$, we have $D = 128 > 0$. Since $f_{xx} = -12(-1)^2 = -12 < 0$, by the second partial derivative test, $(-1, -1)$ is a maximum point.

    \case{2} At $(0, 0)$, we have $D = -16 < 0$. By the second partial derivative test, $(0, 0)$ is a saddle point.

    \case{3} At $(1, 1)$, we have $D = 128 > 0$. Since $f_{xx} = -12(1)^2 = -12 < 0$, by the second partial derivative test, $(1, 1)$ is a maximum point.
\end{solution}
...
\end{solution}
\begin{solution}[2]
    Observe that the longer side of the trapezium is given by $(27 - 2x) + 2(x\cos \t)$, while the height of the trapezium is given by $x \sin \t$. The area $A$ of the trapezium is thus given by \[A = \frac{\bp{27-2x} + \bp{27-2x+2x\cos\t}}{2} \bp{x \sin \t} = 27x\sin \t + x^2 \sin \t \bp{\cos \t - 2}.\] Observe that \[\nabla A = \cvecii{A_x}{A_{\t}} = \cvecii{27\sin \t + 2x \sin \t \bp{\cos \t -2}}{27x \cos \t + x^2\bp{\cos^2 \t - \sin^2 \t - 2\cos \t}}.\] Setting this equal to the zero vector, we get \[27\sin \t + 2x \sin \t \bp{\cos \t -2} = 0 \quad \tand \quad 27x \cos \t + x^2\bp{\cos^2 \t - \sin^2 \t - 2\cos \t} = 0.\] From the first equation, we get \[x = -\frac{27}{2\bp{\cos \t - 2}}.\] Substituting this into the second equation, \[27\bp{\frac{-27}{2\bp{\cos \t - 2}}}\cos \t + \bp{\frac{-27}{2\bp{\cos \t - 2}}}^2\bp{\cos^2 \t - \sin^2 \t - 2\cos \t} = 0.\] Clearing denominators and simplifying, we get $-\cos^2 \t + 2\cos \t - \sin^2 \t = 0$, so $\cos \t = 1/2$, whence $\t = \pi/3$ and $x = 9$.

    We now calculate the second partial derivatives of $A$ at $\t = \pi/3$ and $x = 9$. We have
    \begin{align*}
        A_{xx} &= 2\sin\t \bp{\cos \t - 2} = -2.5981 \tosf{5},\\
        A_{\t\t} &= -27x\sin\t + x^2\bp{-2\cos\t\sin\t -2\sin\t\cos\t+2\sin\t} = -210.44 \tosf{5},\\
        A_{\t x} &= 27\cos \t + 2x\bp{\cos^2 \t - \sin^2 \t -2\cos \t} = -13.5.
    \end{align*}
...
\begin{solution}
    ...
    Let $n$ and $m$ be the amount of product $A$ and $B$ produced by the company. Let $\Pi = 10n + 15m$ be the total revenue earned. Due to resource constraints, we have the inequalities \[2n + m \leq 100 \quad \tand \quad n + 3m \leq 90.\] Additionally, we have $n, m \geq 0$.
