...
\begin{problem}
    Find
    \begin{enumerate}
        \item $\int \tan{\pi/6 - 3x} \d x$,
        \item $\int \tan x \sec^4 x \d x$,
        \item $\int 1/(x^2 + 3x + 2) \d x$,
        \item $\int \cos{3x/2} \cos{5x/2} \d x$,
        \item $\int 2/(x \ln x^2) \d x$,
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        \[\int \frac1{x^2 + 3x + 2} \d x = \int \bp{\frac1{x+1} - \frac1{x+2}} \d x = \ln \abs{x + 1} - \ln \abs{x + 2} + C.\]
    \end{ppart}
    \begin{ppart}
...
\end{problem}
\begin{solution}
    Let \[\frac{x^2 + x + 28}{(1-x)(x^2 + 9)} = \frac{A}{1-x} + \frac{Bx + C}{x^2 + 9},\] where $A$, $B$ and $C$ are constants to be determined. By the cover-up rule, we immediately get $A = 3$. Clearing denominators, we get \[x^2 + x + 28 = 3\bp{x^2 + 9} + \bp{Bx + C}\bp{1 - x} = \bp{3 - B} x^2 + \bp{B - C} x + \bp{27 + C}.\] Comparing coefficients, we get $B = 2$ and $C = 1$, whence \[\frac{x^2 + x + 28}{(1-x)(x^2 + 9)} = \frac{3}{1-x} + \frac{2x + 1}{x^2 + 9}.\]
