...
\end{problem}
\begin{solution}
    We have \[\frac1{y^2} \der{y}{x} = \frac1x.\] Integrating both sides with respect to $x$, \[\int \frac1{y^2} \d y = \int \frac1x \d x.\] Thus, \[-\frac1{y} = \ln \abs{x} + C_1.\] Solving for $y$, we get \[y = \frac1{C - \ln \abs{x}}.\] Since $y(1) = 1$, we have \[1 = \frac{1}{C - \ln \abs{1}} \implies C = 1 \implies y = \frac1{1 - \ln \abs{x}}.\]
\end{solution}
...
\end{problem}
\begin{solution}
    We have \[\frac{10-x}{x} \der{x}{t} = k.\] Integrating with respect to $t$, we have \[\int \frac{10-x}{x} \d x = \int k \d t,\] so \[10\ln x - x = kt + C.\] Evaluating at $x = 1$ and $t = 0$, \[10\ln{1} - 1 = k(0) + C \implies C = -1.\] Evaluating at $x = 2$ and $t = 1$, \[10\ln{2} - 2 = k(1) - 1 \implies k = 10 \ln 2 - 1.\] Hence, evaluating at $x = 5$, we get \[10\ln{5} - 5 = \bp{10 \ln 2 - 1}t - 1 \implies t = 2.04 \tosf{3}.\]
\end{solution}
...
\begin{solution}
    \begin{ppart}
        We have \[\frac{1}{3-2z} \der{z}{x} = 1.\] Integrating both sides with respect to $x$, we get \[\int \frac{1}{3-2z} \d z = \int 1 \d x,\] so \[-\frac12 \ln{3-2z} = x + C_1.\] It follows that \[z = \frac32 - A \e^{-2x}.\] Thus, the general solution is \[z = \frac32 - A \e^{-2x}, \quad A \in \RR^+.\]
    \end{ppart}
    \begin{ppart}
        Since \[\der{y}{x} = \frac32 - A \e^{-2x},\] we have \[y = \int \bp{\frac32 - A \e^{-2x}} \d x = \frac32 x + \frac{A}2 \e^{-2x} + B, \quad B \in \RR.\]
    \end{ppart}
    \begin{ppart}
...
\end{problem}
\begin{solution}
    Since $\der{\t}{t} \propto (20 - \t)$, we have $\der{\t}{t} = k(20 - \t)$, where $k$ is a constant. We now solve for $\t$. Note that \[\frac1{20 - \t} \der{\t}{t} = k.\] Integrating both sides with respect to $t$, we get \[\int \frac1{20 - \t} \d \t = \int k \d t,\] so \[-\ln{20 - \t} = kt + C_1.\] Thus, $\t = 20 - C\e^{-kt}$.
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        We have \[\frac{1}{150-n} \der{n}{t} = \frac1{50}.\] Integrating, we obtain \[\int \frac{1}{150-n} \d n = \int \frac1{50} \d t,\] so \[-\ln{150 - n} = \frac1{50} t + C_1,\] and \[n = 150 - C\e^{-t/50}.\]
