...
\begin{solution}
    \begin{ppart}
        We have \[(2y-5)\der{y}{x} = 3x^2 - 4x + 1.\] Integrating both sides with respect to $x$, \[\int (2y-5) \d y = \int \bp{3x^2 - 4x + 1} \d x.\] This evaluates to \[y^2 - 5y = x^3 - 2x^2 + x + C_1.\] Note that $x^3 - 2x^2 + x = x(x-1)^2$. Hence, \[y^2 - 5y - x(x-1)^2 + C_2 = 0 \implies y = \frac{5 \pm \sqrt{4x(x-1)^2 + C_3}}{2}.\] Since the curve passes through the origin $(0, 0)$, we have \[0 = \frac{5 - \sqrt{C_3}}{2} \implies C_3 = 25.\] Thus, \[f(x) = \frac{5-\sqrt{4x(x-1)^2 + 25}}{2}.\] Note that we reject the positive branch since $f(x) > 0$ in that case.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    \begin{ppart}
        Note that \[y = ux \implies \der{y}{x} = \der{u}{x} x + u.\] Substituting this into the given differential equation, we get \[\der{u}{x} x + u = \frac{x + ux}{x},\] so \[\der{u}{x} = \frac1x.\] Integrating both sides with respect to $x$, we get $u = \ln{x} + C$, so $y = x \ln x + C x$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        We have \[10 \bp{16 - \frac12 h}^{-1/2} \der{h}{t} = 1.\] Integrating with respect to $t$, we get \[10 \int \bp{16 - \frac12 h}^{-1/2} \d h = \int \d t,\] thus \[-10 \sqrt{16 - \frac12 h} + C = t.\] Since $h = 0$ when $t = 0$, we have \[-10\sqrt{16} + C = 0 \implies C = 40.\] Thus, \[t = 40-10\sqrt{16 - \frac12 h}.\]
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Note that \[\der{x}{t} \propto x(1000-x) \implies \der{x}{t} = kx(1000-x)\] for some $k \in \mathbb{R}^+$. Hence, \[\frac1{x(1000-x)} \der{x}{t} = k.\] Integrating with respect to $t$, we obtain \[\int \frac1{x(1000-x)} \d x = \int k \d t,\] which evaluates to \[\frac1{1000} \ln{\frac{x}{1000-x}} + C = kt.\]
