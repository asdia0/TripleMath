...
\begin{problem}
    ...
    \begin{align*}
        f &: x \mapsto \frac{3x-2}{x+1}, \quad x \in \RR \setminus \bc{-1},\\
...
\begin{solution}
    \begin{ppart}
        Let $y = f(x) = (3x-2)/(x+1)$. Clearing denominators, we see that $xy + y = 3x - 2$. Solving for $x$, we get $x = (y+2)/(3-y)$, so \[\inv f(x) = \frac{x+2}{3-x}.\] Further, $\dom{\inv f} = \RR \setminus \bc{3}$ and $\ran{\inv f} = \RR \setminus \bc{-1}$.
    \end{ppart}
    \begin{ppart}
        We have $g^2(x) = 3\bp{3x+4} + 4 = 9x + 16$ and \[gf(x) = 3\bp{\frac{3x-2}{x+1}} + 4 = \frac{13x-2}{x+1},\] so
        \begin{align*}
            g^2 &: x \mapsto 9x + 16, \quad x \in \RR,\\
            gf &: x \mapsto \frac{13x-2}{x+1}, \quad x \in \RR \setminus \bc{-1}.
        \end{align*}
    \end{ppart}
    \begin{ppart}
...
\begin{problem}
    The function $f$ is defined as $f : x \mapsto 2 - (x-1)^2$ for $x \leq 1$.
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        Graphically, we see that the intersection points of $y = f(\b)$ and $y = \inv f(\b)$ lie on the line $y = x$. It thus suffices to solve $f(\b) = \b$. This gives $2 - \bp{\b - 1}^2 = \b$, which simplifies to $\b^2 - \b - 1 = 0$, hence $p = 1$ and $q = -1$.
    \end{ppart}
    \begin{ppart}
        Let $y = f(x) = 2 - (x-1)^2$. Then $x = 1 \pm \sqrt{2 - y}.$ Since $x \leq 1$, we take the negative branch, so $\inv f : x \mapsto 1 - \sqrt{2-x}$ for $x \leq 2$.
    \end{ppart}
\end{solution}

\begin{problem}
    The function $f$ is defined by $f : x \mapsto \cos{\pi x /2}$ for $-2 < x \leq 0$.
    ...
    The function $g$ is defined by $g : x \mapsto (2x+1)^{2/3}$ for $-2 < x \leq 2$.
...
\begin{solution}
    ...
    \begin{ppart}
        ...
        Let $y = f(x) = \cos{\pi x/2}$. Then $x = (2/\pi) \arccos y$. Note further that $\dom{\inv f} = \ran f = [-1, 1]$, so \[\inv f : x \mapsto \frac{2}\pi \arccos x, \quad -1 \leq x \leq 1.\]
    \end{ppart}
    \begin{ppart}
        Using G.C., we see that the solution set is $(-2, 0.673] \setminus \bc{0}$.
    \end{ppart}
    \begin{ppart}
        Since $\ran f = (-1, 1]$ and $\dom g (-2, 2]$, we have $\ran f \subseteq \dom g$, hence $gf$ exists.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Let $x_1, x_2 \in \dom g$ such that $g(x_1) = g(x_2)$. Then \[5 + \frac3{x_1} = 5 + \frac3{x_2}.\] It readily follows that $x_1 = x_2$, so $g(x)$ is one-one and thus invertible, i.e. $\inv g$ exists.
    \end{ppart}
    \begin{ppart}
        Let $y = g(x) = 5 + 3/x$. Then $x = 3/(y-5)$. Note also that for $x \geq k$, \[5 < 5 + \frac3x \leq 5 + \frac3k,\] so \[\inv g : x \mapsto \frac3{x-5}, \quad 5 < x \leq 5 + \frac3k.\]
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    \end{ppart}
    \begin{ppart}
        We have $\inv g(x^2) = 2$, so $x^2 = g(2) = 9$, whence $x = \pm 3$.
