...
\end{problem}
\begin{solution}
    Let $f(y) = 4y-1$ with initial value $y_0 = 1$. By the Euler method $y_{n+1} = y_n + \D t f(y_n)$,
    ...
    Observe that $\derx[2]{y}{t} = 4\derx{y}{t} > 0$ for $y > 0$. Hence, $y$ is concave upward. Thus, the approximation is an underestimate.
\end{solution}

\begin{problem}
    A solution to the differential equation $\derx{y}{x} = y - x$ has $y = 0.5$ at $x = 0$.
    \begin{enumerate}
...
\begin{solution}
    \begin{ppart}
        Let $f(x, y) = y - x$ with initial values $x_0 = 0$ and $y_0 = 0.5$. Take a step size of $\D x = 0.2$. By the Euler method $y_{n+1} = y_n + \D t f(y_n)$,
        ...
        Observe that for $0 \leq x \leq 1$, \[\der{y}{x} = y - x < 1,\] so \[\der[2]{y}{x} = \der{y}{x} - 1 < 0.\] Thus, $y$ is concave downward near $x = 1$, whence the approximation is an overestimate.
    \end{ppart}
    \begin{ppart}
        ...
        At $x = 0$ and $y = 1/2$, we get $C = -1/2$, so $y = x + 1 - \e^x/2$. Evaluating $y$ at $x = 1$ yields $y =  2 - \e/2$.
    \end{ppart}
    \begin{ppart}
...
\begin{solution}
    ...
    Since $y = 1$ at $t = 0$, we get $C = 2$, so $y = -t - 1 + 2\e^t$. Evaluating at $t = 0.6$, we get $y(0.6) = 0.4 + 2\e^{0.6} = 2.044 \todp{3}$.

    \begin{ppart}
        Let $f(t, y) = t + y$ with initial values $t_0 = 0$ and $y_0 = 1$. Take a step size of $\D t = 0.2$. By the Euler method,
...
\begin{solution}
    \begin{ppart}
        We have \[-\frac1{y^2} \der{y}{t} = 1.\] Integrating with respect to $t$, we get \[\int -\frac1{y^2} \d y = \int \d t,\] so \[\frac1y = t + C,\] whence \[y = \frac1{t+C}.\]

        Since $y = 1/2$ when $t = 0$, we have $C = 2$, so \[y = \frac1{t+2}.\]
    \end{ppart}
    \begin{ppart}
        Let $f(y) = -y^2$, $t_0 = 0$, $y_0 = 1/2$ and $\D t = 0.5$. By the improved Euler method,
        \begin{align*}
...
\begin{problem}
    It is given that $\derx{y}{x} = \e^y + x$, and that a particular solution curve passes through the point $(0, 1)$.
    \begin{enumerate}
...
\begin{solution}
    \begin{ppart}
        Let $f(x, y) = \e^y + x$ with initial values $x_0 = 0$ and $y_0 = 1$. Take a step size of $\D x = 0.1$. By the Euler method,
...
\end{problem}
\begin{solution}
    Let $f(y) = y^2 - 2y + 1$. Take a step size of $\D t = 0.5$.
    ...
    \bd{Actual Value.}
    We have \[\frac1{(y-1)^2} \der{y}{t} = 1.\] Integrating with respect to $t$, \[\int \frac1{(y-1)^2} \d y = \int 1 \d t,\] so \[-\frac{1}{y-1} = x + C.\] Thus, \[y = 1 - \frac1{x + C}.\]
    
    Note that $y(0) = 2$, so $C = -1$. Hence, \[y = 1 - \frac1{x-1}.\]
...
\begin{solution}
    \begin{ppart}
        Let $f(x, y) = y - x$ with initial values $x_0 = 0$ and $y_0 = 2$. Take a step size of $\D x = 0.5$. By the Euler method,
        ...
        Note that for $x > 0$, \[\der{y}{x} = y - x > 1,\] hence \[\der[2]{y}{x} = \der{y}{x} - 1 > 0.\] Thus, $y$ is concave upwards, whence the value of $y$ is an underestimate.
    \end{ppart}
    \begin{ppart}
    ...
    \begin{ppart}
        ...
        Since $y = 2$ when $c = 0$, we have $C = 1$, so $y = x + 1 + \e^x$. Thus, at $x = 1$, we have $y = 2 + \e$.
    \end{ppart}
\end{solution}
...
\begin{problem}
    ...
    \begin{enumerate}
        \item Show that \[\der{S}{T} = \frac{100-T-2S}{100-T},\] where $0 < T < 75$.
...
\begin{solution}
    \begin{ppart}
        The concentration of salt in the tank is given by $S/V$. Let $V_i$ and $V_o$ be the volume of liquid entering and leaving the tank in litres, respectively. Then \[\der{V}{T} = \der{V_i}{T} - \der{V_o}{T} = 1 - 2 = -1,\] so $V = 100 - T$, since $V = 100$ initially. Thus, \[\der{S}{T} = 1 \bp{\der{V_i}{T}} - \frac{S}{V} \bp{\der{V_o}{T}} = 1 - \frac{2S}{100 - T} = \frac{100 - T - 2S}{100 - T}.\]
    \end{ppart}
    \begin{ppart}
        ...
        Since $S = 0$ when $T = 0$, we have $C = -1/100$, thus \[S = 100 - T - \frac{(100-T)^2}{100}.\] Hence, when $T = 75$, we have $S = 18.75$, so the amount of salt in the tank after 75 minutes is 18.75 grams.
    \end{ppart}
    \begin{ppart}
        Let $v_i$ and $v_o$ be the volume of liquid entering and leaving the tank in litres, respectively. Since the top tap is still open, we have $\derx{v_i}{t} = 1$. By Torricelli's law, we also have $\derx{v_0}{t} = k\sqrt{v}$. Thus, \[\der{v}{t} =  \der{v_i}{t} - \der{v_o}{t} = 1 - k\sqrt{v}.\] Since the volume of the tank remains constant at 36 litres eventually, we have $1 - k\sqrt{36} = 0$, which gives $k = 1/6$. Thus, \[\der{v}{t} = 1 - \frac{\sqrt{v}}6 = \frac{6-\sqrt{v}}{6} \quad \tand \quad \der{t}{v} = \frac{6}{6 - \sqrt v}.\]

        \begin{psubpart}
            Let $f(v) = 6/(6 - \sqrt v)$ with initial values $v_0 = 25$ and $t_0 = 0$. Take a step size of $\D v = 0.5$. By the Euler method, \[t_1 = t_0 + \D v \, f(v_0) = 3, \qquad t_2 = t_1 + \D v \, f(v_1) = 6.157.\] Hence, when $t \approx 6.16$, the mixture in the tank has risen to 26 litres.
        \end{psubpart}
        \begin{psubpart}
...
\begin{problem}
    A solution to the differential equation $\derx{y}{x} = f(x)$ has $y = y_0$ at $x = x_0$. It is required to estimate the value of $y$ at $x = x_1$ using a numerical method with one step.
    \begin{enumerate}
...
\begin{solution}
    \begin{ppart}
        ...
        \bd{Euler Method.} \[y_1 = y_0 + \D x \, f(x_0)\]

        \bd{Improved Euler Method.} \[y_1 = y_0 + \frac12 \D x \Big[f(x_0) + f(x_1)\Big]\]
    \end{ppart}
    \begin{ppart}
        ...
        The improved Euler method gives the approximation $\D y = \D x \, (1/2) \big[f(x_0) + f(x_1)\big]$. This is represented by the area of the blue-shaded trapezium with base $\D x$ and heights $f(x_0)$ and $f(x_1)$.
    ...
    \begin{ppart}
        ...
        \bd{Improved Euler Method.} \[y_1 = d + \frac12 h \Big[\big(a\big) + \big( a + bh + ch^3\big)\Big] = d + ah + \frac12 bh^2 + \frac12 ch^3.\]

        \bd{Actual Value.}
        We have $\derx{y}{x} = a + bx + cx^2$, so \[y = \int \bp{a + bx + cx^2} \d x = ax + \frac12 bx^2 + \frac13 cx^3 + C.\] Since $y = d$ when $x = 0$, we have $C = d$, thus \[y = d + ax + bx^2/2 + cx^3/3.\] At $x = h$, \[y_1 = d + ah + \frac12 bh^2 + \frac13 ch^3\]
        
        \bd{Error.} \[\text{Error} = \bp{d + ah + \frac12 bh^2 + \frac13 ch^3} - \bp{d + ah + \frac12 bh^2 + \frac12 ch^3} = \frac16 ch^3.\]
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Let $f(x, y) = 1 + y^2 \tan x$ with initial values $x_0 = 1$ and $y_0 = 1$. Take a step size of $\D x = 0.1$, so $x_n = x_0 + n \D x$. By the Euler method, \[y_1 = y_0 + \D x \, f(x_0, y_0) = 1.2557408, \qquad y_2 = y_1 + \D x \, f(x_1, y_1) = 1.6655608.\] Hence, $y(1.2) \approx 1.6656 \todp{4}$.
    \end{ppart}
    \begin{ppart}
