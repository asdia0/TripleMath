...
\end{problem}
\begin{solution}
    \[\der{y}{x} = 6x^2+6x+6 = 6\bp{x + \frac12}^2 + \frac{18}4.\] For all $x \in \RR$, we have $\bp{x + 1/2}^2 \geq 0$. Hence, $\derx{y}{x} > 0$. Thus, $y$ is increasing for all real values of $x$.
\end{solution}

\begin{problem}
    Find, by differentiation, the $x$-coordinates of all the stationary points on the curve $y = x^3/(x+1)^2$ stating, with reasons, the nature of each point.
\end{problem}
\begin{solution}
    Note that \[y' = \frac{(x+1)^2\bp{3x^2} - x^3 \bs{2(x+1)}}{(x+1)^4} = \frac{x^2\bp{x+3}}{(x+1)^3}.\] For stationary points, $y' = 0$. Thus, $x = 0$ or $x = -3$.

    \begin{table}[H]
        \centering
        \begin{tabular}{|c|l|c|c|c|l|c|c|c|}
        \cline{1-1} \cline{3-5} \cline{7-9}
        $x$ &  & $0^-$ & 0 & $0^+$ &  & $(-3)^-$ & $-3$ & $(-3)^+$ \\ \cline{1-1} \cline{3-5} \cline{7-9} 
        $\derx{y}{x}$ &  & +ve & 0 & +ve &  & +ve & 0 & $-$ve \\ \cline{1-1} \cline{3-5} \cline{7-9} 
        \end{tabular}
    \end{table}
...
\begin{solution}
    ...
    Observe that for all $x \in \RR$, $\bp{\cos{x/2} - 1}^2 \geq 0$. Hence, $f'(x) < 0$ for all real values of $x$. Thus, $f(x)$ is strictly decreasing on $\RR$.

    Since $f(x)$ is strictly decreasing, we have $f(x) < f(0) = 0$ for all $x > 0$.
\end{solution}
...
\begin{solution}
    \begin{ppart}
        ...
        Consider the stationary points of $y=ax\sqrt{x+2}$. For stationary points, $\derx{y}{x} =0$. Since $a > 0$, we have $3x+4 = 0$, so $x = -4/3$.
        ...
        By the first derivative test, there is a turning point (minimum point) at $x = -4/3$. Thus, $y = ax\sqrt{x+2}$ has only one turning point.

        Substituting $x = -4/3$ into $y = ax\sqrt{x+2}$, we see that the coordinate of the turning point is $(-4/3, -4a/3 \sqrt{2/3})$.
    \end{ppart}
    \begin{ppart}
...
\begin{problem}
    The function $f$ defined by $f(x) = \ln x - 2\bp{x-1/2}$, where $x > 0$. Find $f'(x)$ and show that the function is decreasing for $x > 1/2$. Hence, show that $2\bp{x - 1/2} - \ln x > \ln 2$ for $x > 1/2$.
\end{problem}
\begin{solution}
    Observe that $f'(x) = 1/x - 2 < 0$ for $x > 1/2$. Thus, $f(x)$ is decreasing for all $x > 1/2$. Since $f(1/2) = -\ln 2$, it follows that for all $x > 1/2$, \[-\ln 2 = f\of{\frac12} > f(x) = \ln x - 2\bp{x - \frac12}.\] Multiplying through by $-1$, we get \[2\bp{x - \frac12} - \ln x > \ln 2.\]
\end{solution}