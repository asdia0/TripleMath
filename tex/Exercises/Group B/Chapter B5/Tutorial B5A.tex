...
\end{problem}
\begin{solution}
    Observe that $f'(x) = 1/x - 2 < 0$ for $x > 1/2$. Thus, $f(x)$ is decreasing for all $x > 1/2$. Since $f(1/2) = -\ln 2$, it follows that for all $x > 1/2$, \[-\ln 2 = f\bp{\frac12} > f(x) = \ln x - 2\bp{x - \frac12}.\] Multiplying through by $-1$, we get \[2\bp{x - \frac12} - \ln x > \ln 2.\]
\end{solution}