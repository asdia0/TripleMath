...
\begin{problem}
    \begin{enumerate}
        ...
        \item Hence, show that $\ln{1+x} \geq 2x/(x+2)$ when $x \geq 0$.
    \end{enumerate}
\end{problem}
\begin{solution}
    ...
    \begin{ppart}
        \[f'(x) = \frac1{1+x} - \frac{4}{(x+2)^2} = \frac{x^2}{(1+x)(x+2)^2}.\] Since $\ln{1+x}$ is defined, we have $1 + x > 0$. We also know that $x^2 \geq 0$ and $(x+2)^2 \geq 0$. Hence, $f'(x) \geq 0$ for all $x$ in the domain of $f$ and is thus never negative.
    \end{ppart}
    \begin{ppart}
        Note that $f(0) = 0$. Since $f'(x) \geq 0$ for all $x \geq 0$, it follows that for all $x \geq 0$, \[\ln{1+x} - \frac{2x}{x+2} = f(x) \geq f(0) = 0,\] which rearranges to give \[\ln{1+x} \geq \frac{2x}{x+2}.\]
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Note that \[\der{y}{x} = 2ax - 2b.\] For stationary points, $\derx{y}{x} = 0$, so $x = b/a$, whence \[y = a\bp{\frac{b}{a}}^2 - 2b\bp{\frac{b}{a}} + c = -\frac{b^2}{a} + c.\] Since $a > 0$, the graph of $y$ is concave upwards. Thus, there is a maximum point at $\bp{\b/a, -b^2/a + c}$.
    \end{ppart}
    \begin{ppart}
        Since the turning point $\bp{\b/a, -b^2/a + c}$ lies on the line $y=x$, we have \[\frac{b}{a} = -\frac{b^2}a + c,\] so \[c = \frac{b + b^2}a = \frac{\bp{b + 1/2}^2 - 1/4}{a}.\] Since $(b + 1/2)^2 \geq 0$, it follows that $c \geq -1/4a$.
    \end{ppart}
    \begin{ppart}
        Since the curve passes through $(0, 6)$, we clearly have $c = 6$. Furthermore, since the curve has a turning point at $(2, 2)$, we know that $b/a = 2$ and $-b^2/a + c = 2$. Solving the two equations simultaneously, we get $a = 1$ and $b = 2$.
    \end{ppart}
\end{solution}
...
\begin{solution}
    \begin{ppart}
        Implicitly differentiating the given equation, \[1 - \der{y}{x} = 2(x+y)\bp{1 + \der{y}{x}}.\] Solving for $\derx{y}{x}$, we obtain \[\der{y}{x} = \frac{1 - (2x + 2y)}{2x + 2y + 1}.\] Thus, \[1 + \der{y}{x} = \frac{2}{2x+2y+1}.\]
    \end{ppart}
    \begin{ppart}
        Implicitly differentiating the above equation, \[\der[2]{y}{x} = -\frac{2\bp{2 + 2 \cdot \derx{y}{x}}}{\bp{2x+2y+1}^2} = -\bp{\frac{2}{2x+2y+1}}^2 \bp{1 + \der{y}{x}} = -\bp{1 + \der{y}{x}}^3.\]
    \end{ppart}
    \begin{ppart}
