...
\begin{problem}
    The equations of the curves $C_{1}$ and $C_{2}$ are given by \[y=\frac{2x+1}{x-3} \quad \tand \quad 3(x-1)^{2}+4y^{2}=12\] respectively. Sketch $C_{1}$ and $C_{2}$ on the same diagram, stating the exact coordinates of any points of intersection with the axes and the equations of any asymptotes.
\end{problem}
...
\begin{problem}
    ...
    \begin{enumerate}
        ...
        \item For $m=1/2$, find the equations of the asymptotes of $C$.
        \item Hence, sketch the curve $C$ for $m=1/2$: indicating clearly the asymptotes, any turning points and axial intercepts.
...
\begin{problem}
    ...
    Another curve $C_{2}$ is defined by the parametric equations \[x=\frac{2}{t^{2}+1},\quad y=3\sqrt{t}\ln t, \quad t > 1.\] Use a non-graphical method to determine the set of possible values of $x$. Sketch the curve $C_{2}$, labelling all axial intercepts and asymptotes (if any) clearly. 
    
    Hence, \bd{without solving the equation}, state the number of real roots to the equation \[9\bp{\frac{2}{t^{2}+1}-1}^{2}=4\bp{3\sqrt{t}\ln t}^{2}+144,\] explaining your reason(s) clearly.
...
\begin{problem}[\chili]
    \begin{enumerate}
        ...
        \end{enumerate}
        \item A hyperbola with equation \[(y-h)^2 - 1 =\frac14(x-k)^2\] has $y=x/2+3/2$ as one of its asymptotes, and the point $(1,3)$ is on the hyperbola. Find the values of $h$ and $k$.
    \end{enumerate}
\end{problem}