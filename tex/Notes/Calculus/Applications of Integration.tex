...
As the number of rectangles approaches $\infty$, the total area of rectangles approaches the actual area under the curve. In other words, \[\area = \lim_{n \to 0} \sum_{k = 1}^n f(a + k \D x) \D x = \lim_{\D x \to 0} \sum_{k = 1}^n f(a + k \D x) \D x.\] In the limit, the Riemann sum becomes the \vocab{Riemann integral}, which is conventionally written as the definite integral \[\int_a^b f(x) \d x.\]
