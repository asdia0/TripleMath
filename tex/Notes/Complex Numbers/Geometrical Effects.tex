    ...
    \begin{figure}[H]\tikzsetnextfilename{345}
        ...
        \begin{tikzpicture}[trim axis left, trim axis right]
            \begin{axis}[
                ...
                \draw[thick] (C) circle[radius=5];
                
                \addlegendimage{no markers, thick}
...
When sketching non-standard loci, one useful technique is to write $z$ in either polar or Cartesian form.

\begin{sample}
    Illustrate, in separate Argand diagrams, the set of points $z$ for which $\Re{z^2} < 0$.
\end{sample}
\begin{sampans}
    We write $z$ in polar form. Let $z = r(\cos \t + \i\sin\t)$ for $0 \leq \t < 2\pi$. Then $\Re(z^2) < 0$ gives $r^2\cos 2\t < 0$, which implies $\cos 2\t < 0$, so \[\t \in \bp{\frac14\pi, \frac34 \pi} \cup \bp{\frac54 \pi, \frac74 \pi}.\] 

    \begin{figure}[H]
        \centering
        \includegraphics{figures/90.pdf}
        \caption{}
    \end{figure}
\end{sampans}
...
\begin{figure}[H]\tikzsetnextfilename{348}
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
        \begin{axis}[
            ...
            \draw[dashed, thick] (C) circle[radius=5];
            
            \addlegendimage{no markers, black, dashed, thick}
...
\begin{figure}[H]\tikzsetnextfilename{349}
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
        ...
        \begin{axis}[
            ...
            \pgfonlayer{pre main}
                \fill[pattern=north east lines, pattern color=black!30] (C) circle[radius=5];
            \endpgfonlayer
            ...
            \draw[->] (6.5, 9) -- (4, 7);
            \node[anchor=south west] at (6.5, 9) {locus of $z$};
        \end{axis}
    \end{tikzpicture}
    \caption{}
\end{figure}
