...
\begin{example}[Binomial Model]
    Recall the example of tossing a fair coin 6 times. This experiment fits a binomial model:
    \begin{itemize}
...
\begin{definition}
    Let $X$ be the number of occurrences of a particular event over an interval of time (or space) $t$. Let $\l$ be the mean rate of occurrence per unit time. Then $X$ is said to follow a \vocab{Poisson distribution} with parameter $\l t$, written as \[X \sim \Po{\l t}.\]
\end{definition}
\begin{remark}
