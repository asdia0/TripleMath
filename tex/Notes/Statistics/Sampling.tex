...
\begin{theorem}[Central Limit Theorem]
    If $X$ does not follow a normal distribution, with $\E{X} = \m$ and $\Var{X} = \s^2$, and $n$ is large, then \[\ol{X} \sim \Normal{\m}{\frac{\s^2}{n}} \text{ approximately}.\]
\end{theorem}
...
Typically, we consider $n \geq 30$ to be large. However, this is just a guideline. Depending on the context, the distribution of the sample mean can still be approximated using a normal distribution with a smaller sample size.
...
\begin{example}[Estimator]
    Suppose we take a random sample of 10 adult men in Singapore and measure their heights (in metres): \[1.58, \quad 1.61, \quad 1.62, \quad 1.62, \quad 1.65, \quad 1.67, \quad 1.67, \quad 1.74, \quad 1.78, \quad 1.83.\] Suppose our quantity of interest is the mean height of all men in Singapore, $\m$. Using our sample data, we can estimate $\m$. For instance, a natural estimate is the sample mean $\ol{x}$, which we can compute to be $1.677$ m. We say that $1.677$ m is the estimate of $\m$ provided by the ``sample mean'' estimator. Another way of estimating $\m$ would be to use the sample median, giving an estimate of $1.66$ m. We could even estimate $\m$ with the average of the sample mean and sample median, which would give an estimate of $(1.677 + 1.66)/2 = 1.6685$ m.
\end{example}

\subsection{Unbiased Estimators}

As illustrated by the above example, there are many estimators we can use to estimate a parameter. We therefore need a principled way to assess the quality or usefulness of different estimators. One commonly used criterion is unbiasedness: the expected value of the estimator should equal the true value of the parameter being estimated.
...
\begin{proof}
   ...
   Noting that $\E{X^2} = \E{X}^2 + \Var{X} = \m^2 + \s^2$ and  $\E{\ol{X}^2} = \E{\ol{X}}^2 + \Var{\ol{x}} = \m^2 +\s^2/n$, we see that
    \begin{align*}
...
\begin{definition}
    A \vocab{population proportion} $p$ is a parameter that describes the percentage of individuals in a population that exhibit a certain property that we wish to investigate. Mathematically, $p = X/N$, where $X$ is the number of ``successes'' in the population (individuals who exhibit the property), and $N$ is the population size. The \vocab{sample proportion} $P_S = X_S/N$ is defined similarly, where $X_S$ is the number of ``successes'' in the sample.
\end{definition}

\begin{example}[Population Proportion]
    Suppose we take a random sample of 10 adult men in Singapore and measure their heights (in metres): \[1.58, \quad 1.61, \quad 1.62, \quad 1.62, \quad 1.65, \quad 1.67, \quad 1.67, \quad 1.74, \quad 1.78, \quad 1.83.\] Suppose we now wish to find the population proportion $p$ of adult men in Singapore taller than 1.70 m. The observed sample proportion, which we denote $\hat{p}$, can be calculated from the data as $\hat{p} = 3/10$.
\end{example}
...
\end{proposition}
\begin{proof}
    Consider a population in which the proportion of ``success'' is $p$. If a random variable of size $n$ is taken from this population, and $X_S$ is the random variable denoting the number of ``successes'' in this sample, then $X_S \sim \Binom{n}{p}$. The expected value of $P_S$ is thus \[\E{P_S} = \E{\frac{X_S}{n}} = \frac{\E{X_S}}{n} = \frac{np}{n} = p,\] so $P_S$ is an unbiased estimator for $p$.
\end{proof}

\begin{proposition}
    For large $n$, \[P_S \sim \Normal{p}{\frac{p(1-p)}{n}} \text{ approximately}.\]
\end{proposition}
\begin{proof}
    Again, write $P_S = X_S/n$, where $X_S \sim \Binom{n}{p}$. Then \[\Var{P_S} = \Var{\frac{X_S}{n}} = \frac{\Var{X_S}}{n^2} = \frac{np(1-p)}{n^2} = \frac{p(1-p)}{n}.\] Applying the Central Limit Theorem gives the desired result.
\end{proof}

\begin{definition}
    The distribution of $P_S$ is known as the \vocab{sampling distribution of the sample proportion} and its standard deviation, $\sqrt{p(1-p)/n}$, is known as the \vocab{standard error of proportion}.
\end{definition}