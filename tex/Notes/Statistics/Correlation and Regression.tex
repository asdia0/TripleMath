...
Correlation is concerned with quantifying the (linear) \emph{relationship} between two variables. Informally, it allows us to tell how strongly two variables move with each other. For instance, suppose we measure the heights and weights of a group of people. Intuitively, we would expect taller people to be heavier; there is a positive correlation between height and weight.

Regression, on the other hand, is concerned with quantifying how a change in one variable will affect the other variable. That is, regression predicts the value of a variable based on the value of the other variable. For example, if a person weighs 70 kg, a regression model may predict this person to be 1.60 m tall.
...
\begin{itemize}
    \item \bd{Positive linear relationship.} As $x$ increases, $y$ increases.
    \item \bd{Negative linear relationship.} As $x$ increases, $y$ decreases.
    \item \bd{Curvilinear relationship.} The points seem to lie on a curve.
    \item \bd{No clear relationship.} The points seem to be randomly scattered.
\end{itemize}
...
We can manipulate $r$ to get rid of $\ol{x}$ and $\ol{y}$: \[r = \frac{\sum xy - (1/n) \sum x \sum y}{\sqrt{\sum x^2 - (1/n) \bp{\sum x}^2} \sqrt{\sum y^2 - (1/n) \bp{\sum y}^2}},\] where $n$ is the number of ordered pairs in the sample.

\subsection{Characteristic of $r$}

The product moment correlation coefficient $r$ can only take on values between $-1$ and 1. A summary of the value(s) of $r$ and the associated linear correlation is given below.
...
To understand why this is the case, consider the sign of $r$. Looking at the definition of $r$, we see that \[r > 0 \iff \sum \bp{x - \ol{x}}\bp{y - \ol{y}} > 0 \quad \tand \quad r < 0 \iff \sum \bp{x - \ol{x}}\bp{y - \ol{y}} < 0.\] Consider then the following figure:
...
By considering the signs of $x - \ol{x}$ and $y - \ol{y}$, we see that $\bp{x - \ol{x}}\bp{y - \ol{y}}$ is positive when $(x, y)$ lies in quadrants $A$ or $C$, and negative when $(x, y)$ lies in quadrants $B$ or $D$.
...
A scatter diagram should always be used in the interpretation of correlation, as it not only shows the pattern trend between the variables, but it also reveals the existence of any outliers which may have affected the value of $r$. For instance, the following two sets of bivariate data differ only by one data point, yet they have drastically different product moment correlation coefficients:

\noindent\begin{minipage}{0.5 \textwidth}
    \begin{figure}[H]\tikzsetnextfilename{483}
    \centering
    \begin{tikzpicture}[trim axis left, scale=0.7]
        \begin{axis}[
...
In statistical studies, when it is observed that a significant linear correlation exists between two variables of study, best-fit lines or regression lines are often obtained in order to make predictions or estimations relating to $x$ and/or $y$. For bivariate data, we can draw either the regression line of $y$ on $x$, or that of $x$ on $y$.
...
\begin{definition}
    The \vocab{vertical residual}, denoted $v_i$, is the deviation between the actual and predicted $y$-values. Mathematically, $v_i = y_i - \bp{a + bx_i}$ for some constants $a$ and $b$.
\end{definition}
...
\begin{figure}[H]\tikzsetnextfilename{485}
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
        ...
        \addplot[very thick] {1.4 * x} node[pos=0.65, above left] {$y = ax+b$};        
...
\begin{definition}
    The \vocab{horizontal residual}, denoted $h_i$, is the deviation between the actual and predicted $x$-values. Mathematically, $h_i = y_i - \bp{c + d x_i}$ for some constants $c$ and $d$.
\end{definition}
...
\begin{figure}[H]\tikzsetnextfilename{486}
    ...
    \begin{tikzpicture}[trim axis left, trim axis right]
        ...
        \addplot[very thick] {x * 2.714 - 3.2857} node[pos=0.55, above left] {$x = c + dy$};
...
\begin{itemize}
    \item \bd{Appropriateness of the regression line used.} The correct regression line should be used for the estimate to be reliable.
    \item \bd{Strength of linear correlation.} $\abs{r}$ should be close to 1 for the estimate to be reliable.
    \item \bd{Interpolation or extrapolation.} Interpolation is likely to give a more reliable estimate than extrapolation.
\end{itemize}
