...
\begin{example}[Samples and Populations]
...
Here, a lower-case `$n$` represents the sample size. We use the uppercase `$N$' to represent the population size. It is essential to make it clear when we are referring to the mean of a sample or when we are referring to the mean of a population. To do so, statisticians use different symbols ($\ol{x}$ and $\m$): \[\text{Sample mean} = \ol{x} = \frac1n \sum x, \quad \text{Population mean} = \m = \frac1N \sum x.\]
...
\begin{example}[Mean]
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|c|c|}
        \hline
        \bd{Test score, $x$} & \bd{Frequency, $f$} \\ \hline\hline
        12 & 2 \\ \hline
        13 & 3 \\ \hline
        15 & 6 \\ \hline
        16 & 5 \\ \hline
    ...
    Then, the mean test score can be calculated as \[\ol{x} = \frac{\sum xf}{\sum f} = \frac{(12)(2) + (13)(3) + (15)(6) + (16)(5) + (17)(4)}{2 + 3 + 6 + 5 + 4} = 15.05.\]

    Suppose now that another student in the class obtained a `1' on the test. The new mean can be calculated as \[\ol{x} = \frac{\sum xf}{\sum f} = \frac{(1)(1) + (12)(2) + (13)(3) + (15)(6) + (16)(5) + (17)(4)}{1 + 2 + 3 + 6 + 5 + 4} = 14.14,\] which is much less than the previous mean of $15.05$. 
\end{example}
...
Generally, for $n$ values $x_1$, $x_2$, $\dots$, $x_n$ arranged in ascending order, \[\text{Median} = \begin{cases}
    x_{(n+1)/2}, & \text{$n$ odd},\\
    (1/2) \bp{x_{n/2} + x_{n/2 + 1}}, & \text{$n$ even}
\end{cases}\]

Unlike the mean, the median is not sensitive to outliers.

\begin{example}[Median]
    Consider again the example of 20 students. The set of data in ascending order is \[12, \; 12, \; 13, \; 13, \; 13, \; 15, \; 15, \; 15, \; 15, \; 15, \; 15, \; 16, \; 16, \; 16, \; 16, \; 16, \; 17, \; 17, \; 17, \; 17.\] The median is hence the average of the two middle values, i.e. $(15 + 15)/2 = 15$.

    When we add in the outlier, the set of data in ascending order becomes \[1, \; 12, \; 12, \; 13, \; 13, \; 13, \; 15, \; 15, \; 15, \; 15, \; 15, \; 15, \; 16, \; 16, \; 16, \; 16, \; 16, \; 17, \; 17, \; 17, \; 17.\] The median is now the 11th value, $15$. There is no change to the median.
\end{example}
...
\subsection{Bonus: Minimizers of \texorpdfstring{$L^p$}{Lp}-norms}
...
\end{proposition}
\begin{proof}
    By definition, \[\norm{\vec x - \vec c}_2 = \bp{\sum_{i = 1}^n (x_i - c)^2}^{1/2}.\] Differentiating this with respect to $c$, \[\der{}{c} \norm{\vec x - \vec c}_2 = -\bp{\sum_{i = 1}^n (x_i - c)^2}^{-1/2} \sum_{i = 1}^n (x_i - c).\] For stationary points, $(\derx{}{c}) \norm{\vec x - \vec c}_2 = 0$, so $\sum_{i = 1}^n (x_i - c) = 0$. This implies that \[c = \frac1n \sum_{i = 1}^n x_i,\] which is exactly the definition of the mean. It is an exercise for the reader to show that this stationary point is a minimum.
\end{proof}
...
This can be represented mathematically with the formula \[\text{Mean distance} = \frac1n \sum \abs{x - \ol x}.\] Unfortunately, a formula involving the modulus sign is awkward to handle algebraically. This can be avoided by squaring each of the quantities $x - \ol x$, leading to the expression \[\frac1n \sum \bp{x - \ol x}^2\] as a measure of spread. We call this quantity the \vocab{variance} of the distribution.

If the data values $x_1$, $\dots$, $x_n$ have units associated with them, then the variance will be measured in units$^2$. This can be avoided by taking the positive square root of the variance. The positive square root of the variance is known as the \vocab{standard deviation}, and it always has the same units as the original data values, i.e. \[\text{Standard deviation} = \sqrt{\frac1n \sum \bp{x - \ol x}^2}.\]
...
\begin{proposition}
    \[\text{Variance} = \frac1n \sum x^2 - \ol{x}^2.\]
\end{proposition}
\begin{proof}
    We have \[\text{Variance} = \frac1n \sum \bp{x - \ol x}^2 = \frac1n \sum \bp{x^2 - 2x \ol{x} + \ol{x}^2} = \frac1n \sum x^2 - \frac{2\ol{x} \sum x}{n} + \frac{\ol{x}^2 \sum 1}{n}.\] Observe that $\sum x/n = \ol x$ and $\sum 1 = n$. Thus, \[\text{Variance} = \frac1n \sum x^2 - 2\ol{x}^2 + \ol{x}^2 = \frac1n \sum x^2 - \ol{x}^2.\]
\end{proof}