...
We call a sentence such as ``$x$ is even'' that depends on the value of $x$ a ``statement about $x$''. We can denote this statement more compactly as $P(x)$. For instance, $P(5)$ is the statement ``5 is even'', while $P(72)$ is the statement ``72 is even'', and so forth.

We now introduce two important symbols, namely the ``for all'' symbol ($\forall$) and the ``there exists'' symbol ($\exists$)

\begin{definition}
    Let $P(x)$ be a statement about $x$, where $x$ is a member of some set $S$. Then the notation \[(\forall x \in S) \, P(x)\] means that $P(x)$ is true for every $x$ in the set $S$. The notation \[(\exists x \in S) \, P(x)\] means that there exists at least one element of $x$ of $S$ for which $P(x)$ is true.
\end{definition}
...
To prove this statement, we will need the following definitions:
