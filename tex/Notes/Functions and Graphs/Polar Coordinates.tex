...
Recall that in the Cartesian coordinate system, each point has a unique representation. This is not the case in the polar coordinate system. For example, the point $(1, 5\pi/4)$ could also be written as $(1, 13\pi/4)$ or as $(-1, \pi/4)$. In general, because a complete anti-clockwise rotation is given by the angle $2\pi$, the point $(r, \t)$ can also be represented by $(r, \t + 2n\pi)$ and $(-r, (2n + 1)\pi)$, where $n$ is any integer.
...
Suppose the point $P$ has Cartesian coordinates $(x, y)$ and polar coordinates $(r, \t)$. From Figure~\ref{fig:333}, we have \[\cos \t = \frac{x}{r} \quad \tand \quad \sin \t = \frac{y}{r}.\] Thus, \[x = r\cos \t \quad \tand \quad y = r\sin\t.\] Note that while the above were deduced from the case where $r > 0$ and $0 < \t < \frac\pi2$, these equations are valid for all values of $r$ and $\t$.
...
\end{proposition}
\begin{proof}
    Recall that \[x = r\cos \t \quad \tand \quad y = r\sin\t.\] Differentiating both equations with respect to $\t$, we see that \[\der{x}{\t} = r' \cos \t - r\sin\t \quad \tand \quad \der{y}{\t} = r' \sin \t + r\cos\t.\] Thus, \[\der{y}{x} = \frac{\derx{y}{\t}}{\derx{x}{\t}} = \frac{r' \sin \t + r \cos \t}{r' \cos \t - r \sin \t}.\]
\end{proof}
\begin{remark}
