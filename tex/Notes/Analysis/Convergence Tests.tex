...
\begin{proof}
    ($\implies$) Suppose $\bc{u_n}$ converges to $L$. By the definition of the limit of a sequence, for all $\ve > 0$, there exists some natural number $N$ such that for all integers $n \geq N$, we have $\abs{u_n - L} < \ve$. Equivalently, for all $n \geq N$, we have $u_N < L + \ve$. Since the sequence is increasing, we also have for all $1 \leq i < N$ that $u_i \leq u_N < L + \ve$. Thus, $u_j < L + \ve$ for all positive integers $j$, whence the sequence is bounded above by $L + \ve$.
    ...
    Fix $\ve > 0$. Since $L$ is the lowest upper bound, there must exist some index $N$ such that $u_N > L - \ve$. Since $\bc{u_n}$ is increasing, for all $n \geq N$, we have $u_n \geq u_N > L - \ve$. Further, we must have $u_n < L$ by definition of the supremum. Putting everything together, we have for all $n \geq N$ that $\abs{u_n - L} < \ve$, whence $L$ is the limit of $\bc{u_n}$ and the sequence converges.
\end{proof}

\section{Tests for Series}\label{sec:convergence-series}

In this section, let $\bc{u_n}$ be a sequence and $S_n$ its $n$th partial sum.
...
\begin{proposition}[Comparison Tests]
...
\end{proposition}
\begin{proof}
    Observe that $0 \leq u_n + \abs{u_n} \leq 2 \abs{u_n}$ for all $n$. Define \[T_N = \sum_{n = 1}^N (u_n + \abs{u_n}).\] Since each summand $u_n + \abs{u_n}$ is positive and \[0 \leq T_N \leq \sum_{n = 1}^N 2\abs{u_n} \leq 2\sum_{n=1}^\infty \abs{u_n},\] we have that $T_N$ is monotone and bounded, so $T_n$ converges by the monotone convergence theorem. Since \[S_N = \sum_{n = 1}^N u_n = \sum_{n = 1}^N \bs{\bp{u_n + \abs{u_n}} - \abs{u_n}} = T_N - \sum_{n = 1}^N \abs{u_n}\] is the difference of convergent series, it follows that $S_n$ also converges.
\end{proof}
...
\begin{example}[Direct Comparison Test]
...
\begin{example}[Limit Comparison Test]
