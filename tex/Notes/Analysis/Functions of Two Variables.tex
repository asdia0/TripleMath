...
Recall that for a function $f$ of one variable $x$, we defined the derivative function as \[f'(x) = \lim_{\D x \to 0} \frac{f(x + \D x) - f(x)}{\D x}.\] The usual notations are \[\der{y}{x} \quad \tor \der{f}{x}\] if $y = f(x)$.
...
If we want to approximate $f(x)$ for $x$ near $0$, we can truncate the Maclaurin series of $f(x)$. For instance, the linear approximation to $x$ is \[f(x) \approx f(0) + f'(0) x,\] which is the tangent line at $x = 0$. If we want better approximations, we can simply take more terms.
