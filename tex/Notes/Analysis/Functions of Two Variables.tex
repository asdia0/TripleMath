...
For the rest of this chapter, we study scalar-valued functions of two variables, which has the general form $z = f(x, y)$. We will see how the ideas from univariate functions can be extended to two variable functions and how concepts of vectors can be useful in studying these functions.
...
The domain of a univariate function $g(x)$ is a subset of the real number line, i.e. $\dom g \subseteq \RR$. Generalizing this to functions of two variables, the domain of $f$ is a subset of the $xy$-plane, denoted $\RR \crossp \RR$ or $\RR ^2$. Mathematically, $\dom f \subseteq \RR^2$.
...
For univariate functions $g(x)$, we defined to be the collection of all points $(x, y)$ in the $xy$-plane such that the values $x$ and $y$ satisfy $y = g(x)$ (Definition~\ref{def:function-graph}). We can extend this to functions of two variables.
...
Effectively, we are fixing the value of $z$ to be some constant $c$, and drawing the 2D graph of $f(x, y) = c$.

\begin{definition}
    \vocab{Vertical traces} are the resulting curves when we intersect the surface $z = f(x, y)$ with vertical planes.
\end{definition}

Effectively, we are fixing the values of $x$ and/or $y$ and drawing the resulting 2D graph.
...
\begin{example}[Graph of $z = f(x, y)$]
    Let $f(x, y) = \ln{x^2 + y^2}$. Consider the horizontal traces of $z = f(x, y)$. Setting $z = c$, we get $\ln{x^2 + y^2} = c$, which rearranges more familiarly as $x^2 + y^2 = \e^c$. That is, the horizontal trace of $z = f(x, y)$ at $z = c$ corresponds to a circle centred at the origin with radius $\e^c$. This allows us to visualize the graph of $z = f(x, y)$, which is plotted below.
    ...
    \begin{figure}[H]\tikzsetnextfilename{378}
        ...
        \begin{tikzpicture}[trim axis left, trim axis right]
            \begin{axis}[
                ...
                \addplot3[surf,shader=faceted interp, colormap/blackwhite]{ln(x^2 + y^2)};
            \end{axis}
        \end{tikzpicture}
        \caption{}
    \end{figure}
...
Examples of cylinders include the graphs of $x^2 + z^2 = 1$ and $z = y^2$, which are plotted below.

\begin{figure}[H]
    \centering
    \noindent\begin{minipage}[t]{.5\textwidth}\tikzsetnextfilename{379}
      \centering
      \begin{tikzpicture}[trim axis left, trim axis right, scale=0.7]
        \begin{axis}[
            ...
            \addplot3[surf,shader=faceted interp, colormap/blackwhite,y domain=0:2*pi] ({cos(deg(y))},{x},{sin(deg(y))});
        \end{axis}
        \end{tikzpicture}
    ...
    \begin{minipage}[t]{.5\textwidth}\tikzsetnextfilename{380}
      ...
      \begin{tikzpicture}[trim axis left, trim axis right, scale=0.7]
        \begin{axis}[
            ...
            \addplot3[surf,shader=faceted interp, colormap/blackwhite] {y^2};
        \end{axis}
        \end{tikzpicture}
...
Another common surface is a quadric surface, which is a 3D generalization of 2D conic sections. A conic section in 2D has the general form $Ax^2 + Bxy + Cy^2 + Dx + Ey + F = 0$. We can generalize this into 3D to get a quadric surface.

\begin{definition}
    A \vocab{quadric surface} has the form \[Ax^2 + By^2 + Cz^2 + Dxy + Eyz + Fzx + Gx + Hy + Iz + J = 0,\] where $A, B, \dots, J \in \RR$ and at least one of $A$, $B$ and $C$ is non-zero.
\end{definition}
...
\begin{figure}[H]\tikzsetnextfilename{381}
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{An ellipsoid with $a = 1$, $y = 2$ and $z = 1$.}
\end{figure}

When $a = b = c = r$, we get the equation $x^2 + y^2 + z^2 = r^2$, which represents a sphere centred at the origin with radius $r$. Observe the similarity between the equation of a circle ($x^2 + y^2 = r^2$) and the equation of a sphere.

\section{Partial Derivatives}

In Definition~\ref{def:derivative}, we defined the derivative function of a univariate function $f(x)$ as \[f'(x) = \lim_{\D x \to 0} \frac{f(x + \D x) - f(x)}{\D x}.\] The usual notations are $\derx{f}{x}$, or $\derx{y}{x}$ if $y = f(x)$.

The notation $\derx{y}{x}$ gives some insight into how derivatives are derived. We can view
\begin{itemize}
    \item ``$\d x$'' as a small change in $x$, and
    \item ``$\d y$'' as the change in $y$ due to the small change in $x$.
\end{itemize}

We can extend this idea to functions of two variables $z = f(x, y)$. There are now two variables that can change the value of $f$. We can choose to vary either $x$ or $y$ slightly ($\D x$ and $\D y$ respectively) and see how $f$ changes ($\D f$). This gives us some notion of a derivative. However, because we are only varying one independent variable at a time, we are only differentiating the function $f(x,y)$ ``partially''. We hence call these derivatives the partial derivatives of $f$.
...
\begin{definition}
    ...
    \end{align*}
    In Liebniz notation, \[f_x(x, y) = \pder{f}{x} \quad \tand \quad f_y(x, y) = \pder{f}{y}.\]
\end{definition}
...
\end{sample}
\begin{sampans}
    To partially differentiate it with respect to $x$, we treat $y$ as a constant. Using the chain rule, \[f_x (x, y) = -\sin{xy + y^2} \, \pder{}{x} \bs{xy + y^2}.\] Since $y$ is a constant, \[\pder{}{x} (xy) = y \quad \tand \quad \pder{}{x} y^2 = 0.\] Hence, $f_x (x, y) = -y\sin{xy + y^2}$.
\end{sampans}
...
The curve $C_1$ is the graph of the function $g(x) = f(x, b)$, which is the intersection curve of the surface and the vertical plane $y = b$. The gradient of its tangent $T_1$ at $P$ is $g'(x) = f_x(a, b)$.

Similarly, the curve $C_2$ is the graph of the function $h(y) = f(a, y)$, which is the intersection curve of the surface and the vertical plane $x = a$. The gradient of its tangent $T_2$ at $P$ is $h'(y) = f_y(a, b)$.

We can hence visualize partial derivatives at the point $P$ on $S$ as gradients to the tangent lines $T_1$ and $T_2$ at that point.
...
Similar to second-order derivatives for univariate functions, we can also consider the partial derivatives of partial derivatives: \[(f_x)_x, \qquad (f_x)_y, \qquad (f_y)_x, \qquad (f_y)_y.\] If $z = f(x, y)$, we use the following notation for the second partial derivatives:
\[
...
\begin{example}[Second Partial Derivatives]
    Consider the function $f(x, y) = xy^2 + x^3 + \ln y$. Its partial derivatives are \[f_x = y^2 + 3x^2 \quad \tand \quad f_y = 2xy + \frac1y,\] while its second partial derivatives are \[f_{xx} = 6x, \quad f_{xy} = 2y, \quad f_{yx} = 2y, \quad f_{yy} = 2x  - \frac1{y^2}.\]
\end{example}
...
Recall that for a univariate function $y = f(x)$, where the variable $x$ is a function of $t$, i.e. $x = g(t)$, the chain rule states that \[\der{y}{t} = \der{y}{x} \der{x}{t}.\] We can generalize this result to multivariate functions using partial derivatives:

\begin{proposition}[Chain Rule for Functions of Two Variables]\label{prop:MCR}
    Consider the function $f(x, y)$, where $x$ and $y$ are functions of $t$. Then \[\der{f}{t} = \pder{f}{x} \der{x}{t} + \pder{f}{y} \der{y}{t}.\]
\end{proposition}
\begin{proof}
    We begin by returning to the definition of a partial derivative.
    ...
    \end{align*}
    Partially differentiating (2) with respect to $x$, we get \[f_x(x, y + \D y) = f_x(x, y) + \D y f_{yx}(x, y).\] Substituting this into (3) yields
    \begin{align*}
        f(x + \D x, y + \D y) &= f(x, y) + \D y f_y(x,y) + \D x \bs{f_x(x, y) + \D y f_{yx}(x, y)}\\
        &= f(x, y) + \D y f_y(x,y) + \D x f_x(x, y) + \D x\D y f_{yx}(x, y). \tag{4}
    \end{align*}
    Since $\D x$ and $\D y$ are both infinitesimally small, the quantity $\D x \D y$ is negligible and can be disregarded. The total change in $f$ is hence given by
    \begin{align*}
        \D f &= f(x + \D x, y + \D y) - f(x, y)\\
        &= \D x f_x(x, y) + \D y f_y(x,y).
    \end{align*}
    Dividing throughout by $\D t$ and writing $f_x$, $f_y$ in Liebniz notation, we have \[\frac{\D f}{\D t} = \pder{f}{x} \frac{\D x}{\D t} + \pder{f}{y} \frac{\D y}{\D t}.\] In the limit, we have \[\frac{\D f}{\D t} \to \der{f}{t}, \quad \frac{\D x}{\D t} \to \der{x}{t}, \quad \frac{\D y}{\D t} \to \der{y}{t}.\] Thus, \[\der{f}{t} = \pder{f}{x} \der{x}{t} + \pder{f}{y} \der{y}{t}.\]
\end{proof}

If we had applied (2) before (1) on $f(x + \D x, y + \D y)$, we would have got \[f(x + \D x, y + \D y) = f(x, y) + \D y f_y(x,y) + \D x f_x(x, y) + \D x\D y f_{xy}(x, y).\] However, by Clairaut's theorem, we know $f_{xy} = f_{yx}$, so we would still have ended up with (4).

More generally, we have the following theorem.

\begin{theorem}[Multivariable Chain Rule]
    Given a multivariate function $f(x_1, \dots, x_n)$, where $x_i$ are functions of $t$, we have \[\der{f}{t} = \sum_{i = 1}^n \pder{f}{x_i} \der{x_i}{t}.\]
\end{theorem}
...
\begin{proposition}
    The directional derivative can be computed with the dot product \[D_{\vec u} f(x, y) = \nabla f \cdot \vec u = u_x f_x(x, y) + u_y f_y (x, y).\]
\end{proposition}
\begin{proof}
    In the proof of Proposition~\ref{prop:MCR}, we derived the equation \[f(x + \D x, y + \D y) - f(x, y) = \D x f_x(x, y) + \D y f_y(x, y),\] where $\D x$ and $\D y$ are infinitesimally small. If we take $\cveciix{\D x}{\D y}$ to be in the same direction as $\cveciix{u_x}{u_y}$, i.e. \[\cvecii{\D x}{\D y} = \lim_{h \to 0} h \cvecii{u_x}{u_y},\] then we have \[\lim_{h \to 0} f(x + h u_x, y + h u_y) - f(x, y) = \lim_{h \to 0} \bs{h u_x f_x(x, y) + h u_y f_y(x, y)}.\] Dividing both sides by $h$, \[\lim_{h \to 0} \frac{f(x + h u_x, y + h u_y) - f(x, y)}{h} = u_x f_x(x, y) + u_y f_y(x, y),\] which was what we wanted.
\end{proof}
...
Using the multivariable chain rule, we can formalize the action of differentiating an equation implicitly.
...
\end{proposition}
\begin{proof}
    Differentiating $f(x, y)$ totally with respect to $x$, we have by the chain rule that \[\der{f}{x} = \pder{f}{x} \der{x}{x} + \pder{f}{y} \der{y}{x} = \pder{f}{x} + \pder{f}{y} \der{y}{x}.\] But $f(x, y) = 0$ is a constant function, so $\derx{f}{x} = 0$. Rearranging \[0 =\pder{f}{x} + \pder{f}{y} \der{y}{x},\] we get \[\der{y}{x} = -\frac{f_x(x, y)}{f_y(x,  y)}\] as desired.
\end{proof}
...
\end{proposition}
The proof is identical to the univariate case.
...
If we want to approximate $f(x)$ for $x$ near $0$, we can truncate the Maclaurin series of $f(x)$. For instance, the linear approximation to $x$ is $y = f(0) + f'(0) \, x$, which represents the tangent line to $y = f(x)$ at $x = 0$. If we want better approximations, we can simply take more terms.
...
\begin{proposition}[Linear Approximation]
    The linear approximation to $f(x, y)$ at $(a, b)$ is given by $T(x, y) = f(a, b) + f_x(a,b) (x-a) + f_y(a, b) (y-b)$.
\end{proposition}
\begin{proof}
    Let the linear approximation to $f(x, y)$ at $(a, b)$ be $T(x, y)$. Since $T(x, y)$ is linear, it has the general form $T(x, y) = C_1 + C_2 (x-a) + C_3 (y-b)$, where $C_1$, $C_2$ and $C_3$ are constants to be determined.

    From the first condition, we have $C_1 = T(a,b) = f(a, b)$. From the second condition, we have $C_2 = T_x(a,b) = f_x(a, b)$ and $C_3 = T_y(a,b) = f_y(a, b)$.

    Thus, $T(x, y) = f(a, b) + f_x(a,b) (x-a) + f_y(a, b) (y-b)$.
\end{proof}
...
\begin{corollary}
    The normal vector to $z = f(x, y)$ at $(a, b)$ is \[\cveciii{f_x(a,b)}{f_y(a,b)}{-1}.\]
\end{corollary}
...
\begin{proof}
    To get the quadratic approximation $Q(x, y)$, we add terms of order 2 to the linear approximation $T(x, y)$. That is, $Q(x, y) =  T(x,y) + C_1 (x-a)^2 + C_2 (x-a)(y-b) + C_3 (y-b)^2$, where $C_1$, $C_2$ and $C_3$ are constants. We can determine them by equating the second partial derivatives of $Q(x, y)$ with that of $f(x, y)$'s:
    \begin{align*}
...
For functions of two variables, the stationary points we often come across are maxima, minima and saddle points (so named because it looks like a horse saddle).

\begin{figure}[H]
    \captionsetup{singlelinecheck=false, aboveskip=7pt, belowskip=0pt,justification=centering}
    \setcapindent{0pt}
    \centering
    \noindent\begin{minipage}[t]{.33\textwidth}\tikzsetnextfilename{383}
      \centering
      \begin{tikzpicture}[scale=0.5,trim axis left, trim axis right]
        \begin{axis}[
            hide axis,
            samples=20,
            ...
            \addplot3[surf,shader=faceted interp, colormap/blackwhite,opacity=0.7,domain=-1:1,y domain=-1:1,empty legend] {x^2 + y^2};
        \end{axis}
        \end{tikzpicture}
      \captionof{figure}{\\A minimum point.}
    \end{minipage}%
    \begin{minipage}[t]{.33\textwidth}\tikzsetnextfilename{384}
      \centering
      \begin{tikzpicture}[scale=0.5,trim axis left, trim axis right]
        \begin{axis}[
            hide axis,
        ...
        \end{axis}
        \end{tikzpicture}
        \captionof{figure}{\\A maximum point.}
    \end{minipage}%
    \begin{minipage}[t]{.33\textwidth}\tikzsetnextfilename{385}
      \centering
        \begin{tikzpicture}[scale=0.5,trim axis left, trim axis right]
            \begin{axis}[
                hide axis,
            ...
            \end{axis}
        \end{tikzpicture}
        \captionof{figure}{\\A saddle point.}
    \end{minipage}
\end{figure}

\subsection{Global and Local Extrema}

In optimization, we may distinguish between a \vocab{local extremum} from a \vocab{global extremum}. Informally, a global maximum/minimum is the largest/smallest value of the function across its domain, while a local maximum/minimum is largest/smallest relative to nearby points.

Unlike local extrema, global extrema need not be attained at stationary points. Instead, they could be attained along the boundary of the function's domain.

\begin{example}
    Consider the function $f(x, y) = x^2 + y^2$ with domain $-2 \leq x \leq 2$, $-2\leq y \leq 2$. Its global maximum is attained at $(2, 2)$, $(2, -2)$, $(-2, 2)$ and $(-2, -2)$, all of which lie on the boundary of the domain, and none of which are stationary points. However, its global minimum is attained at the stationary point $(0, 0)$.
    ...
    \begin{tikzpicture}[scale=0.7,trim axis left, trim axis right]
        \begin{axis}[
            ...
            \addplot3[surf,shader=faceted interp, colormap/blackwhite,opacity=0.7,domain=-2:2,y domain=-2:2] {x^2+ y^2};
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{The graph of $z = f(x, y)$. Points where the global maximum is attained are indicated.}
\end{figure}
\end{example}
...
\begin{proposition}[Second Partial Derivative Test]
    Let $(a, b)$ be a stationary point of $f(x, y)$ and define $D = f_{xx}(a, b) f_{yy}(a,b) - \bs{f_{xy}(a, b)}^2$.
    \begin{itemize}
        \item If $D > 0$, and
        \begin{itemize}
