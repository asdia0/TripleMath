...
In previous chapters, we learnt about differentiation, which can be thought as finding the derivative $f'(x)$ from a function $f(x)$. Reversing this, we define integration as the process of finding the function $f(x)$ from its derivative $f'(x)$. Simply put, integration ``undoes'' differentiation and vice versa.

\subsection{Indefinite Integration}

\begin{definition}
    The \vocab{primitive} of a function $f(x)$ is a function $F(x)$ such that $F'(x) = f(x)$.
\end{definition}

Note that a function may have multiple primitives. For instance, both $2\sin[2]{x}$ and $-\cos{2x}$ are primitives of $4\sin{x}\cos{x}$.

\begin{definition}
    The \vocab{indefinite integral} of a function $f(x)$, denoted $\int f(x) \d x$, is the class of functions $F(x) + C$, where $F(x)$ is a primitive of $f(x)$ and $C$ is a \vocab{constant of integration}. Here, $f(x)$ is called the \vocab{integrand}.
\end{definition}

Informally, the indefinite integral of $f(x)$ represents \emph{all} functions whose derivative is $f(x)$. The presence of $C$ is due to the fact that the derivative of a constant is 0.\footnote{More technically, we can view differentiation as a linear transformation $\mathrm{D}$ between vector spaces of functions. In performing integration, we are seeking all functions $F$ such that $\mathrm{D}F = f$. Since the kernel of $\mathrm{D}$ is the set of all constants $C$, the general solution is $F_p + C$, where $F_p$ is a particular solution (primitive).}

With our notation, we can write down the notion of integration ``undoing'' differentiation mathematically: \[\int \der{}{x} \bs{f(x)} \d x = f(x) + C \quad \tand \quad \der{}{x} \bs{\int f(x) \d x} = f(x).\]
...
\begin{fact}[Properties of Indefinite Integrals]
    ...
    \begin{itemize}
        \item $\displaystyle\int \bs{f(x) + g(x)} \d x = \int f(x) \d x + \int g(x) \d x$.
...
\begin{definition}
    Suppose $f$ is a continuous function defined on the interval $[a, b]$ and $F(x)$ is a primitive of $f(x)$. Then, the \vocab{definite integral} of $f(x)$ from $a$ to $b$ with respect to $x$ is denoted by \[\int_a^b f(x) \d x = \evalint{F(x)}{a}{b} = F(b) - F(a).\] We call $a$ the \vocab{lower limit} and $b$ the \vocab{upper limit} of the integral.
\end{definition}
...
\begin{fact}[Properties of Definite Integrals]
    ...
    \begin{itemize}
        \item $\displaystyle\int_a^b \bs{f(x) + g(x)} \d x = \int_a^b f(x) \d x + \int_a^b g(x) \d x$.
        \item $\displaystyle\int_a^b k f(x) \d x = k \int_a^b f(x) \d x$.
        \item $\displaystyle\int_a^b f(x) \d x = \int_a^c f(x) \d x + \int_c^b f(x) \d x$.
    \end{itemize}
\end{fact}

Note that from the last property, we deduce that \[\int_a^a f(x) \d x = 0 \quad \tand \quad \int_a^b f(x) \d x = -\int_b^a f(x) \d x.\]
...
Equivalently, \[\int \sec x \d x = \ln \abs{\sec x + \tan x} + C \quad \tand \quad \int \csc x \d x = -\ln \abs{\csc x + \cot x} + C.\]

Integrands involving products of trigonometric functions can be simplified with the following identities:
\begin{alignat*}{3}
...
Integrands involving powers of trigonometric functions can also be simplified with the following identities:
\begin{alignat*}{3}
...
\begin{recipe}[Integration by Substitution]
    Given an integral $\int f(x) \d x$ and a substitution $x = g(u)$, convert all instances of $x$ in terms of $u$. This includes replacing $\d x$ with $g'(u) \d u$. If the integral is definite, the bounds should also be converted to their corresponding $u$ values. If the integral is indefinite, once the integral is evaluated in terms of $u$, all instances of $u$ should be converted back to $x$.
\end{recipe}

\begin{sample}[Definite Integration by Substitution]
    Evaluate the definite integral \[\int_{2/\sqrt3}^2 \frac1{x\sqrt{x^2 - 1}} \d x.\]
\end{sample}
\begin{sampans}
    Under the substitution $x = 1/u$, we have \[\der{x}{u} = -\frac1{u^2},\] so $\d x = -u^{-2} \d u$. When $x = 2/\sqrt 3$, the corresponding $u$-value is $u = \sqrt3/2$. When $x = 2$, the corresponding $u$-value is $u = 1/2$. Thus, the integral becomes \[\int_{\sqrt3 / 2}^{1/2} \frac{u}{\sqrt{u^{-2} - 1}} \frac1{u^2} \d u = \int_{1/2}^{\sqrt3 /2} \frac1{\sqrt{1 - u^2}} \d u = \evalint{\arcsin u}{1/2}{\sqrt3 /2} = \frac\pi6.\]
\end{sampans}

\begin{sample}[Indefinite Integration by Substitution]
    Evaluate the indefinite integral \[\int \frac1{x\sqrt{x^2 - 1}} \d x.\]
\end{sample}
\begin{sampans}
    Under the substitution $x = 1/u$, we get \[\int \frac1{x\sqrt{x^2 - 1}} \d x = \int \frac1{\sqrt{1 - u^2}} \d u = \arcsin u + C.\] Recasting this answer back in terms of $x$, we have \[\int \frac1{x\sqrt{x^2 - 1}} \d x = \arcsin \frac1x + C.\] 
\end{sampans}
...
\begin{recipe}[LIATE]
    ...
    \begin{itemize}
        \item \bd{L}ogarithmic
        \item \bd{I}nverse trigonometric
        \item \bd{A}lgebraic
        \item \bd{T}rigonometric
        \item \bd{E}xponential
    \end{itemize}
\end{recipe}

\begin{sample}[Integration by Parts]
    Evaluate \[\int \ln x \d x.\]
\end{sample}
\begin{sampans}
    Picking $u = \ln x$ and $v' = 1$, we get \[\int \ln x \d x = uv - \int u' v \d x = (\ln x)(x) - \int \bp{\frac1x} (x) \d x = x\ln x - x + C.\]
\end{sampans}

The astute reader would have noticed that we actually dropped an arbitrary constant when integrating $v$ in the above example. From $v' = 1$ we took $v = x$ instead of the expected $v = x + C$. However, including the arbitrary constant does not matter: if we replace $v$ with $v + C$ into the integration by parts formula, we get \[\int u \d v = u(v + C) - \int (v + C) \d u = uv + Cu - \bp{\int v \d u + Cu} = uv - \int v \d u,\] which is what we would have got had we not included the arbitrary constant $C$.
...
\begin{sample}[Including Arbitrary Constant]
    Evaluate \[\int \ln{x + 1} \d x.\]
\end{sample}
\begin{sampans}
    Picking $u = \ln{x+1}$ and $v' = 1$ (which implies $v = x + C$), we get \[\int \ln{x+1} \d x = uv - \int u' v \d x = (x + C) \ln{x+1} - \int \frac{x+C}{x+1} \d x.\] Here, a convenient choice for $C$ would be 1, as the integral on the RHS would simplify to $\int 1 \d x$, which we can easily integrate. Thus, \[\int \ln{x+1} \d x = (x+1) \ln{x+1} - x + C.\]
\end{sampans}
...
\begin{recipe}[DI Method]
    ...
    \end{array}
    \]
    In other words, keep differentiating the middle column ($D$) and keep integrating the right column ($I$), while alternating the sign in the left column. This sign is ``attached'' to the $u$ terms.
