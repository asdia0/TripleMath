...
\end{theorem}
\begin{proof}
    We have $-\abs{a} \leq a \leq \abs{a}$ and $-\abs{b} \leq b \leq \abs{b}$. Adding both inequalities, we get $-\bp{\abs{a} + \abs{b}} \leq a + b \leq \abs{a} + \abs{b}$, so $\abs{a + b} \leq \abs{a} + \abs{b}$ as desired.
\end{proof}
...
\begin{definition}
    For real numbers $x_1, \dots, x_n$, the \vocab{arithmetic mean} is defined as $\sum_{i = 1}^n x_n/n$ and the \vocab{geometric mean} is defined as $\prod_{i = 1}^n x_i^{1/n}$.
\end{definition}
