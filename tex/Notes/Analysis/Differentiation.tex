...
\begin{definition}
    A function $f$ is said to be \vocab{differentiable} at some point $a$, if its domain contains an open interval containing $a$, and the limit \[L = \lim_{h \to 0} \frac{f(a+h) - f(a)}{h}\] exists. If $f$ is differentiable, then $L$ is the \vocab{derivative} of $f$ at $a$, denoted $\der{}{x} f(a)$. If the derivative exists for all points in the domain, we may define a \vocab{derivative function} that maps $x$ to the value of the derivative at $x$, denoted $\der{}{x} f(x)$ or $f'(x)$.
\end{definition}
