...
\begin{definition}
    A \vocab{sequence} or \vocab{progression} is a set of numbers $\bc{u_1, u_2, u_3, \dots}$ arranged in a defined order according to a certain rule. In general, $u_n$ is called the \vocab{$n$th term}.
\end{definition}
...
\begin{definition}
    A \vocab{series} is the sum of the terms of a sequence $u_n$. The sum to $n$ terms is denoted by $S_n = u_1 + u_2 + \dots + u_{n-1} + u_n$.
\end{definition}
...
\begin{definition}
    An \vocab{arithmetic progression} (AP) is a sequence $u_n$ in which each term differs from the preceding term by a constant called the \vocab{common difference}.
\end{definition}

\begin{proposition}
    The general term of an arithmetic progression with first term $a$ and common difference $d$ is $u_n = a + \bp{n-1} d$.
\end{proposition}
\begin{proof}
    Trivial.
\end{proof}
...
\begin{proposition}
    The $n$th term $S_n$ of an arithmetic series is given by \[S_n = \frac{n\bp{a + l}}{2},\] where $l = u_n = a + \bp{n-1} d$ is the last term of the arithmetic progression.
\end{proposition}
\begin{proof}
    Note that for all integers $k = 1, 2, \dots, n$, \[u_k + u_{n - k + 1} = \bs{a + (k-1)d} + \bs{a + (n-k)d} = a + \bs{a + (n-1)d} = a + l.\] Hence, by pairing the $k$th term with the $(n-k+1)$th term, we get \[2S_n = (u_1 + u_n) + (u_2 + u_{n-1}) + \dots + (u_{n-1} + u_2) + (u_n + u_1) = n\bp{a + l}.\] Dividing by two yields the claim.
\end{proof}

\section{Geometric Progression}

\begin{definition}
    A \vocab{geometric progression} (GP) is a sequence $u_n$ in which each term is obtained form the preceding one by multiplying a non-zero constant, called the \vocab{common ratio}. The first term of a GP is usually denoted by $a$ and the common ratio by $r$.
\end{definition}

In the case where the common ratio is 1, the geometric progression becomes an arithmetic progression.

\begin{proposition}
    The general term of a geometric progression with first term $a$ and common ratio $r$ is $u_n = a r^{n-1}$.
\end{proposition}
\begin{proof}
    Trivial.
\end{proof}
...
\end{proposition}
\begin{proof}
    By the definition of a series, we have \[S_n = a + ar + \dots + ar^{n-2} + ar^{n-1}. \tag{1}\] Multiplying both sides by $r$ yields \[r S_n = ar + ar^2 + \dots + ar^{n-1} + ar^{n}. \tag{2}\] Subtracting (2) from (1), we obtain $\bp{1-r}S_n = a - ar^{n}$. Since $r \neq 1$, we may divide by $1-r$ to find \[S_n = \frac{a(1 - r^{n})}{1 - r}.\]
