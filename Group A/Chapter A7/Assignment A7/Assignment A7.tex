    ...
    \solution
        \part
            ...
            \begin{alignat*}{2}
                ...
                \implies&&\vec a &= \lambda^2 \vec a\\
                \implies&&\lambda^2 &= 1
            \end{alignat*}

            Hence, $\lambda = \pm 1$, whence $\vec b = \pm \vec a$. Let $A= \abs{\vec a} = \abs{\vec b}$. Note that the cosine of the angle between $\vec a$ and $\vec b$ is $\pm 1$.
            \begin{align*}
                A &= \abs{a}\\
                &= \abs{(\vec a \cdot \vec b)\vec b}\\
                &= \abs{\vec a \cdot \vec b} \abs{\vec b}\\
                &= \abs{\vec a} \abs{\vec b} \abs{\pm 1} \abs{\vec b}\\
                &= A^3
            \end{align*}
            Hence, $A = 1$. Note that we reject $A = 0$ since $\vec a$ is a non-zero vector. Thus, $\abs{\vec b} = 1$.
