...
\begin{document}
    ...
    \solution
        \part          
            \boxt{Unique solution: $a = -9$, $b = 2$, $c = 0$}

        \part
            \boxt{No solution.}

        \part
            \boxt{Infinitely many solutions: $x_1 = 8-t$, $x_2 = t-3$, $x_3 = t$}
        ...
        \part
            ...
            \boxt{Unique solution: $p = \dfrac12$, $q = -\dfrac13$, $r = \dfrac16$}

        \part
            \begin{center}
                $\sin\a = 1$, $\cos\b = -1$, $\tan\g = 0$
            \end{center}

            \boxt{Unique solution: $\a = \dfrac\pi2$, $\b = \pi$, $\g = 0$}
    ...
    \solution 
        \part 
            ...
            \boxt{Solution set: $\bc{x \in \R \colon x \leq -\sqrt2 \lor x \geq \sqrt2}$}

        \part
            \begin{alignat*}{2}
                && 4x^2 - 12x + 10&>0\\
                \implies&& x^2 - 3x + \frac52 &> 0 \\
                \implies&& x^2 - 3x + \bp{\frac32}^2 - \bp{\frac32}^2 + \frac52 &> 0\\
                \implies&& \bp{x-\frac32}^2 + \frac{19}4 &> 0
            \end{alignat*}

            Since $\bp{x-\dfrac32}^2 \geq 0$, all $x \in \R$ satisfy the inequality.
        ...
        \part
            \begin{alignat*}{2}
                ...
                \implies&& \bp{x+2}^2 + 9 &< 0
            \end{alignat*}

            Since $\bp{x+2}^2 \geq 0$, there is no solution to the inequality.

            \boxt{Solution set: $\varnothing$}
        ...
        \part 
            ...
            \boxt{Solution set: $\bc{x \in \R \colon x < -3 \lor 0 < x < 2}$}
        ...
        \part
            ...
            \boxt{Solution set: $\bc{x \in \R \colon x \leq -3 \lor x = 0 \lor x \geq 1}$}
    ...
    \solution
        \part
            ...
            \boxt{Solution set: $\bc{x \in \R \colon -3 < x< -\dfrac13}$}

        \part
            \case{1}{} $x-2 < 2x \implies x > -2$

            \case{2}{} $-(x-2) < 2x \implies -x + 2 < 2x 3x > 2 \implies x > \dfrac23$

            Combining both inequalities, we have $x > \dfrac23$.

            \boxt{Solution set: $\bc{x \in \R \colon x > \dfrac23}$}
    ...
    \solution
        ...
        \boxt{Mr Mok invested \$30,000, \$5,000 and \$15,000 in Funds A, B and C respectively.}
    ...
    \solution
        \part
             \begin{alignat*}{2}
                &&2x-1 &\geq \frac6x, \qquad x \neq 0 \\
                \implies&& x^2(2x-1) &\geq 6x\\
                \implies&& x^2(2x-1) - 6x &\geq 0\\
                \implies&& x\big(x(2x-1) - 6\big) &\geq 0\\
                \implies&& x\bp{2x^2-x-6} &\geq 0\\
            ...
            \boxt{$-\dfrac32 \leq x < 0 \lor x \geq 2$}
        ...
        \part
            \begin{alignat*}{2}
                ...
                \implies&&x^3 - x &< x^2\\
                \implies&&x^3 - x^2 - x &< 0\\
                \implies&&x\bp{x^2 - x - 1} &< 0\\
                \implies&&x(x-\f)(x-\bar{\f}) &< 0
            \end{alignat*}
            ...
            \boxt{$x \leq \bar{\f} \lor 0 < x \leq \f$}
    ...
    \solution
        ...
        \part
            ...
            \boxt{$-1 < x < -0.916 \lor x > 0.518$}

    \problem{}
        Sketch the graphs of $y = \abs{x-20}$ and $y=\dfrac1x$ on the same diagram. Hence or otherwise, solve the inequality $\abs{x-20} < \dfrac1x$, leaving your answers correct to 2 decimal places.
    ...
    \solution
        ...
        \boxt{$0 < x < 0.05 \lor 19.95 < x < 20.05$}
    ...
    \solution
        \begin{alignat*}{2}
            ...
            \implies&& (x-9)\bp{x^2-9} &\leq \bp{x^2 - 9}^2\\
        ...
        \boxt{$x < -3 \lor 0 \leq x \leq 1 \lor x > 3$}
        ...
        \part
            ...
            \case{1}{} $\abs{x} < -3 \implies$ No solution.

            \case{2}{} $0 \leq \abs{x} \leq 1 \implies -1 \leq x \leq 1$

            \case{3}{} $\abs{x} > 3 \implies x < -3 \lor x > 3$

            Combining all three cases, we finally have

            \boxt{$x < -3 \lor -1 \leq x \leq 1 \lor x > 3$}
        ...
        \part
            ...
            \case{1}{} $-x < -3 \implies x > 3$
            
            \case{2}{} $0 \leq -x \leq 1 \implies -1 \leq x \leq 0$

            \case{3}{} $-x > 3 \implies x < -3$

            Combining all three cases, we finally have

            \boxt{$x < -3 \lor -1 \leq x \leq 0 \lor x > 3$}
    ...
    \solution
        ...
        \part
            ...
            \boxt{The rocket will remain at or above 100 m from the ground for 3 seconds.}
    ...
    \solution
        ...
        \boxt{Xinxin will stop donating in the 11th quarter.}        
\end{document}
