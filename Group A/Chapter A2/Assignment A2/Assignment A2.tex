...
\begin{document}
    ...
    \solution
        ...
        Let $f(x) = x+4\cos x$. Let $\beta$ be the negative root of the equation $f(x) = 0$. Using linear interpolation on the interval $[-1.5. -1]$,

        \begin{align*}
            \beta &= \dfrac{-1.5f(-1) - (-1)f(-1.5)}{f(-1)-f(1.5)}\\
            &= -1.24 \todp{2}
        \end{align*}
    ...
    \solution
        ...
        Hence, $x = 0$ or $x = -\dfrac23$. When $x = 0$, $y = 0$. When $x = -\dfrac23$, $y = \dfrac4{27}$. Thus, the coordinates of the stationary points of $y = x^3 + x^2$ are $(0, 0)$ and $\left(-\dfrac23, \dfrac4{27}\right)$.

        \boxt{
            $(0, 0)$, $\left(-\dfrac23, \dfrac4{27}\right)$
        }
        ...
        \begin{center}
            \begin{tikzpicture}[trim axis left, trim axis right, scale=0.86]
                \begin{axis}[
                    ...
                    \fill (-2/3, 4/27) circle[radius=2.5pt] node[anchor=south] {$\left(-\dfrac23, \dfrac4{27}\right)$};
                \end{axis}
            \end{tikzpicture}
        \end{center}

        Therefore, $k \in \left(0, \dfrac4{27}\right)$.

        \boxt{
            $\{k \in \mathbb{R} \colon 0 < k < \dfrac4{27} \}$
        }
        ...
        \part
            For small $x$, $x^3$ is negligible when compared to $x^2$. Consider $g(x) = x^2-0.1$. Then the positive root of $g(x)=0$ is approximately $\alpha$. Hence, an alternative approximation to $\alpha$ is $\sqrt{0.1} = 0.316 \tosf{3}$.
    ...
    \solution
        \part
            Consider $f(x) = 2\cos x$. Then $f^\prime(x) = -2\sin x$.

            Observe that $\sin x$ is increasing on $[1, 1.2]$. Since $\sin 1 > \dfrac12$, $\abs{f^\prime(x)} > 1$ for all $x \in [1, 1.2]$. Thus, fixed-point iteration fails and will not converge to $\alpha$.
        ...
        \part
            Consider $f(x) = \cos x + \dfrac12 x$. Then $f^\prime(x) = -\sin x + \dfrac12 -\left(\sin x - \dfrac12\right)$. Since $0 \leq \sin x \leq 1$ for $x \in \left[0, \dfrac{\pi}2\right]$, and $[1, 1.2] \subset \left[0, \dfrac{\pi}2\right]$, we know $-\dfrac12 \leq \sin x - \dfrac12 \leq \dfrac12$ for $x \in [1, 1.2]$. Thus, $0 \leq \abs{\sin x - \dfrac12} \leq \dfrac12$ for $x \in [1, 1.2]$. Hence, fixed-point iteration will work and converge to $\alpha$.
        ...
        \part
            Consider $f(x) = \dfrac23 (\cos x + x)$. Then $f^\prime(x) = \dfrac23(-\sin x + 1)$.

            For fixed-point iteration to converge to $\alpha$, we need $\abs{f^\prime(x)} < 1$ for $x$ near $\alpha$. It thus suffices to show that $\abs{-\sin x + 1} < \dfrac32$ for all $x \in [1, 1.2]$. Observe that $1 - \sin x$ is strictly decreasing and positive for $x \in \left[0, \dfrac{\pi}2\right]$. Since $1 - \sin 1 < \dfrac32$, and $[1, 1.2] \subset \left[0, \dfrac{\pi}2\right]$, we have that $\abs{-\sin x + 1} < \dfrac32$ for all $x \in [1, 1.2]$. Thus, $\abs{f^\prime(x)} < 1$ for $x$ near $\alpha$. Hence, fixed-point iteration will work and converge to $\alpha$.
