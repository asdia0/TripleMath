...
\begin{document}
    ...
    \problem{}
        A curve $E$ has polar equation $r = \dfrac3{\sqrt{\cos^2\theta + (\sin\theta - \cos\theta)^2}}$ for $0 \leq \theta < 2\pi$.
        ...
        \begin{enumerate}
            ...
            \item Hence or otherwise, find the exact Cartesian coordinates of the point(s) of intersection between $E$ and the graph with polar equation $r = \dfrac1{\sin \theta - \cos \theta}$ for $\dfrac\pi4 < \theta < \dfrac{5\pi}4$.
        \end{enumerate}

    \solution
        \part
            \begin{alignat*}{2}
                &&r &= \dfrac3{\sqrt{\cos^2\theta + (\sin\theta - \cos\theta)^2}}\\
                \implies&&r \sqrt{\cos^2\theta + (\sin\theta - \cos\theta)^2} &= 3\\
                \implies&&r^2 \left(\cos^2\theta + (\sin\theta - \cos\theta)^2\right) &= 9\\
                \implies&&r^2\left(2\cos^2\theta - 2\cos\theta\sin\theta + \sin^2\theta\right) &= 9\\
                \implies&&2(r\cos\theta)^2 - 2(r\cos\theta)(r\sin\theta) + (r\sin\theta)^2 &= 9\\
        ...
        \part
            \begin{alignat*}{2}
                &&r &= \dfrac1{\sin\theta - \cos\theta}\\
                \implies&&r(\sin \theta - \cos\theta) = 1\\
                \implies&&r\sin\theta - r\cos\theta &= 1\\
                \implies&&y - x &= 1\\
                \implies&&y &= x + 1
            \end{alignat*}
            ...
            \boxt{
                $\coord{2\sqrt{2}}{1+2\sqrt{2}}$, $\coord{-2\sqrt{2}}{1-2\sqrt{2}}$
            }

    \problem{}
        The function $f$ is defined by
        \begin{equation*}
            f(x) = 2 - \dfrac1x, \, x \in \mathbb{R}, x \geq 1
        \end{equation*}
    ...
    \solution
        \part
            \begin{alignat*}{2}
                &&f^2(x) &= 2 - \dfrac1{f(x)}\\
                && &= 2 - \left(2 - \frac1x\right)^2\\
                && &= \dfrac{3x - 2}{2x - 1}\\
                \implies&&f^3(x) &= 2 - \dfrac1{f^2(x)}\\
                && &= 2 - \left(\frac{3x - 2}{2x - 1}\right)^{-1}\\
                && &= \dfrac{4x - 3}{3x - 2}\\
                \implies&&f^4(x) &= 2 - \dfrac1{f^3(x)}\\
                && &= 2 - \left(\frac{4x - 3}{3x - 2}\right)^{-1}\\
                && &= \dfrac{5x - 4}{4x - 3}
            \end{alignat*}

            \boxt{
                $f^2(x) = \dfrac{3x - 2}{2x - 1}$, $f^3(x) = \dfrac{4x - 3}{3x - 2}$, $f^4(x) = \dfrac{5x - 4}{4x - 3}$
            }

            \noindent \textbf{Conjecture.} For all $n \in \mathbb{N}$, $f^n(x) = \dfrac{(n+1)x - n}{nx - (n-1)}$.

        \part
            Let $P_n$ be the statement that $f^n(x) = \dfrac{(n+1)x - n}{nx - (n-1)}$ for $n \in \mathbb{N}$.

            \noindent \textbf{Base Case.} $n = 1 : f^1(x) = 2 - \dfrac1x = \dfrac{2x - 1}{x - 0}$. Hence, $P_1$ is true.
            
            \noindent \textbf{Inductive Hypothesis.} Assume $P_k$ is true for some $k \in \mathbb{N}$.

            \noindent \textbf{Inductive Step.} Consider $f^{k+1}(x)$.
            \begin{align*}
                f^{k+1}(x) &= 2 - \dfrac1{f^k(x)}\\
                &= 2 - \left(\dfrac{(k+1)x - k}{kx - (k-1)}\right)^{-1}\\
                &= 2 - \dfrac{kx - (k-1)}{(k+1)x - k}\\
                &= \dfrac{2\left[(k+1)x - k\right] - \left[kx - (k-1)\right]}{(k+1)x - k}\\
                &= \dfrac{(k+2)x - (k+1)}{(k+1) - k}
            \end{align*}

            \noindent Hence, $P_k \implies P_{k+1}$. Since $P_1$ is true, by induction, $P_n$ is true for all $n \in \mathbb{N}$.

        \part
            Since $f^n(x) = \dfrac{(n+1)x - n}{nx - (n-1)}$ for all $n \in \mathbb{N}$, we have the restriction $nx - (n-1) \neq 0 \implies x \neq \dfrac{n-1}{n}$. Hence,
            \boxt{
                $A = \mathbb{R}\setminus\left\{n \in \mathbb{N} : \dfrac{n-1}{n}\right\}$
            }

    \problem{}
        A curve $T$ has polar equation $r = \sqrt{\dfrac2{\sin 3\theta}}$ where $-\pi < \theta \leq \pi$.

        \begin{enumerate}
            \item Determine the range of values of $\theta$ for which the value of $r$ is undefined. Hence state the equations of the asymptotes of $T$ in polar form.
    ...
    \solution
        \part
            For $r$ to be undefined, either $\sin 3\theta = 0$ or $\dfrac2{\sin3\theta} < 0$. In the latter case, $\sin3\theta < 0$. Hence, it suffices to determine the range of values of $\theta$ such that $\sin3\theta \leq 0$. Note that $- \pi < \theta \leq \pi \implies -3\pi < 3\theta \leq 3\pi$. Hence,
            \begin{equation*}
                3\theta \in (-3\pi, -2\pi] \cup [-\pi, 0] \cup [\pi, 2\pi] \cup \{3\pi\}\\
            \end{equation*}

            \boxt{
                $r$ is undefined for $\theta \in \left(-\pi, -\dfrac23\pi\right] \cup \left[-\dfrac13\pi, 0\right] \cup \left[\dfrac13\pi, \dfrac23\pi\right] \cup \{\pi\}$.
            }

            \boxt{
                Asymptotes: $\theta = -\dfrac23 \pi, -\dfrac13\pi, 0, \dfrac13\pi, \dfrac23\pi, \pi$.
            }
            ...
            \begin{center}
                \begin{tikzpicture}[trim axis left, trim axis right]
                    \begin{axis}[
                        ...
                        \node[anchor=south west] at (-2, 0) {$\theta = \pi$};

                        \node[anchor=north west] at (1, 2) {$\theta = \frac13\pi$};

                        \node[anchor=north east] at (-1, 2) {$\theta = \frac23\pi$};

                        \node[anchor=south west] at (1, -2) {$\theta = -\frac13\pi$};

                        \node[anchor=south east] at (-1, -2) {$\theta = -\frac23\pi$};

                        \addlegendentry{$T$};

                        \fill (0, -1.414) circle[radius=2.5pt] node[anchor=south] {$\coord{-\frac\pi2}{\sqrt2}$};
                    \end{axis}
                \end{tikzpicture}
            \end{center}
    ...
    \solution
        ...
        \part
            \subpart
                ...
                \noindent Let $l = \lim_{n \to \infty} u_n$.
                \begin{alignat*}{2}
                    &&l &= \dfrac{l^2 + 5}{2l + 4}\\
    ...
    \problem{}
        ...
        \noindent 8 Omega variant cases were reported in the first week of the outbreak, while 15 new cases were reported the week after.
    ...
    \solution
        \part
            Let $\Delta x_n$ be the increase in cases from week $n$ to $n+1$. We have the relations
            \begin{equation*}
                x_n = x_{n-1} + \Delta x_{n-1} \text{ and } \Delta x_n = k\Delta x_{n-1}
            \end{equation*}
            Note that $x_{n-1} = x_{n-2} + \Delta x_{n-2} \implies \Delta x_{n-2} = x_{n-1} - x_{n-2}$. Then
            \begin{align*}
                x_n &= x_{n-1} + \Delta x_{n-1}\\
                &= x_{n-1} + k\Delta x_{n-2}\\
                &= x_{n-1} + k\left(x_{n-1} - x_{n-2}\right)\\
                &= (1+k)x_{n-1} -kx_{n-2}
            \end{align*}
        ...
        \part
            ...
            \begin{align*}
                x_n &= \left(8 - \dfrac{15}{k-1}\right) + \left(\dfrac{15}{k(k-1)}\right)k^n\\
                &= 8 - \dfrac{15}{k-1} + \dfrac{15}{k-1} k^{n-1}\\
                &= 8 + \dfrac{15}{k-1}\left(k^{n-1} - 1\right)
            \end{align*}

            \boxt{
                $x_n =  8 + \dfrac{15}{k-1}\left(k^{n-1} - 1\right)$
            }

        \part
            Suppose $k > 1$. Then
            \begin{align*}
                \lim_{n \to \infty} x_n &= \lim_{n \to \infty} \left[8 + \dfrac{15}{k-1}\left(k^{n-1} - 1\right)\right] \to \infty
            \end{align*}
            Hence, everyone in Singapore will get the Omega variant, i.e. the Omega variant will not stop spreading.

            \medskip
            
            \noindent Suppose $0 < k < 1$. Then
            \begin{align*}
                \lim_{n \to \infty} x_n &= \lim_{n \to \infty} \left[8 + \dfrac{15}{k-1}\left(k^{n-1} - 1\right)\right] = 8 - \dfrac{15}{k-1} = 8 + \dfrac{15}{1-k}
            \end{align*}
        ...
        \part
            The weekly infection growth rate $k$ is unlikely to be a constant. For instance, as most of the population gets the Omega variant, there are less people available for the Omega variant to infect, hence $k$ will start to decrease.

        \part
            Observe that
            \begin{align*}
                x_n - x_{n-1} &= (x_{n-1} + \Delta x_{n-1}) - x_{n-1}\\
                &= \Delta x_{n-1}
            \end{align*}
            If the weekly infection growth rate is 1, $\Delta x_n$ is constant, whence $x_n$ is in arithmetic progression.
        ...
        \part
            When $k = 4$,
            \begin{align*}
                x_4 &= 8 + \dfrac{15}3 \left(4^3 - 1\right) = 323\\
                x_5 &= 8 + \dfrac{15}3 \left(4^4 - 1\right) = 1283\\
            \end{align*}
            Hence, $\Delta x_4 = 1283 - 323 = 960$. Let $x_n'$ denote the total number of Omega variant cases in Singapore within the first $n$ weeks of the outbreak in the given simulation. Since $k$ becomes 1 starting from week 5, we have
            \begin{align*}
                x_n' &= x_5 + \Delta x_5 (n-5), \, n \geq 5\\
                &= 1283 + 960(n-5)\\
                &= 960n - 3517
            \end{align*}
            ...
            \end{alignat*}
            \boxt{
                Alert Orange will be triggered in week 15.
            }
\end{document}