...
\begin{document}
    ...
    \problem{}
        The terms in the sequence $u_0$, $u_1$, $u_2$, $\ldots$ satsify the recurrence relation
        \begin{equation*}
    ...
    \problem{}
        A curve is defined parametrically by $x = \dfrac{t^2}{1 + t^2}$, $y = t^3 - \lambda t$, where $\lambda$ is a positive constant.

        \begin{enumerate}
            \item Sketch the curve, stating the equation of its asymptote.
            \item Find in terms of $\lambda$, the $x$-coordinate of the point $P$ where the curve intersects itself.
            \item Show that the area of the region bounded by the curve between $P$ and the origin is given by an integral of the form
            \begin{equation*}
                4\int_0^{f(\lambda)} g(t^2) \d t
            \end{equation*}
            where $f(\lambda)$ is a function of $\lambda$ and $g(t^2)$ is a function of $t^2$ to be determined.
        \end{enumerate}
    ...
    \solution
            ...
            \begin{center}
                \begin{tikzpicture}[trim axis left, trim axis right]
                    \begin{axis}[
                        ...
                        \addlegendentry{$x = \frac{t^2}{1 + t^2}$, $y = t^3 - \lambda t$};
        ...
        \part
            ...
            \begin{alignat*}{2}
                &&y &= 0\\
                \implies&&t^3 - \lambda t &= 0\\
                \implies&&t^2 - \lambda &= 0
            \end{alignat*}
            Hence, $\lambda = t^2$, whence $x = \dfrac{t^2}{1 + t^2} = \dfrac{\lambda}{1 + \lambda}$.
            \boxt{
        ...
        \part
            ...
            Hence,
            \boxt{
                $f(\lambda) = -\sqrt{\lambda}$, $g(t^2) = \dfrac{t^2 \left(t^2 - \lambda\right)}{(1+t^2)^2}$
            }
    ...
    \solution
        ...
        \part
            ...
            \begin{alignat*}{2}
                ...
                \implies&&x_4 &= \nrm{x_3}{f} = 0.40964
            \end{alignat*}
            Since $f(0.405) = 0.02 > 0$ and $f(0.415) = -0.02 < 0$, $\alpha \in (0.405, 0.415)$. Thus,
            \boxt{
                $\alpha = 0.41 \todp{2}$
            }
        ...
        \part
            ...
            \noindent When $x_1^\ast = 1$,the tangent to the curve intersects the $x$-axis at a negative $x$-value, thus giving a negative $x_2$. Since $f(x)$ is only defined for $x \geq 0$ due to the presence of $\sqrt{x}$, $f(x_2)$ and thus $x_3$ will be undefined. Hence, the Newton-Raphson method will fail to given an approximation to $\alpha$.

    \problem{}
        \begin{enumerate}
            \item For a positive constant $a$, there is an angle $\phi$ such that $\sin \phi = a$ and $\dfrac\pi2 < \phi < \pi$.
            
            Evaluate $\displaystyle\int_{-1}^0 \dfrac1{\sqrt{1 - a^2x^2}} \d x$, leaving your answer in terms of $a$, $\phi$ and $\pi$.
    ...
    \solution
        \part
            ...
            \boxt{
                $\displaystyle\int_{-1}^0 \dfrac1{\sqrt{1 - a^2x^2}} \d x = \dfrac{\pi - \phi}{a}$
            }
        ...
        \part
            ...
            \end{align*}
            \boxt{
                $\displaystyle\int \dfrac{\cos x}{1 + \cos x - \sin x} \d x = \dfrac{x}{2} + \ln \abs{\sec \dfrac{x}2} + C$
            }
            ...
            \begin{center}
                \begin{tikzpicture}[trim axis left, trim axis right]
                    \begin{axis}[
                        ...
                        \fill (1.5, 0) circle[radius=2.5pt] node[anchor=north west] {$\coord{k}0$};
            ...
            \begin{center}
                \begin{tikzpicture}[trim axis left, trim axis right]
                    \begin{axis}[
                        ...
                        \draw pic [draw, angle radius=8mm, "$\theta$"] {angle = P1--K--A};
                        \draw pic [draw, angle radius=7mm, "$\theta$"] {angle = B--K--P1};
                        \draw pic [draw, angle radius=8mm, "$\frac\pi4$"] {angle = C--K--B};
                    \end{axis}
                \end{tikzpicture}
            \end{center}

            \noindent Let $l$ be the line with equation $y = m(x-k)$. Since $l$ is a line of symmetry of $G$, $l$ bisects the angle between the asymptotes. Since the asymptote $y = x - k$ makes an angle $\dfrac\pi4$ with the point $\coord{k}0$, we have
            \begin{equation*}
                \theta + \theta + \dfrac\pi4 = \pi
            \end{equation*}
            whence $\theta = \dfrac38 \pi$. Thus, $l$ makes an angle $\theta + \dfrac\pi4 = \dfrac78 \pi$ with the point $\coord{k}0$, giving it a gradient of $\tan \dfrac78 \pi$. Hence,
    ...
    \problem{}
        It is given that $I_n = \displaystyle\int_0^\pi \cos^n (2\theta) \d \theta$, where $n$ is a positive integer.
    ...
    \solution
        \part
            \begin{align*}
                I_2 &= \int_0^\pi \cos^2 (2\theta) \d \theta\\
                &= \int_0^\pi \dfrac{\cos 4\theta + 1}2 \d \theta \usub{u &= 4\theta\\\d u &= 4 \d \theta}\\
        ...
        \part
            \begin{align*}
                I_n &= \int_0^\pi \cos^n 2\theta \d \theta\\
                &= \int_0^\pi \cos 2\theta \cos^{n-1} 2\theta \d \theta
            \end{align*}
            Note that $\der{}{\theta} \cos^{n-1} 2\theta = -2(n-1) \sin 2\theta \cos^{n-2} 2\theta$. Integrating by parts, we have
            \begin{equation*}
                \begin{array}{r c @{\hspace*{1.0cm}} c}\toprule
                    & D & I \\\cmidrule{1-3}
                    + & \cos^{n-1} 2\theta & \cos 2\theta \\
                    - & -2(n-1) \sin 2\theta \cos^{n-2} 2\theta & \dfrac12 \sin 2\theta \\\bottomrule
                \end{array}
            \end{equation*}
            \begin{alignat*}{2}
                &&I_n &= \eval{\dfrac12 \sin 2\theta \cos^{n-1} 2\theta}{0}{\pi} + (n-1) \int_0^\pi \sin^2 2\theta \cos^{n-2} 2\theta \d \theta\\
                && &= (n-1) \int_0^\pi \sin^2 2\theta \cos^{n-2} 2\theta \d \theta\\
                && &= (n-1) \int_0^\pi \left(1 - \cos^2 2\theta\right) \cos^{n-2} 2\theta \d \theta\\
                && &= (n-1) \int_0^\pi \left(\cos^{n-2} 2\theta  - \cos^n 2\theta\right) \d \theta\\
        ...
        \part
            Note that $I_1 = \displaystyle\int_0^\pi \cos 2\theta \d \theta = \dfrac12 \eval{\sin 2\theta}{0}{\pi} = 0$. For all odd $n$, $I_n$ will eventually reduce to $I_1$ with the recurrence relation derived above. Hence, $I_n = 0$ for odd $n$, which is independent of $n$.
    ...
    \problem{}
        ...
        \noindent The sales manager sets a target for the club membership to reach 750 by the end of 6 months.
    ...
    \solution
        ...
        \part
            ...
            \boxt{
                $M_n = (1-p)^{n-1} \left(500 -\dfrac{k}{p}\right) + \dfrac{k}{p}$
            }
        ...
        \part
            ...
            \end{alignat*}
            From G.C., $k > 111.05 \todp{2}$. Since $k \in \mathbb{N}$, the least $k$ is 112.

            \boxt{
                The club must recruit at least 112 members each month.
            }
