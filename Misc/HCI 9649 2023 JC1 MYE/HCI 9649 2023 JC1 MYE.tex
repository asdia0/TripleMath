...
\begin{document}
    ...
    \solution
        \part
            ...
            \begin{alignat*}{2}
                ...
                \implies&&\left(r^2\right)^2 + 2a^2\left((r\sin\t)^2 - (r\cos\t)^2\right)&= 0\\
                \implies&&r^4 + 2a^2r^2\left(\sin^2\t - \cos^2\t\right)&= 0\\
    ...
    \solution
        ...
        \part
            ...
            \begin{equation*}
              ...
              \boxt{$\displaystyle\int_0^1 ye^{2y-1} \d y = \dfrac14 \left(e + e^{-1}\right)$}
        ...
        \part
            ...
            \boxt{$\displaystyle\int_{e^{-1}}^e (\ln x + 1)^2 \d x = 2\left(e - e^{-1}\right)$}
    ...
    \solution
        ...
        \end{table}
         ...
         From the First Derivative Test, we see that $y = \sqrt{\dfrac23}$ is a minimum point. Hence, $h(y)$ is strictly decreasing on the interval $\left[0, \sqrt{\dfrac23}\right)$ and strictly increasing on the interval $\left(\sqrt{\dfrac23}, \infty\right)$. Since $h(0) = -1 < 0$, we have that $h(y) < 0$ for all $y \leq \sqrt{\dfrac23}$. Since $h(y)$ is strictly increasing for $y > \sqrt{\dfrac23}$, and $h\left(\sqrt{\dfrac23}\right) < 0$, $h(y)$ has only one real root. Thus, $g(x)$ has only one real root.
    ...
    \solution
        \part
            ...
            \end{equation*}
            \begin{alignat*}{8}
                &&\cos \angle OCF &= \dfrac{CF}{OC} &\implies&& \cos \left(\dfrac\pi2 - \t\right) &= \dfrac{CF}{2a} &\implies&& CF &= 2a\sin\t\\
                &&\cos \angle PCG &= \dfrac{CG}{PC} &\implies&& \cos \left(\dfrac\pi2 - 2\t\right) &= \dfrac{CG}{a} &\implies&& CG &= a\sin2\t
            \end{alignat*}
        ...
        \part
            ...
            \begin{align*}
                \left(\der{x}{\t}\right)^2 &+ \left(\der{y}{\t}\right)^2\\
                &= \left(-2a\sin\t + 2a\sin2\t\right)^2 + \left(2a\cos\t - 2a\cos2\t\right)^2\\
                &= 4a^2\Big[\left(-\sin\t + \sin2\t\right)^2 + \left(\cos\t - \cos2\t\right)^2\Big]\\
                &= 4a^2\Big[\sin^2\t - 2\sin\t\sin2\t + \sin^2 2\t + \cos^2\t - 2\cos\t\cos2\t + \cos^2 2\t\Big]\\
                &= 4a^2\Big[\left(\sin^2\t + \cos^2\t\right) + \left(\sin^2 2\t + \cos^2 2\t\right) - 2\left(\sin\t\sin2\t + \cos\t\cos2\t\right)\Big]\\
                &= 4a^2\left(1 + 1 - 2\cos\t\right)\\
                &= 8a^2\left(1 - \cos\t\right)
            \end{align*}
            Hence, the length of the path of $P$ is given by
            {\allowdisplaybreaks
            \begin{align*}
                \text{Length} &= \int_0^{2\pi} \sqrt{\left(\der{x}{\t}\right)^2 + \left(\der{y}{\t}\right)^2} \d \t\\
                &= \int_0^{2\pi} \sqrt{8a^2(1 - \cos\t)} \d \t\\
                &= \sqrt8 a \int_0^{2\pi} \sqrt{1 - \cos\t}\d \t\\
                &= \sqrt8 a \int_0^{2\pi} \sqrt{2\sin^2 \left(\dfrac\t2\right)}\d \t\\
                &= 4 a \int_0^{2\pi} \sin \left(\dfrac\t2\right) \d \t \usub{u &= \t/2\\\d u &= \d \t/2}\\
    ...
    \solution
        ...
        \part
            ...
            \boxt{$u_k = \dfrac16 \left(\dfrac67\right)^k \left( m - 360\right) + 60$}
